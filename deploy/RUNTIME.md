...
| `0 * * * *`           | `health:summary`                                                                                                                                                                                                                                                              |
| `10 3 * * *`          | `scripts/ops/backup-db-r2-api.sh` — установка: `sudo -u salpha bash scripts/ops/install-vps-pg-backup-cron.sh` |
...
| `*/10 * * * *`        | `jupiter-shadow-watch.mjs` — доля ошибок в `signal-lab.jsonl` / `mtm-shadow.jsonl` (Jupiter lite-api); **Telegram ALERT** при пороге (`SHADOW_WATCH_*`); установка: `sudo bash scripts/cron/install-jupiter-shadow-watch-cron-salpha.sh`; лог `data/logs/jupiter-shadow-watch.log` |
| `0 * * * *`           | `jupiter-shadow-hourly.mjs` — почасовая сводка (signal-lab + mtm-shadow) в **лог** `data/logs/jupiter-shadow-hourly.log`; в Telegram **не шлёт** без `JUPITER_SHADOW_HOURLY_TELEGRAM=1`. При `errs=0` в TG не шлёт (кроме `JUPITER_SHADOW_HOURLY_SEND_IF_ZERO_ERRORS=1`). Установка: `sudo bash scripts/cron/install-jupiter-shadow-hourly-cron-salpha.sh` |
...
### Hourly Telegram — Health · таблица `swaps` (≥ sa-alpha-1.11.102)

**Нормативная семантика:** строка **Health · swaps** в `scripts-tmp/hourly-telegram-report.mjs` помечает **OK/STALE по ингесту**: сравнивается **`now()` − max(`created_at`)`** с порогом **`HOURLY_HEALTH_SWAPS_INGEST_MAX_MIN`** (по умолчанию **30** минут; см. `.env.example`). Поле **`block_time`** — время **блока на цепи** у события; при контуре **wallet-backfill** / **sigseed** (не near-tip firehose) **`max(block_time)`** может отставать от часов **десятки минут** без остановки записи. Поэтому в том же сообщении выводится **`chain_lag`** = `now() − max(block_time)` **для контекста**, а не как отдельный триггер STALE.

**Чеклист при подозрении на реальный застой (оба лага порядка часов и дольше):**

1. **`SELECT count(*) FROM swaps WHERE created_at >= now() - interval '15 minutes'`** — ноль или почти ноль при отсутствии других объяснений указывает на остановку ингеста.
2. Логи **`data/logs/wallet-backfill-pilot-*.log`**, **`data/logs/sigseed-run.log`**, **`npm run wallet-backfill:metrics`** (SQL, без RPC).
3. Лимиты **QuickNode** / **`sa_qn_global_daily`** / коды остановки в JSON прогонов sigseed/backfill.

Отдельный near-real-time контур в **`swaps`** без отдельного бюджета RPC в этом документе **не требуется** для интерпретации Health.

### Dip-bot intel (W9.0, тег `dip_bot`, якоря только Live Oscar)

Норматив: **[`docs/strategy/specs/W9.0_dip_bot_intel_spec.md`](../docs/strategy/specs/W9.0_dip_bot_intel_spec.md)** §11.

| Режим | Cron (UTC, `salpha`) | Назначение |
|--------|----------------------|------------|
| **Bootstrap** (начальный сбор) | `25 2 * * *` — **ежедневно** 02:25 | Быстро накопить первые `dip_bot`; выход из режима по критериям в спеки |
| **Steady** | `25 2 * * 2` — **по вторникам** 02:25 | Актуализация раз в неделю |

Слот **02:25** выбран вне коридора **03:10–04:35** и вне почасовых **:08/:18** sigseed. Установка (идемпотентно): **`sudo bash /opt/solana-alpha/scripts/cron/install-dip-bot-cron-salpha.sh`** — по умолчанию **steady** (вт **02:25**). **Bootstrap** (ежедневно **02:25**, §11.3): **`DIP_BOT_CRON_BOOTSTRAP=1 sudo bash /opt/solana-alpha/scripts/cron/install-dip-bot-cron-salpha.sh`**. Произвольное расписание: **`DIP_BOT_CRON_SCHEDULE='…' sudo bash …`**. Ручной прогон: `cd /opt/solana-alpha && npm run dip-bot-intel:run` (нужны **`DATABASE_URL`** и строки в **`swaps`** по якорным mint). **v1:** только чтение Postgres, без QuickNode в джобе.

#### Аналитика: почему по якорным mint не было строк в `swaps`

Исторически **`swaps`** наполнялись декодером **только транзакций pump.fun bonding curve** (`Instruction: Buy/Sell` на программе pump). Live Oscar исполняет сделки через **Jupiter**; на цепочке типичны **PumpSwap AMM**, **Raydium**, **Orca**, **Meteora** — они **не проходили** узкий pump.fun‑декодер, поэтому **`sigseed:run`** и **`wallet-backfill`** могли получать сигнатуры по mint, но **не вставлять** строки.

**Исправление (продукт 1.11.99+):** **`src/parser/allowlisted-dex-swap.ts`** — после точного pump.fun‑разбора, если в транзакции есть одна из разрешённых программ (**Jupiter v6/v4**, **PumpSwap**, **Raydium** AMM v4 / CLMM / **CPMM** / **route**, **Orca** Whirlpool, **Meteora** DLMM / **DAMM v2**, **Phoenix**, **Lifinity v2**, **Moonit/Moonshot** launchpad `MoonCV…`, pump.fun), выполняется **балансовый** разбор (WSOL / лампорты / USDC / USDT как quote). Подключено к **`npm run sigseed:run`** и **`wallet-backfill`**. **`sa-parser`** по-прежнему только pump.fun Buy/Sell на стриме (точность по типу событий).

**После деплоя:** прогнать **`sigseed`** по очереди для актуальных mint **или** **`wallet-backfill`** по нужным кошелькам → **`npm run dip-bot-intel:coverage`** → **`npm run dip-bot-intel:run`**.

**Смягчение порогов v1 (операционно):** в **`.env`** задайте **`DIP_BOT_T_PRE_MS`** (мс до входа по якорю; типично **180000** = 3 мин), **`DIP_BOT_MIN_USD_ONE_EVENT`** (порог USD на агрегат покупателя в пре‑окне; типично **15** при живом инжесте), **`DIP_BOT_MIN_HITS`** (сколько якорей нужно кошельку для тега; типично **2** для bootstrap). Контракт переменных — **`.env.example`**. Ужесточайте пороги после появления первых **`dip_bot_intel`**.

**Повторный полный разбор якорей:** джоба пропускает пары `(anchor_mint, anchor_entry_ts_ms)` уже в **`dip_bot_intel_anchors_processed`**. Чтобы пересчитать историю с новыми порогами: под **`postgres`** удалите строки **`dip_bot_intel_observations`**, **`dip_bot_intel_anchors_processed`**, теги **`wallet_tags`** с **`tag=dip_bot`** и **`source=dip_bot_intel`**, выставьте **`dip_bot_intel_state.last_jsonl_offset_bytes = 0`**, затем несколько раз **`npm run dip-bot-intel:run`** (батчи до исчерпания хвоста JSONL).

#### Провал приёмки на проде (зафиксировано интегратором)

Джоба **не считается рабочей по смыслу W9.0**, если после прогона **`dip_bot_intel_observations`** и теги **`dip_bot` / `dip_bot_intel`** остаются нулевыми **из‑за отсутствия строк в `swaps` по якорным `base_mint`**, а не из‑за порогов USD/hits. На VPS после первого боевого прога именно это и произошло: **`anchors_processed` > 0**, **`buyer_rows = 0`** при отсутствии строк в **`swaps`** для класса сделок Live Oscar (**до расширения декодера** — разрыв инжеста относительно гипотезы W9 v1).

**Обязательная проверка перед отчётом «продукт в бою»:**  
`cd /opt/solana-alpha && npm run dip-bot-intel:coverage`  
— при коде выхода **2** и вердикте **`non_functional_v1_all_anchor_mints_missing_swaps_buys`** нужно: (1) завести задачу на **покрытие `swaps`** для якорных mint **или** (2) сменить источник якорей / (3) планировать **v2** с дозаполнением сделок по RPC; **не** трактовать как «ботов не нашли».
...
**Целевой порядок конвейера (UTC-сутки, после появления кода):** оркестратор **`wallets`** (W6.8) → узкий **sigseed** → **`swaps`** (S03) → wallet-centric **nightly backfill** → **`money_flows`** / доп. **`swaps`** (S02) → backfill **`wallets.funding_source`** из flows (S04) → **`npm run scam-farm:detect`** → опционально **`npm run scam-farm:graph`** (W6.14, treasury/мета-кластеры; включается **`SCAM_FARM_GRAPH_ENABLED=1`**). Дневной потолок QuickNode-кредитов — **единый ledger** для всех тяжёлых RPC-джоб (S01).
...
**Detective ledger (Smart Lottery ingest):** дневной потолок **`SA_QN_GLOBAL_CREDITS_PER_DAY`** (типично **2 000 000** credits UTC) для **`sa_qn_global_daily`** — при превышении billable RPC через **`sa-qn-json-rpc`** блокируются, один Telegram **`[ALERT][detective-qn-day-cap]`** за сутки (**`SA_QN_DETECTIVE_CAP_TELEGRAM`**). **Paper/live** и прочие вызовы через **`qn-client`** / **`solana-rpc-meter`** этим ledger **не** отключаются.

**Целевой контур без стрима на `v2`:** Gecko/DEX коллекторы + **`sa-wallet-orchestrator`** → **`wallet-backfill:*`** → опционально **sigseed** → **`money_flows`** / **`swaps`** → **`wallet-funding:backfill`** → **`npm run scam-farm:detect`** → **`npm run scam-farm:graph`** (после детектива, если включено в `.env`) → учёт **`sa_qn_global_daily`**.
...
- **Worker:** `SA_SIGSEED_ENABLED=1 npm run sigseed:run` — для каждого mint из очереди: `getSignaturesForAddress` → `getTransaction` → **`decodePumpfunSwap`** → **`swaps`** (`source=sigseed`). Один активный прогон: **`pg_advisory_lock(941337041)`**. Лимиты: **`SA_SIGSEED_MAX_MINTS_PER_RUN`** (default **22**), **`SA_SIGSEED_SIG_PAGES_MAX`** (**2**), **`SA_SIGSEED_MAX_TX_PER_MINT`** (**16**), пауза **`SA_SIGSEED_RPC_SLEEP_MS`** (**380**), жёсткий потолок за один запуск **`SA_SIGSEED_MAX_CREDITS_PER_RUN`** (default **60000**, `0` = без потолка), мягкий дневной по компоненту **`SA_SIGSEED_MAX_CREDITS_PER_DAY`** (default **400000** в коде, если env пуст). При остановке по cap финальный JSON — **`ok: false`** с кодом (не смешивается с успешным summary).
- **Cron:** **`install-detective-data-plane-salpha.sh`** — **ежечасно**: минута **:08** `sigseed:enqueue`, **:18** `sigseed:run` (UTC по системному crontab `salpha`). Gates **`SA_SIGSEED_*_ENABLED`**. Логи: `data/logs/sigseed-enqueue.log`, `data/logs/sigseed-run.log`.
...
| `HOURLY_HEALTH_SWAPS_INGEST_MAX_MIN` | Порог STALE для **Health · swaps** по **`max(created_at)`** (минуты; default **30**). **`chain_lag`** по **`block_time`** — только текст в отчёте. |
...
- `hourly-telegram-report.mjs` — сводка в Telegram: Coverage + **оркестратор кошельков** (новые **`wallets`** за **`HOURLY_COVERAGE_HOURS`** по **`seed_lane`**, W6.8 §10 п.4) + **детектив** (тег `bot` за час / всего, узкие теги, очередь backfill, топ ошибок; только PG, **`HOURLY_APPEND_DETECTIVE_INTEL`**) + Health (**`swaps`:** OK/STALE по **`created_at`**, см. §2 «Hourly Telegram — Health · `swaps`»; **`HOURLY_HEALTH_SWAPS_INGEST_MAX_MIN`**) + **Live Oscar** (PnL, открытыя), Eval (paper `pt1-oscar.jsonl`), баланс кошелька (**`HOURLY_RPC_URL`** / **`HOURLY_WALLET_PUBKEY`**, иначе fallback **`SA_RPC_*`** / **`LIVE_WALLET_PUBKEY`**), неуспешные исполнения за час; см. **`HOURLY_*`** в `.env.example`
