...
**Исправление (продукт 1.11.99+):** **`src/parser/allowlisted-dex-swap.ts`** — после точного pump.fun‑разбора, если в транзакции есть одна из разрешённых программ (Jupiter v6, PumpSwap, Raydium AMM/CLMM, Orca Whirlpool, Meteora DLMM, pump.fun), выполняется **балансовый** разбор (WSOL / лампорты / USDC / USDT как quote). Подключено к **`npm run sigseed:run`** и **`wallet-backfill`**. **`sa-parser`** по-прежнему только pump.fun Buy/Sell на стриме (точность по типу событий).
