...
| `0 * * * *`           | `jupiter-shadow-hourly.mjs` — почасовая сводка по фоновому Jupiter-каналу (signal-lab + mtm-shadow): Telegram только если были ошибки (`errs>0`), иначе только строка в лог cron; опция `JUPITER_SHADOW_HOURLY_SEND_IF_ZERO_ERRORS=1` — как раньше всегда REPORT/ALERT; установка: `sudo bash scripts/cron/install-jupiter-shadow-hourly-cron-salpha.sh`; лог `data/logs/jupiter-shadow-hourly.log` |
