...
**В репозитории `v2` код sigseed отсутствует:** нет таблиц **`rpc_features`** / **`signatures_seed_queue`**, нет **`npm run sigseed:*`**, нет worker-файлов — есть только **спеки и исторический текст** RUNTIME ниже. Включать нечего, пока контур не импортирован в этот продукт.

**Целевой контур без стрима на `v2` сейчас:** Gecko/DEX коллекторы + **`sa-wallet-orchestrator`** → **`wallet-backfill:pilot`** / nightly backfill → **`money_flows`** / **`swaps`** (pump.fun) → **`wallet-funding:backfill`** → **`npm run scam-farm:detect`** → учёт **`sa_qn_global_daily`**.
...
##### Sigseed (pipeline 2) — эталонная архитектура (на будущее, если появится код)

**Sigseed** — отдельный конвейер **без стрима**: очередь **`signatures_seed_queue`** → PM2 **`sa-sigseed-worker`** → **`rpc_features`** → cron **`sigseed:parse`** → таблица **`swaps`** (mint/signature-ориентированный ingest). Нормативно: **[`docs/Smart Lottery V2/W6.12_S03_sigseed_bounded_swaps_ingest_spec.md`](../docs/Smart%20Lottery%20V2/W6.12_S03_sigseed_bounded_swaps_ingest_spec.md)**.

- **Назначение:** наполнять **`swaps`** и кормить **Atlas / scam-farm / отчёты**, где нужны свежие факты по минтам, **не** заменяя полный wallet-centric backfill (S02).
- **Торговые стратегии (paper/live):** напрямую sigseed не «торгует»; он обеспечивает **данные** (свопы, признаки), от которых могут зависеть downstream-сигналы. Если **`swaps`** уже покрываются другим путём (стрим у вас выключен по политике, pilot/backfill — узкий pump.fun), sigseed — **опциональное** усиление охвата DEX через парсер RPC-tx.
- **Проверка на конкретном хосте:** `sudo -u salpha pm2 list` (есть ли **`sa-sigseed-worker`**), `sudo -u salpha crontab -l` (строки **`sigseed:enqueue`** / **`sigseed:parse`**). Таблица cron в §2 выше — **эталон после install-скриптов**; на установке без ocean/sigseed cron процесс **может отсутствовать**, тогда расход кредитов на sigseed **нулевой** — это нормально.
