...
Слот **02:25** выбран вне коридора **03:10–04:35** и вне почасовых **:08/:18** sigseed. Установка: `sudo bash /opt/solana-alpha/scripts/cron/install-dip-bot-cron-salpha.sh`. Ручной прогон: `cd /opt/solana-alpha && npm run dip-bot-intel:run` (нужны **`DATABASE_URL`** и строки в **`swaps`** по якорным mint). **v1:** только чтение Postgres, без QuickNode в джобе.
