...
**Enqueue pipeline 2:** `npm run sigseed:enqueue` (cron `*/15`), второй проход — `scripts/cron/enqueue-seed-followup.sh` (`*/30`). **Парсинг:** `sigseed:parse` (`*/3`). В логах воркера: `queueDepth` (остаток в `signatures_seed_queue`), `rpcFeaturesTxUnparsed` (хвост `rpc_features` до парсера).
...
- `hourly-telegram-report.mjs` (другие env: `PAPER_TRADES_PATH=/tmp/paper-trades.jsonl`)
