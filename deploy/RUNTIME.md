...
Джоба **не считается рабочей по смыслу W9.0**, если после прогона **`dip_bot_intel_observations`** и теги **`dip_bot` / `dip_bot_intel`** остаются нулевыми **из‑за отсутствия строк в `swaps` по якорным `base_mint`**, а не из‑за порогов USD/hits. На VPS после боевого прогона именно это и произошло: **`anchors_processed` > 0**, **`buyer_rows = 0`** по всем якорям — инжест **`swaps`** не покрывает класс mint/DEX, которые даёт Live Oscar.
