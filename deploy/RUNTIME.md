...
| `0 * * * *`           | `jupiter-shadow-hourly.mjs` — почасовая сводка по фоновому Jupiter-каналу (signal-lab + mtm-shadow): события / ошибки / медианы; шлёт `[REPORT][jupiter-shadow]` (или `[ALERT]` при `JUPITER_SHADOW_HOURLY_ALERT_RATIO`); установка: `sudo bash scripts/cron/install-jupiter-shadow-hourly-cron-salpha.sh`; лог `data/logs/jupiter-shadow-hourly.log` |
