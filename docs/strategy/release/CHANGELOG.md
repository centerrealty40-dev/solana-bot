...
- Черновик релиза **дашборда** (четыре колонки Live Oscar / фикс-порядок) **отозван** коммитом revert на **`b7d7d11`**; дерево продукта остаётся на **1.6.0**.
- Переменные **`PAPER_DIP_RECOVERY_VETO_*`** в PM2 **не задавать** до отдельного согласованного релиза (локальный черновик другого агента не вливался).
