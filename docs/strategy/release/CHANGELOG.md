...
**Правка.** Движение измеряется внутри одной ленты. Первая марка после
исполнения становится базой для MFE, взведения трейла и giveback, если она выше
цены сделки; база защёлкивается один раз, чтобы поздний всплеск не поднял её и
не стёр настоящий рост. `pnlPct` сохраняет базу по цене сделки — стоп обязан
отвечать за реальные деньги.

Четыре теста на сценарий: неподвижная цена не взводит трейл и не банкует;
последующий рост считается от ленты; марка ниже сделки базу не меняет; поздний
всплеск базу не двигает.

**Rollback.** Убрать `mfeBasisPriceUsd` из `src/milddip/state.ts`,
`src/milddip/exit-engine.ts` и `src/milddip/gates.ts` — MFE вернётся к базе по
цене сделки.
