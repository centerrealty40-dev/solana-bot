## [1.11.1092] — 2026-08-31

### Изменено

- GREEN-лейн `mild-dip-own2` теперь допускает входы на подтверждённом
  pullback: минимум `pc5m=-25%`, `pc1h=-15%` и пролив от пика не менее `10%`.
- Для own2 добавлен верхний предел tape-отскока `2%`; после достаточного
  пролива ограничение rally into peak не блокирует вход. Остальные процессы
  сохраняют прежние значения по умолчанию.

### Откат

- Вернуть own2 `MILD_DIP_GREEN_MIN_PC5M_PCT=4`,
  `MILD_DIP_GREEN_MIN_PC1H_PCT=0` и `MILD_DIP_GREEN_MIN_RET1M_PCT=5`.
  Удалить `MILD_DIP_GREEN_MAX_TAPE_RET1M_PCT` и
  `MILD_DIP_GREEN_MIN_DUMP_FROM_PEAK_PCT`; значения по умолчанию `1000` и
  `0` отключают новые гейты.

## [1.11.1091] — 2026-08-31

### Изменено

- Диапазон клипа `mild-dip-mirror` bot1 расширен с `$10` до `$10–40`:
  изменены только максимумы sizing law и покупки по размеру лидера.
  `mild-dip-mirror2`, own2 и платформенные файлы не затрагивались.

### Откат

- Вернуть `'10'` в `sizeLiqMaxUsd` и `sizeFromLeaderMaxUsd` для
  `mild-dip-mirror`.

## [1.11.1090] — 2026-08-31

### Изменено

- `mild-dip-own2` переведён на общий seed покупок обоих лидеров с памятью
  покупок 7 дней, а GREEN-лейн снова требует подтверждение лидера.
- Для GREEN добавлены верхние ограничения уже пройденного хода: rally into
  peak `20%` и bounce from trough `25%`; отсутствующие метрики пропускаются.
  Ограничения включены только для own2, остальные процессы сохраняют прежнее
  поведение.

### Откат

- Вернуть own2 `MILD_DIP_GREEN_REQUIRE_LEADER_SEEN=0`, путь seed на
  `data/milddip-own2/leader-seed-empty.json`, а оба новых порога установить
  в `0`.
...
- Клип `mild-dip-mirror` уменьшен с `$15` до `$10` в шести полях
  sizing-конфигурации по запросу пользователя. Изменён только
  `mild-dip-mirror`; платформенные файлы не затрагивались. Rollback:
  вернуть значение `'15'` в те же шесть полей.

## [1.11.1087] — 2026-08-31

### Изменено

- Для `mild-dip-own2` выключен lstyle-лейн: он давал `$9.49` из `$11.95`
  реализованного убытка за 4 часа и обходил пост-выходной кулдаун. Базовый
  кулдаун минта увеличен до 15 минут, после убыточного выхода — до 60 минут.
  Изменён только процесс own2; платформенные файлы не затронуты.
  Rollback: вернуть `MILD_DIP_LSTYLE_ENABLED='1'` и оба кулдауна к `60000`.

## [1.11.1086] — 2026-08-31

### Изменено

- Клип `$5` у `mild-dip-own2` закреплён во всех лейнах процесса: first-touch,
  thick, micro и leader-style больше не наследуют размеры общего bot-профиля
  (`$10`/`$3`/`$3`/`$5`). Других процессов и общего профиля изменение не
  касается. Платформенные файлы не затрагивались.

## [1.11.1085] — 2026-08-31

### Изменено

- Добавлен отдельный `mild-dip-own2` для собственного GREEN-discovery на
  Helius-стриме: кошелёк FxQfFTmj, фиксированный клип `$5`, изолированный
  каталог `data/milddip-own2`, пустой leader-seed и лимит реализованного
  убытка `$50` за сутки МСК по всем lane. Процесс оставлен вне PM2-экспорта
  для точечного запуска; bot1 и mirror2 не изменены, остановленные
  mirror-процессы и watchdog не изменены. Платформенные файлы не затрагивались.

## [1.11.1084] — 2026-08-31

### Изменено

- `mild-dip-mirror2` запущен на кошельке FxQfFTmj с лидером 7BNaxx и
  параметрами bot1 один-в-один, кроме клипа `$5` и лимита просадки `$50`
  (у bot1 — `$15`/`$150`): кап премии `0` / `+1%` в первую минуту, зелёный
  `+10%` с обходом пола ликвидности, полы `$30k`/`$50k`/`1ч`, докупки
  `−25%`/`−50%` по стоимости остатка, реестр открытых мешков лидера и
  усыновление ручных покупок; процесс возвращён в PM2-экспорт и под watchdog.
  Bot1 не изменён.

## [1.11.1083] — 2026-08-31

### Изменено

- Фиксированный клип mirror bot1 установлен на `$15`; mirror2 и усреднения
  не изменены.

## [1.11.1082] — 2026-08-30

### Изменено

- Кап премии bot1 после первой минуты равен `0` — вход допускается только
  по нулю или со скидкой; исполнительный допуск `+2` п.п. действует только
  внутри entry grace. Mirror2 не изменён.

## [1.11.1081] — 2026-08-30

### Изменено

- Клип mirror bot1 снижен до диапазона `$10–50`: изменены минимум и максимум
  sizing law, доля покупки лидера и fallback-клип; усреднения и mirror2 не изменены.

## [1.11.1080] — 2026-08-30

### Изменено

- Зелёный mirror-вход bot1 больше не отбивается полом ликвидности в общем
  entry-гейте исполнения: исполнитель получает решение слоя решения; остальные
  структурные полы не изменены.

## [1.11.1079] — 2026-08-30

### Изменено

- Для первой mirror-покупки bot1 исполнительный кап премии допускает
  дополнительные `+2` п.п. дрейфа котировки между решением и свапом; политика
  решения (`+1%`), вторая нога, усреднения и mirror2 не изменены.

## [1.11.1078] — 2026-08-30

### Изменено

- Для instant-green входов mirror bot1 исполнительный кап премии теперь
  соответствует капу решения даже при неизвестном `pc5m`; обычные входы,
  клипы, усреднения и retry-пути не изменены.

## [1.11.1077] — 2026-08-30

### Изменено

- Добавлено безопасное усыновление ручных покупок на mirror bot1 с отдельным
  учётом себестоимости и трейлом от пикового значения; ручные докупки уже
  известных боту минтов остаются под существующими guard'ами.

## [1.11.1076] — 2026-08-30

### Изменено

- Для mirror bot1 пол ликвидности теперь не блокирует зелёные входы при
  включённом флаге `MILD_DIP_MIRROR_GREEN_IGNORE_LIQUIDITY_FLOOR`; остальные
  структурные полы сохраняются. Mirror2 и обычный dip-бот не изменены.

## [1.11.1075] — 2026-08-28

### Изменено

- Усыновлённые mirror fills теперь записываются в trade journal с фактическим
  расходом USDC, себестоимостью лота и mirror cash attribution; лоты
  персистятся в state, а периодическая сверка cash ledger только журналирует
  расхождения.
- Для mirror bot1 включены две ступени усреднения от исходной цены входа
  (`−25%` и `−50%`) с sizing по текущей mark стоимости мешка и лимитом `$200`;
  клипы входа подняты до диапазона `$30–150`. Mirror2 и обычный dip-бот
  не изменены.

## [1.11.1074] — 2026-08-28

### Изменено

- Для mirror bot1 нижняя граница сайза снижена до `$10`, при этом верхняя
  граница остаётся `$50`; также снижены размер позиции и усреднения до `$10`.
  Кап премии к филлу лидера установлен на `1%`, а пол ликвидности повышен до
  `$30k` по результатам последних замеров. Mirror2 и обычный dip-бот не изменены.

## [1.11.1073] — 2026-08-28

### Изменено

- Для mirror bot1 добавлен персистентный реестр открытых сумок лидера и
  периодическое перевзведение наблюдений после временной нехватки средств,
  отказа гейта или завершения наблюдения. Реестр удаляется при продаже лидера,
  нулевом балансе, собственной позиции или истечении срока; mirror2 и обычный
  dip-бот не изменены.

## [1.11.1072] — 2026-08-28

### Изменено

- Для mirror bot1 все клипы зафиксированы на `$50`: основной тир, микротир,
  liq-law/flat путь и усреднения. Mirror2 и обычный dip-бот не изменены.

## [1.11.1071] — 2026-08-28

### Изменено

- Для mirror bot1 все клипы зафиксированы на `$10`: основной и микротир,
  liq-law/flat путь и усреднения. Первая покупка теперь выполняется одной
  ногой, а порог добора дастового остатка снижен до `$2`. Mirror2 и обычный
  dip-бот не изменены.

## [1.11.1070] — 2026-08-28

### Изменено

- Для mirror bot1 снят кап первого касания `$30`: первая покупка монеты
  теперь сайзится по общему правилу `0.25 × покупки лидера` с зажимом
  `$30–100`. Для mirror2 и обычного dip-бота прежнее ограничение сохранено.

## [1.11.1069] — 2026-08-28

### Изменено

- Лимит просадки mirror bot1 теперь считается только по реализованной кассе:
  открытые мешки учитываются по остаточной себестоимости, а не по mark-to-market
  оценке. Ранее без единой продажи статус в период 22:05–22:10 гулял от `$72`
  до `$31` и `$54` из-за переоценки открытых мешков. Порог `$150`, суточное
  окно по Москве и остальные entry/exit-параметры не изменены.

## [1.11.1068] — 2026-08-28

### Изменено

- Для mirror bot1 отключён кэп отношения оборота `vol5m` к ликвидности:
  лидер купил `A4Ma16Xr…` на `$680`, наша котировка была на `−4.6%`
  дешевле его филла, а все структурные полы прошли (ликвидность `$67k`,
  оборот за 5 минут `$249k`, возраст пары `2.1 ч`). Вход блокировался только
  предохранителем `vol_liq_churn_too_high=3.70>=2`, поэтому
  `MILD_DIP_MIRROR_MAX_VOL5M_TO_LIQ` для bot1 установлен в `0`. Для mirror2
  значение `2` сохранено.
