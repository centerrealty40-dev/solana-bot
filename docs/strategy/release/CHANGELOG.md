...
- `mild-dip-mirror2` запущен на кошельке FxQfFTmj с лидером 7BNaxx и
  параметрами bot1 один-в-один: клип `$15`, кап премии `0` / `+1%` в первую
  минуту, зелёный `+10%` с обходом пола ликвидности, полы `$30k`/`$50k`/`1ч`,
  докупки `−25%`/`−50%` по стоимости остатка, реестр открытых мешков лидера,
  лимит просадки `$150` по суткам МСК и усыновление ручных покупок; процесс
  возвращён в PM2-экспорт и под watchdog. Bot1 не изменён.
