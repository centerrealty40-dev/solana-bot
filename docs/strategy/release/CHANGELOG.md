...
- Добавлен одноразовый ops-скрипт
  `scripts-tmp/milddip-force-exit.ts` для принудительного полного выхода из
  одной управляемой mild-dip позиции по mint в live-режиме.
- Скрипт защищён от продажи стейблов/WSOL и dust-балансов, использует
  `executeCopySell` с причиной `manual_force_exit` и не изменяет
  `data/milddip/state.json`; после исчезновения токенов работающий бот сам
  подтвердит пустой баланс как `confirmed_empty`.
