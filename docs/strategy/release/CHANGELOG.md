...
- В wait-dip добавлен настраиваемый пол глубины от исходной сигнальной цены:
  `MILD_DIP_WAIT_DIP_MAX_DUMP_FROM_SIGNAL_PCT=10`. При уходе цены глубже
  порога сиденье снимается и покупка не выполняется; минт остаётся доступен
  для остальных источников входа.
- В 7-дневном production-срезе 1496 из 2403 позиций вошли через сиденье:
  медиана фила `−7.68%` от сигнала, p75 `−12.85%`, p90 `−19.07%`.
  Доходность на $100: полоса `−3…−10%` дала `+$0.09/+0.93`, `−10…−20%`
  дала `−$3.34` (418 сделок, `−$70`), глубже `−20%` — `−$6.67`
  (129 сделок, `−$43`).
- Проверка выполняется и на ready-марке, и на свежей цене перед попыткой
  входа. Отказы журналируются как `mild_dip_wait_dip_too_deep` не чаще
