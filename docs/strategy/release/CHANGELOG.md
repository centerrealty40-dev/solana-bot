## [1.11.1066] — 2026-08-28

### Изменено

- Для mirror bot1 пол оборота `vol5m` снижен с `$2000` до `$500`: по `663`
  закрытым сделкам bucket `vol5m < $500` давал `−3.1%` (`−10.3%` за 48 ч),
  тогда как диапазон `$500–1000` давал `+8.4%`. Пол снижен до `$500`, а не
  отменён; остальные полы и параметры mirror не изменены.

## [1.11.1067] — 2026-08-28

### Изменено

- На instant-green пути неизвестная структура больше не фиксирует размер
  покупки на `$30`: при покупке лидера `$500` это искусственно ограничивало
  mirror. Теперь применяется обычный размер основного тира — `0.25 ×`
  покупка лидера с зажимом `$30–100`; для известного mcap ниже `$40k`
  микротир `$30` сохранён.

## [1.11.1065] — 2026-08-28

### Изменено

- Для mirror bot1 добавлен мгновенный зелёный вход за лидером по собственной
  свежей котировке: не ниже филла лидера, в первую минуту, для первой ноги
  клипа и с премией до `+10%`. Такой вход не ждёт структуру DexScreener и не
  применяет структурные полы; дип-входы и их полы не изменены.

## [1.11.1064] — 2026-08-28

### Изменено

- Свежий хит лидера получает Dex-структуру вне общей очереди слотов; ранее
  ожидание слота достигало `53.6 с`. Для зелёных входов кап премии `+10%`
  теперь отсчитывается с момента появления `pc5m`, если при покупке лидера
  показатель был неизвестен; действует жёсткий лимит `300 с`. Базовый кап
  `−0.5%` не изменён.

## [1.11.1063] — 2026-08-28

### Изменено

- Ускорено структурное разрешение и получение котировки для новых наблюдений
  mirror: наблюдаемая задержка входа составляла медиану около `35 с` и p75 около
  `138 с`. Исправлено несоответствие окна свежести котировки и интервала
  обновления; bot1 mirror начинает покупки с проскальзыванием `400 bps`.

## [1.11.1062] — 2026-08-28

### Добавлено

- Для зелёных входов mirror bot1 добавлен отдельный кап премии: в первую минуту
  после покупки лидера, для первой ноги клипа, при `pc5m > 0` допускается премия
  к филлу лидера до `+10%`; после grace зелёные и остальные входы сохраняют
  ограничение `−0.5%`.

## [1.11.1061] — 2026-08-28

### Изменено

- Доля от покупки лидера bot1 mirror `0.5` → `0.25` — на покупках лидера $200+ клип сразу упирался в потолок `$100`.

## [1.11.1060] — 2026-08-28

### Изменено

- Пол ликвидности bot1 mirror `$20,000` → `$12,000` — пол отбивал все сделки с иксами.
- Потолок leader-based клипа bot1 `$30` → `$100`; микротир (mcap < `$40k`) остаётся `$30`.

## [1.11.1059] — 2026-08-28

### Изменено

- Клип входа bot1 mirror фиксирован на `$30` — потолок leader-based клипа `$200` → `$30`.

## [1.11.1058] — 2026-08-28

### Изменено

- Потолок leader-based клипа bot1 mirror поднят с `$120` до `$200`.

## [1.11.1057] — 2026-08-28

### Изменено

- Для bot1 mirror усреднение ограничено глубокой зоной скидки `−50%`;
  добавлен минимальный оборот `vol5m` `$2,000` для отсечения мелких пулов.

## [1.11.1056] — 2026-08-28

### Изменено

- Для bot1 mirror вход разрешён только по скидке к филлу лидера: кап премии
  и грейс первой минуты `-0.5%`. Замер закрытых сделок показал, что зелёные
  входы (по цене лидера или выше) дают ROI ниже дипа во всех окнах: 12 ч
  `-7.1%` против `-4.3%`, вся история `+21.1%` против `+27.0%`.

## [1.11.1055] — 2026-08-28

### Изменено

- По замеру PnL для bot1 mirror возвращены структурные полы и фильтры
  ликвидности, капитализации и возраста пары; leader-based клип установлен
  в диапазон `$30–120`.

## [1.11.1054] — 2026-08-15

### Изменено

- Для bot1 mirror лимит просадки снижен в суточное окно по Москве с порогом
  `$150`; при срабатывании новые покупки блокируются, выходы сохраняются.

## [1.11.1052] — 2026-08-27

### Изменено

- Для bot1 максимальный leader-based mirror-клип снижен с `$200` до
  `$130`; малый mcap-клип `$30` и остальные правила сохранены.

## [1.11.1051] — 2026-08-20

### Изменено

- Для bot1 mirror монеты с mcap ниже `$40k` получают фиксированный клип
  `$30`; остальные mirror-входы используют прежний leader-size закон.

## [1.11.1050] — 2026-08-20

### Исправлено

- Leader-based mirror-клип больше не урезается first-touch cap ниже
  настроенного минимума входа; структурные и green-gate правила не изменены.

## [1.11.1049] — 2026-08-20

### Исправлено

- Для первого mirror-касания нижняя граница first-touch cap поднята до
  минимального размера liquidity-закона; итоговый размер по-прежнему не
  превышает рассчитанный lane cap.

## [1.11.1048] — 2026-08-20

### Изменено

- Для первого mirror-входа при включённом законе поднят только нижний предел
  first-touch до liquidity floor; глобальный `$10` cap и прочие lane-политики
  сохранены.

## [1.11.1047] — 2026-08-20

### Изменено

- Tier-лейн отключён для обоих mirror-ботов; параметры tier сохранены для
  возможного обратного включения одним флагом.

## [1.11.1046] — 2026-08-20

### Исправлено

- Mirror-клип при включённом mirror-законе больше не ограничивается общим
  степенным законом DIP; сохраняются только риск-клипы mirror-лейна.

## [1.11.1045] — 2026-08-20

### Изменено

- Mirror-клип теперь зависит от ликвидности пула и ограничен долей `0.15%`.
  Диапазоны размера: `$30–120` для `mild-dip-mirror` и `$10–30` для
  `mild-dip-mirror2`.

## [1.11.1044] — 2026-08-20
## [1.11.1044] — 2026-08-20

### Изменено

- Добавлен явный флаг `MILD_DIP_MIRROR_LADDER_ENABLED` для отключения
  TP-лестницы без изменения семантики `MILD_DIP_MIRROR_LADDER_MAX_RUNGS=0`.
  У `mild-dip-mirror` лестница временно отключена, а у `mild-dip-mirror2`
  сохранена включённой; зеркальный выход за продажей лидера не меняется.

### Откат

- Установить `MILD_DIP_MIRROR_LADDER_ENABLED='1'`.

## [1.11.1043] — 2026-08-20

### Исправлено

- Включён tier-лейн `mild-dip-mirror2` с клипом `$10` и отдельным
  лимитом открытых позиций. Для него полностью сняты структурные полы
  ликвидности, `pc5m`, возраста пары и капитализации; обязательные
  торговые и риск-гейты сохраняются.

### Откат

- Установить `MILD_DIP_MIRROR_TIER_ENABLED='0'` и
  `MILD_DIP_MIRROR_TIER_IGNORE_FLOORS='0'`.

## [1.11.1042] — 2026-08-20

### Изменено

- Для `mild-dip-mirror2` включён отдельный tier-лейн с клипом `$10`,
  который может обходить структурные полы ликвидности и предвходового
  изменения `pc5m`, сохраняя обязательные торговые и риск-гейты.
...
## [1.11.1041] — 2026-08-20

### Изменено

- В `mild-dip-mirror` добавлен отдельный `tier`-лейн для кандидатов,
  отклонённых только полом возраста пары или капитализации: клип задаётся
  через `MILD_DIP_MIRROR_TIER_POSITION_USD`, усреднение и TP-лестница для
  него отключены, а сделки помечаются `lane: 'tier'`.

### Откат

- Отключить tier через `MILD_DIP_MIRROR_TIER_ENABLED='0'`.

## [1.11.1040] — 2026-08-20

### Изменено

- `mild-dip-bot` исключён из PM2-экспорта и целей мониторинга: оператор
  окончательно отключил dip-стратегию, поэтому reload не должен её поднимать
  или создавать ложные алерты.
- Определение `mild-dip-bot` сохранено во внутреннем списке ecosystem для
  построения mirror-конфигурации и fallback RPC observer; ручной `lane main`
  также продолжает находить его окружение.

### Откат

- Удалить `mild-dip-bot` из списка исключений и вернуть его в monitoring targets;
  полный откат — revert этого коммита.

## [1.11.1038] — 2026-08-20

### Изменено

- Mirror-наблюдения за покупками лидеров теперь сохраняются в state,
  восстанавливаются после рестарта и реконсилируются по ограниченному хвосту
  журнала наблюдателя за последние 6 часов; все обычные структурные гейты
  применяются без ослабления.
- Резервация позиции до подтверждения покупки больше не сохраняется на диск,
  поэтому неуспешная покупка не оставляет фантомную открытую позицию.

### Откат

- Вернуть прежнее хранение mirror-наблюдений только в памяти и откатить
  изменения резервирования покупки; полный откат — revert этого коммита.

### Предыдущие изменения

- Выход mirror вслед за продажей привязанного лидера теперь сохраняется в
  state до отправки и повторяется после рестарта или ошибки исполнения.
  При старте добавлена реконсиляция продаж лидеров за последние 6 часов,
  а попытки выхода журналируются с детектом, отправкой и итоговой задержкой;
  реконсиляция читает только ограниченный хвост журнала, повторные попытки
  разделены интервалом 5 секунд.

### Откат

- Вернуть прежний одноразовый выход через live feed и отключить стартовую
  реконсиляцию; полный откат — revert этого коммита.

## [1.11.1036] — 2026-08-20

### Изменено

- Mirror теперь после короткого grace-периода терминально отбрасывает
  наблюдения без валидной цены исполнения лидера и известные покупки ниже
  минимального размера, освобождая такие наблюдения вместо бесконечного retry.

### Откат

- Вернуть `MILD_DIP_MIRROR_MIN_LEADER_SIZE_USD=0` и увеличить
  `MILD_DIP_MIRROR_LEADER_FILL_GRACE_MS` до большого значения; полный откат —
  revert этого коммита.

## [1.11.1037] — 2026-08-20

### Изменено

- Для mirror усреднение теперь ищет цель каскадом окон `1ч/2ч/3ч/4ч/6ч`,
  выбирая первый минимум, который минимум на 10% ниже цены входа; свежий хвост
  сокращён до 2 минут, а OHLCV запрашивается на всё самое длинное окно.

### Откат

- Вернуть `MILD_DIP_MIRROR_AVERAGE_MIN_DISCOUNT_PCT=2`,
  `MILD_DIP_MIRROR_AVERAGE_EXCLUDE_TAIL_MS=900000` и одно окно `10800000`;
  полный откат — revert этого коммита.

## [1.11.1035] — 2026-08-20

### Изменено

- Для `mild-dip-mirror` снят лимит числа активных монет в Jupiter-квот-фиде:
  каждый текущий mirror-watch получает квоту, с коротким TTL и ограничением
  параллелизма до 16 запросов; зелёный Jupiter-лейн не изменён и сохраняет
  собственный кап.
- Усреднение mirror теперь разрешено только после удержания позиции минимум
  120 секунд и при марке не менее чем на 2% ниже первоначального fill, помимо
  существующего условия локального лоя.

### Откат

- Вернуть `MILD_DIP_MIRROR_MAX_QUOTE_MINTS=8`,
  `MILD_DIP_MIRROR_AVERAGE_MIN_DISCOUNT_PCT=0` и
  `MILD_DIP_MIRROR_AVERAGE_MIN_HOLD_MS=0`; полный откат — revert этого коммита.

## [1.11.1034] — 2026-08-20

### Изменено

- Для primary `mild-dip-mirror` отключено обязательное направление свечи:
  `MILD_DIP_MIRROR_REQUIRE_DIP_CANDLE=0`. Теперь при валидных liquidity,
  pair-age и mcap mirror может войти и по зелёной свече; обязательным ценовым
  условием остаётся только quote не хуже установленной скидки к fill лидера.
  `mild-dip-mirror2` сохраняет прежнее поведение.

### Откат

- Вернуть `MILD_DIP_MIRROR_REQUIRE_DIP_CANDLE=1` для primary mirror и выполнить
  обычный reload конфигурации; полный откат — revert этого коммита.

## [1.11.1033] — 2026-08-20

### Изменено

- Для `mild-dip-mirror` добавлен 8zkg/7BNaxx dip-only режим: строгий mcap
  floor `$120000`, минимальный возраст пары `4 ч`, вход только при скидке не
  менее 1%, clip `$5` и повторные попытки, пока привязанный лидер держит монету.
- Неизвестные структурные метрики остаются fail-closed `wait`, а известные
  значения ниже полов дают окончательный `skip`; позиция сохраняет привязку к
  лидеру и не переназначается на второй hit.
- Добавлены одноразовое усреднение `$5` по локальному лою и mirror TP-лестница
  `5%/10%`; продажа лидера по-прежнему закрывает позицию полностью. `mild-dip-mirror2`
  остаётся выключенным.
- Клип primary mirror увеличен до `$10`, одноразовое усреднение — до `$5`.
  Дастовый порог лестницы снижен до `$1` и теперь сравнивается с текущей
  рыночной стоимостью остатка, поэтому пять ступеней не закрывают `$10`-позицию
  полностью; полный выход по-прежнему выполняется только продажей привязанного
  лидера или safety max-hold.

### Откат

- Вернуть `mild-dip-mirror` в `OSCAR_VPS_EXCLUDED_APPS` либо отключить retry,
  ladder и averaging env-параметры и вернуть прежние значения mirror-конфига.
- При полном откате вернуть `docs/strategy/release/VERSION` на `1.11.1031` и
  отменить этот коммит.
...
- Расширено только журналирование лидерских входов и выходов: добавлены
  широкие окна DexScreener, агрегация пулов, размер сделки относительно
  ликвидности и оборота, execution metadata, cadence/re-entry и профиль
  выхода.
- В native mild-dip журнал добавлены источник и возраст структурного
  снапшота, transaction/turnover/buy-share, clip относительно ликвидности
  и доступные метрики tape/stream на момент решения.
- Торговые решения, гейты, пороги, размеры и существующие ключи журнала
  не изменяются; неизвестные значения записываются как `null`.

### Откат

- Вернуть `docs/strategy/release/VERSION` на `1.11.1028` и удалить запись
  `1.11.1030`; для полного отката логирования revert коммита релиза.
...
- Для `mild-dip-bot` пороги импульса лидера снижены до
  `MILD_DIP_ENTRY_MIN_TXNS_5M=20` и `MILD_DIP_ENTRY_MIN_TURNOVER=0.05`.
  Гейт теперь принимает частично неизвестные Dex-метрики, если хотя бы
  одна настроенная метрика известна и проходит; при полной неизвестности
  сохраняется отказ с диагностической причиной `impulse_metrics_unknown`.
  Анализ лидерских полос показывает рост покрытия с 46.0% до 66.5% входов,
  при этом медианный результат вырос с +0.5% до +0.9%, а winrate — с 51.2%
  до 52.2%; отсечённая полоса имеет median −1.6% и winrate 46.0%.
...
- Для `leader_mirror` добавлен режим полного зеркалирования выхода:
  плановые green-выходы подавляются, а позиция закрывается по продаже лидера;
  предохранитель `MILD_DIP_MIRROR_SAFETY_MAX_HOLD_MS` ограничивает срок
  удержания.
- Добавлен изолированный процесс `mild-dip-mirror2` со вторым кошельком,
  отдельными журналами и state-файлами, но общими read-only потоками лидера.

### Откат

```text
MILD_DIP_MIRROR_LEADER_SELL_ONLY=0
MILD_DIP_MIRROR_SAFETY_MAX_HOLD_MS=0
Не запускать приложение mild-dip-mirror2.
```

## [1.11.1025] — 2026-08-20

### Изменено

- В production `mild-dip-bot` выключен staged-entry для новых мешков:
  при `MILD_DIP_STAGED_ADD_MAX_USD=0` прежний staged first clip фактически
  превращал все первые покупки в плоские `$5` и подавлял liquidity power-law.
- Начальный размер теперь определяется first-touch cap и кривой ликвидности:
  новые монеты получают `$5…$10`, знакомые — `$5…$30`, а ножи сохраняют
  защитный clip с полом `$5`.
...
## [1.11.1024] — 2026-08-20

### Изменено

- Для включённого `leader_mirror` продажа настроенного лидера теперь
  немедленно инициирует полный выход позиции, только если продажа не старше
  заданного окна и произошла не раньше скопированной покупки лидера; для
  старых позиций без этого timestamp действует только проверка свежести.
- Продажа лидера во время окна наблюдения снимает будущий вход, а сигнал
  продажи сохраняется между тиками до обработки или истечения окна.
- В журнал выхода записываются событие лидера и лаг от его blockTime до нашего
  решения, чтобы измерять своевременность реакции.

### Откат

```text
MILD_DIP_MIRROR_LEADER_SELL_ENABLED=0
```
...
- Для dedicated `leader_mirror` добавлен немедленный bounded refire после
  `confirm_timeout`: перед повтором проверяется свежий on-chain остаток,
  чтобы не допустить двойной продажи уже исполненной транзакции.
- Для mirror-процесса confirmation timeout снижен до 15 секунд; обычный
  mild-dip процесс сохраняет прежние 45 секунд.

### Откат

```text
MILD_DIP_MIRROR_EXIT_REFIRE_MAX=0
LIVE_CONFIRM_TIMEOUT_MS не задавать (вернётся 45000)
```
...
- Добавлен dry-run-first ops-скрипт `scripts-tmp/milddip-force-exit.ts` для
  пакетной распродажи выбранного лейна (`main`/`mirror`): всех сирот, явного
  списка mint-ов или одной управляемой позиции с явным
  `--include-managed`.
- Скрипт загружает env выбранного приложения из `ecosystem.config.cjs`, не
  требует суффикса `pump`, защищён от продажи стейблов/WSOL и dust-балансов,
  ограничивает число продаж и минимальную оценку выхода, а реальные сделки
  выполняет только с `--execute`. `data/milddip/state.json` не изменяется;
  после исчезновения токенов работающий бот сам подтвердит пустой баланс как
  `confirmed_empty`.
...
- Клип копитрейдера (`mild-dip-mirror`) $10 → $50
  (`MILD_DIP_MIRROR_POSITION_USD=50`). Максимум позиций 8 не менялся, поэтому
  теоретический предел экспозиции $400 — фактически он ограничен балансом USDC
  кошелька: при исчерпании funding-гейт режет клип до остатка.
  Гейты входа, зелёный коридор и выходы не изменены.
...
- Leader-style теперь определяет возраст пары по цепочке DEX `pairCreatedAt`,
  registry и нижней границе собственной tape-наблюдаемости. В telemetry
  добавлен `pairAgeSource`; причины отказа разделены на
  `insufficient_pair_age` и `insufficient_ring_span`.
- Добавлен `MILD_DIP_LSTYLE_MIN_RING_SPAN_MS`: production `60000` позволяет
  измерять откат по 60 секундам истории внутри 120-секундного окна; значение
  `0` возвращает прежнее требование полного окна.
...
- Предел одновременных mirror-позиций поднят с 3 до 8 по запросу владельца:
  при клипе $10 предельная экспозиция ветки — $80. Пороги входа, гейты
  тонких монет и выходы не менялись.
...
## [1.11.1006] — 2026-08-20

### Изменено

- Mirror-клип поднят с $2 до $10 по запросу владельца. Максимум позиций
  ветки не менялся (3), то есть предельная экспозиция mirror — $30.
...
- Добавлен bounded `leader_mirror` lane: свежие входы настроенных лидеров
  проходят короткое quote-наблюдение, green/run-up veto, cap премии к fill,
  скромные structural floors и отдельный ранний trailing exit.
- `leader_mirror` изолирован от DIP ladder и GREEN lane; GREEN остаётся
  выключенным, staged adds остаются выключенными.

### Rollback

```text
MILD_DIP_MIRROR_ENABLED=0
MILD_DIP_MIRROR_LEADERS=
...
- В mild-dip отключены staged adds через
  `MILD_DIP_STAGED_ADD_MAX_USD=0`, но staged entry и первый клип `$5`
  сохранены (`MILD_DIP_STAGED_ENTRY_ENABLED=1`,
  `MILD_DIP_STAGED_FIRST_USD=5`). Отключать staged entry целиком нельзя:
  тогда lane купит весь clip liquidity-кривой одним выстрелом, вплоть до
  `MILD_DIP_SIZE_MAX_USD=30` — именно диапазон крупных размеров показывает
  отрицательный результат.
- Основание — 7 суток production journal в фактических долларах:
  3736 закрытых циклов, `$11,647` cost и `+$324` cash. По суммарно
  вложенным деньгам: `$0–4` — 2964, `−$242.6` / `−3.75` на $100;
  `$4–6` — 537, `+$669.0` / `+24.95` (после trim `+$487.8`);
  `$6–12` — 191, `−$18.5` / `−1.08`; `$12–25` — 36, `−$55.2` /
  `−9.74`; `$25+` — 8, `−$28.6` / `−12.88`.
- На leader book тот же эффект: клипы `$75–200` дали `+5.42` на $100
  (`+$29.0k`), клипы `$600–5000` — `−4.95` (`−$23.0k`); top-10%
  крупнейших клипов — `−$24.6k` против `+$37.6k` у остальных, на обоих
  наблюдавшихся leader wallets.
- При нулевом cap `evaluateStagedEntryAdd` возвращает `target_filled` с
  `addUsd=0`; отдельного add buy нет. Skip telemetry для staged adds
  ограничена и не пишет `target_filled` на каждом mark.
...
- В wait-dip сохранён пол глубины от исходной сигнальной цены, но порог
  пересмотрен с `10%` до `25%`: это предохранитель от ножа/рага, а не
  попытка покупать дороже. Проверка на ready-марке и свежей quote/fill цене
  снимает сиденье без cooldown/blacklist; остальные источники входа остаются
  доступными. Отказы журналируются как `mild_dip_wait_dip_too_deep` не чаще
  одного раза на минт за 15 секунд.
- Пересмотр основан на фактических долларах за 7 суток: 2227 сидений,
  `$7107` оборота и `+$230.5` кассы. По глубине фила: `−3…0%` — `+$9.39`
  на $100 / `+$50.3`, `−6…−3%` — `−$0.41` / `−$6.1`, `−10…−6%` —
  `+$1.74` / `+$28.5`, `−15…−10%` — `+$7.25` / `+$101.0`,
  `−20…−15%` — `+$8.21` / `+$84.2`, глубже `−20%` — `−$2.69` /
  `−$27.3`. Прежний порог `10%` был выведен из per-$100 реконструкции
  с фиксированным клипом `$5` и опровергнут пересчётом в реальных долларах.
- `MILD_DIP_WAIT_DIP_MAX_WATCH_MS` сокращён с `1200000` до `600000` мс:
  за 7 суток 600 секунд сохраняют `n=1593` и `+$229.1` из `+$231.9`
  (`+$4.36` на $100), тогда как 300 секунд оставляют `n=1024` и `+$136.9`.
  Длительность ожидания не предсказывает глубину фила: медианы `−11.5%`,
  `−10.5%`, `−10.8%`, `−11.9%` для бакетов `0–60с`, `60–300с`,
  `300–600с`, `>600с`.

### Откат

```text
MILD_DIP_WAIT_DIP_MAX_DUMP_FROM_SIGNAL_PCT=0
...
- В mild-dip staged-add якорь перенесён с цены первого фила на непрерывно
  обновляемый локальный лой после входа. Долив разрешён в коридоре `+8…+12%`
  от лоя (`MILD_DIP_STAGED_ADD_TROUGH_TRIGGER_PCT=8`,
  `MILD_DIP_STAGED_ADD_TROUGH_BAND_PCT=4`) только после стабилизации лоя не
  менее `60000` мс (`MILD_DIP_STAGED_ADD_MIN_TROUGH_AGE_MS=60000`).
- Потолок погони относительно первого фила сохраняется; staged-add журналирует
  якорную цену/время, отскок от лоя и процент к первому филу для успешных и
  отказанных решений.
- Откат якоря к прежнему поведению: `MILD_DIP_STAGED_ADD_ANCHOR=fill`.
- Live Oscar, dc-trader, копитрейдер, зелёная линия и другие продукты не
  менялись.
...
- Для mild-dip добавлено bounded loss-reclaim wait только для bounce-like
  soft-loss причин `never_arm_bounce`, `dead_set_bounce`, `never_arm_time_red`,
  `never_arm_stale`, `never_arm_timeout` и `breakeven_stop`. При money-basis
  убытке мельче `10%` ожидание ждёт возврата к `+2%` от той же базы; при цели
  закрывается весь мешок с причиной `loss_reclaim`.
- Ожидание снимается при углублении до `−25%` или через `60` минут, после чего
  обычные loss exits продолжают решать. `hard_stop`, `cliff_dump`, `liq_drain`,
  `never_arm_freefall`, `max_hold_underwater` и прочие protective/green exits
  ожиданием не блокируются. Таймер хранится в состоянии позиции, а start/clear
  события журналируются по одному на мешок.
- Основание: 225 полных убыточных выходов с минутной лентой, пересечения по
  монете исключены. На подвыборке убытков мельче `−10%` (`n=49`) 69% вернулись
  к входу за час, 67% — к входу `+2%`, медианный максимум после продажи был
  `+13.6%`. Вариант `+2% / −25% / 60m` дал медиану `+4.0%`, был лучше в 59%
  случаев, дал `+$74` при `$190` фактических поступлений и `+$27` после
  усечения двух лучших/худших результатов; 29/49 достигли цели (медиана
  `+12.3%`, обычно за 1–2 минуты), 18/49 дошли до стопа (медиана `−40.8%`),
  2/49 — до time cap. Для убытков глубже `−10%` правило было хуже: медиана
  `−6.3%`, лучше только в 39% случаев. Оговорка: `n=49`, почти вся касса
  пришлась на один календарный день.

### Rollback

```text
MILD_DIP_EXIT_LOSS_RECLAIM_MAX_LOSS_PCT=0
```
...
- GREEN-позиции без движения закрываются после 15 минут, если максимальный
  MFE не достиг `+3%`; стоп и трейл сохраняют приоритет над этим тайм-катом.
- Стандартный GREEN-трейл теперь армится с `+3%` и закрывает при откате `6%`;
  fast-профиль и DIP-путь не менялись.

### Rollback

```text
MILD_DIP_GREEN_NO_MOVE_CUT_MS=0
...
- Production GREEN watch values are enabled at
  `MILD_DIP_GREEN_WATCH_ENABLED=1`,
  `MILD_DIP_GREEN_WATCH_WINDOW_MS=600000`,
  `MILD_DIP_GREEN_WATCH_MIN_HITS=3`, and
  `MILD_DIP_GREEN_WATCH_MAX_MINTS=60`; the GREEN leader-seen requirement is
  disabled with `MILD_DIP_GREEN_REQUIRE_LEADER_SEEN=0`.
...
- Добавлен универсальный entry-risk gate `MILD_DIP_ENTRY_MIN_LIQ_USD=4000`: известные пулы с ликвидностью ниже $4k отклоняются с причиной `liq_too_thin`, а неизвестная ликвидность и значение `0` не блокируют вход. На данных лидеров входы при liquidity < $4k дали −$8.5k, медиану −5.1% и 29% побед; у mild-dip за 48 часов такие входы составляли 1.9% сделок и −$1.60.
- Включён уже реализованный абсолютный exit floor `MILD_DIP_EXIT_LIQ_ABS_FLOOR_USD=4000` при отрицательном PnL и возрасте позиции более 10 минут. Он использует подтверждение на двух разных замерах ликвидности, пропускает вооружённого прибыльного раннера и сохраняет защиту котировки убыточного выхода.
- Ratio-сигнал `depthDrainRatio < 0.7` выключен через `MILD_DIP_EXIT_LIQ_DRAIN_RATIO=0`; код и телеметрия сохранены для последующего rollback/анализа. На данных лидеров этот сигнал срабатывал лишь в 7 сессиях и был слегка отрицательным.
- Анализ абсолютного floor на 5 925 закрытых сессиях лидеров дал примерно +$3.65k к факту, n=452 (8%), помог в 66% случаев, медиана +$1.6; результат устойчив для порогов $3k–$10k.

### Откат

1. Отключить минимальную ликвидность входа: `MILD_DIP_ENTRY_MIN_LIQ_USD=0`.
2. Отключить абсолютный floor выхода: `MILD_DIP_EXIT_LIQ_ABS_FLOOR_USD=0`.
3. Для возврата ratio-сигнала установить `MILD_DIP_EXIT_LIQ_DRAIN_RATIO=0.7`.

## [1.11.969] — 2026-08-16

### Добавлено

- Solana Alpha mild-dip теперь может закрывать убыточную позицию при подтверждённом сливе ликвидности: `depthDrainRatio < 0.7`, то есть ликвидность уходит быстрее цены. Сигнал основан на анализе 5 925 закрытых сессий: вероятность выхода лидеров в следующие 5 минут выросла с 8.7% до 17.8%; контрфактический выход улучшил кэш примерно на $39k, медиана составила +$6.8 на сессию, сигнал помог в 59% случаев.
- Значения по умолчанию: минимальный возраст позиции 10 минут, 2 последовательные марки, пропуск вооружённого прибыльного раннера включён. Дополнительный абсолютный floor ликвидности выключен (`MILD_DIP_EXIT_LIQ_ABS_FLOOR_USD=0`).
- `liq_drain` — полный one-shot выход без ожидания отскока и с защитой котировки убыточного выхода `MILD_DIP_EXIT_LOSS_FILL_MAX_SLIP_PCT`.
- Подтверждение считается только по разным timestamp-сэмплам Dex-метрик; повторное чтение того же сэмпла не увеличивает счётчик.

### Откат

1. Отключить ratio-сигнал: `MILD_DIP_EXIT_LIQ_DRAIN_RATIO=0`.
2. Если включался абсолютный floor, вернуть `MILD_DIP_EXIT_LIQ_ABS_FLOOR_USD=0`.
...
- Also fixed the 1.11.949 green-sleeve delivery defect: the env loader
  populated `mfeBankSleeveGreenPartialFraction`, but the exit schema stripped
  it before runtime. The schema now preserves the field with default `0`, and
  loader/schema key-survival coverage prevents another silent drop.
- Production evidence from
  `7i56zDtkKpQRQwuVZxwpeh2hVnYe3TBvhpB4SqD5T946`: `tp_grid` sold 56.44% at
  +71.86% and left a runner, then 22s later `mfe_bank_sleeve` sold the
  remainder at fraction `1` and +49.23% (sell
  `Bu8Th2kASyzTARLeEWs8s7uyq1kmMYqXnxHVqztMdiqztyQd3rWTWdnB8gVSmPGMfMUiramNyhPiGfC7cCjU7S7`).
  The live env was `0.5`; the schema loss made the gate see `0`.
...
- To roll back the green-sleeve schema fix independently, set
  `MILD_DIP_EXIT_MFE_BANK_SLEEVE_GREEN_PARTIAL_FRACTION` to `'0'` and revert
  the schema/test portion of this release.
...
### Мы переплачивали на входе, и это была причина «монета стоит ровно, а мы в минусе»

**Сделка `a3WJNqUN…` / `259KArDP`, первые секунды:**

```
   -1.8s mark px=5.6020e-05          цена перед покупкой
   +0.0s copy_buy px=5.7911e-05      наша сделка, +3.4% к рынку
   +4.4s mark px=5.6020e-05  pnl=-3.27%
```

Цена стояла в диапазоне 5.60–5.89e-05 все шесть минут. Никаких фантомов, все
марки от Dex, карантина нет. Позиция ушла в минус на 3.27% через четыре секунды
только потому, что мы купили на 3.4% выше рынка.

**Замер по 472 покупкам**, против марки Dex, стоящей медианно за **1.02
секунды** до сделки — то есть это не устаревшая цена:

| | значение |
|---|---|
| медиана переплаты | **+1.81%** |
| p75 | +4.15% |
| p90 | +7.5% |
| покупок дороже рынка | **70.8%** |
| дороже на 2%+ | 47.5% |
| дороже на 5%+ | 20.8% |

Взвешенно по деньгам +1.02%, то есть $6.83 отдано на входе с $666 оборота.

**Переплата предсказывает исход.** 297 закрытых сумок:

| переплата | сумок | медиана | винрейт | на сумку |
|---|---|---|---|---|
| 0–1% | 23 | −3.03% | 0.478 | +0.073 |
| 2–4% | 54 | −3.00% | 0.333 | +0.002 |
| **4–8%** | **46** | **−9.12%** | **0.283** | **−0.609** |
| выше 8% | 38 | −1.12% | 0.447 | −0.406 |

| потолок | остаётся сумок | итог |
|---|---|---|
| нет | 100% | −$76.79 |
| 8% | 87.2% | −$61.38 |
| **4%** | **71.7%** | **−$33.34** |
...
**Что это разрешало.** `MILD_DIP_SLIPPAGE_BPS: '500'` — мы сообщали Jupiter, что
исполнение на 5% хуже котировки приемлемо. Плюс `MAX_CHASE_PCT: 10` и
`FAST_PATH_CHASE_PCT: 12`.

**Новые значения:** проскальзывание **200 bps**, оба лимита погони **4%**.

**Rollback.** `MILD_DIP_SLIPPAGE_BPS: '500'`, `MILD_DIP_MAX_CHASE_PCT: '10'`,
...
### Откат `ENRICH_MAX` 60 → 20: лишние сканы ушли в пустоту

1.11.863 поднял размер прохода до 60 на арифметике «батч по 30 монет против
потолка 120 RPM тянет 3600 монет в минуту». Через пять часов живых данных
видно, что на практике это неверно:

| метрика | было (12) | стало (60) |
|---|---|---|
| сканов в минуту | 37.9 | 50.4 |
| различных монет | 411 | 572 |
| **доля пустых ответов** | **1.6%** | **35.5%** |
| **интервал возврата, медиана** | **82.0 с** | **94.1 с** |

Лишние слоты ушли в хвост списка кандидатов, по которому у DexScreener просто
нет данных: треть каждого прохода возвращалась пустой, а интервал возврата к
монетам, которые нам нужны, от этого **ухудшился**.

Поставлено **20**. Отмена интервала в 15 секунд при открытых позициях
(3-секундная каденция вместо неё) остаётся — это и была та часть 1.11.863,
которая работала.

**Rollback.** `MILD_DIP_ENRICH_MAX: '60'`.
...
### Потолок объёма на входе: $40 000 за пять минут

**Как нашли.** Разбирали, что съедает заработок. За 12 часов из 70 полностью
закрытых сумок 34 ушли в глубокий убыток (`hard_stop` или
`never_arm_time_red`). Сравнение с остальными 36 дало единственный различающий
признак:

| метрика на входе | глубокий убыток | остальные |
|---|---|---|
| **объём 5м** | **$19 361** | **$6 702** |
| ликвидность | $17 889 | $16 251 |
| капитализация | $65 727 | $54 185 |
| pc5m | −21.46% | −18.97% |

Ликвидность, капитализация и глубина пролива у них неразличимы. Различается
только объём — почти втрое. Убыточные при этом успевают дойти до медианного
пика **+11.18%** и лишь потом умирают, то есть дело не в том, что мы покупаем
мертвечину.

**Проверка на 499 полностью закрытых сумках**, счёт по кассе:

| объём 5м на входе | сумок | медиана | винрейт | итог |
|---|---|---|---|---|
| $0–2 000 | 112 | −4.53% | 0.393 | −$12.94 |
| $2 000–6 000 | 112 | −5.06% | 0.384 | −$23.90 |
| $6 000–15 000 | 89 | −3.21% | 0.416 | −$20.02 |
| $15 000–40 000 | 92 | −3.00% | 0.370 | −$29.82 |
| **выше $40 000** | **94** | **−8.49%** | **0.298** | **−$61.72** |

Девятнадцать процентов сумок несут **42% всего убытка**.
...
Взят **$40 000**: убирает $61.71 убытка, сохраняя 81% оборота. Ниже начинается
размен объёма на всё меньшую выгоду.

Порог применяется в обоих структурных гейтах — быстром и медленном. **Зелёная
ветка не затронута**: она проверяется до этих полов и по построению хочет
горячие имена, у неё собственный пол объёма в $8 000.

**Rollback.** `MILD_DIP_MAX_VOLUME_5M_USD: '0'`.
...
### Fix: у входов через `wait_dip` MFE открывался на −11% и позиция замирала

**Как нашли.** Присланы `LwsyhZgw…` и `4rFDTev…`. Обе оказались **покупками**, и
обе позиции висели открытыми. В трассе видно невозможное:

```
   -3.4s  mark px=6.2290e-05  pnl=+0.00%  mfe=-13.63%
   +2.7s  mark px=6.2290e-05  pnl=-3.15%  mfe=-10.82%
```

**MFE отрицательный.** Это максимум прибыли, он не может быть меньше нуля.

**Причина — моя правка 1.11.848.** Базой для измерения движения я взял
`c.priceUsd`, цену Dex, на которой принималось решение о входе. Для `wait_dip`
эта цена — сигнал, поставленный до двадцати минут назад, и он по построению
стоит примерно на 15% выше цены исполнения: весь смысл припаркованной заявки в
том, чтобы купить ниже него.

Сумка открывалась с MFE около −11% и не могла ни взвести трейл, ни дойти до
ступени лестницы, пока цена не отыграет эти одиннадцать пунктов просто до нуля.
`AENK1YJ9` и `Ggec8Zysy` именно поэтому и стояли по несколько минут без
единого действия.

**Правка, две части.**

Базой теперь служит проба из ценового кольца **не старше 30 секунд** — марка,
стоящая рядом с исполнением, а не цена, которая когда-то квалифицировала
кандидата. Марки Dex на открытых сумках идут раз в 6.1 секунды, так что 30
секунд это несколько марок запаса и одновременно надёжное отсечение
двадцатиминутного сигнала. Если свежей пробы нет, база возвращается к цене
сделки — поведение до 1.11.848.

И страховка в самом движке: **MFE зажат снизу нулём**. Отрицательное значение
означает, что база стоит выше пика, то есть неисправность базы, а не движение
цены, и протекать в лестницу и во взведение трейла оно не должно ни при каком
источнике базы.

**Заодно проверено** утверждение про долю стрим-марок из 1.11.868: за два часа
`dex` 10 118 (98.0%), `stream` 209 (**2.0%**).

**Rollback.** Вернуть `entryMarkPriceUsd: c.priceUsd > 0 ? c.priceUsd :
undefined` и снять `Math.max(0, …)` вокруг `mfeFromEntryPct`.
...
### Фантом из стрима прошёл под порогом в 25%

`37HTrCMb…`: купили по `5.2196e-04`, через двадцать секунд продали половину, ещё
через двадцать — остаток, обе ноги в минус, при том что цена не двигалась.

| t | цена | pnl | источник |
|---|---|---|---|
| +12.0 с | 5.1300e-04 | −1.72% | dex |
| **+18.1 с** | **6.3386e-04** | **+21.44%** | **stream, +23.56% за тик** |
| +19.5 с | продажа половины | −1.92% | `tp_grid` |
| +34.8 с | 5.1480e-04 | −1.37% | dex |
| +41.1 с | продажа остатка | −1.94% | `breakeven_stop` |

Сторож из 1.11.852 требует подтверждения на скачок **больше 25%**, а этот был
**+23.56%** — прошёл на волосок, взвёл трейл и дёрнул ступень лестницы.

**Разгадка не в размере, а в источнике.** Доля разворотов почти не зависит от
величины скачка — 10–15% в любой полосе. Зато зависит от того, откуда принт:

| размер скачка | развернулось у стрим-принтов |
|---|---|
| 5–10% | **46.1%** |
| 10–15% | 37.0% |
| 15–20% | 26.5% |
...
А среди растущих скачков на 20–25% доля стрима **31.2%** — выше, чем в любой
другой полосе.

**Правка.** Порог подтверждения теперь зависит от источника: **8%** для принта
из стрима, прежние 25% для Dex. Один и тот же +23.56% теперь задерживается,
если пришёл из стрима, и проходит, если из Dex.

**Rollback.** `MILD_DIP_EXIT_MARK_JUMP_CONFIRM_STREAM_PCT: '0'` — вернётся общий
порог.
...
### Зелёная ветка: порог возраста 1 ч → 3 минуты

`4y4EzQyH…`: два лидера взяли монету, которую мы отвергли. По нашим же
замерам ей было **88 секунд**, а не два часа:

| относительно их покупки | pc5m | ликвидность | возраст |
|---|---|---|---|
| −53.3 с | +14.02% | $12 514 | 19 с |
| −23.8 с | +24.19% | $13 128 | 66 с |
| **−3.8 с** | **+51.8%** | $14 667 | **88 с** |
| +66.0 с | **+125%** | $18 185 | 146 с |

Снайп запуска: вход на $191 в монету полутора минут от роду сразу после +52%.

**Проверка порога на сэмплере**, с зелёным выходом (+30% / −6% / 10 мин):

| порог возраста | сигналов | среднее | винрейт | сумма |
|---|---|---|---|---|
| **без порога** | **153** | **+3.39** | 0.272 | **+512** |
| 0.05 ч | 150 | +3.34 | 0.270 | +494 |
| 0.5 ч | 100 | +3.05 | 0.283 | +302 |
| 1 ч (было) | 60 | +2.75 | 0.271 | +162 |
| 2 ч | 37 | +1.75 | 0.250 | +63 |

Качество сделки от возраста не зависит, порог только выбрасывает сигналы.
Полоса до часа отдельно: 93 сигнала, среднее **+3.54**, до +30% доходит 25.8%,
на стопе −6% выбивает 73.1% — то есть ровно 0.258 × 30 − 0.731 × 6.

**Шестичасовой порог дип-ветки сюда не переносится.** Он подобран там, где
выход сидит в просадке до −25%, а эта ветка выходит через десять минут и режет
на −6%. Ограниченный выход и есть то, что делает молодые монеты переносимыми.

Три минуты оставлены только чтобы у пары был читаемый снапшот: у `Cmgg6FUi`
на момент покупки лидером весь объект `dex` у обзервера пришёл пустым.

**Rollback.** `MILD_DIP_GREEN_MIN_PAIR_AGE_HOURS: '1'`.
...
### `wait_dip`: цель −15% → −5%, овершут 5 → 3

Механизм работал наоборот замыслу. За сутки припарковано **1997** seat'ов,
**1348 истекло** не купив.

**Куда ушла цена, от которой мы отказались** (по истёкшим seat'ам):

| исход | доля |
|---|---|
| **закончила выше сигнала** | **61.4%** |
| выше на 10%+ | 24.9% |
| выше на 25%+ | 11.6% |
| выше на 50%+ | 4.6% |
| ниже на 10%+ | 6.4% |

**Насколько глубоко падала, пока мы ждали:** медиана −2.87%, p25 −8.37%,
**32% не падали ни разу**.

| цель | наполнилось бы истёкших |
|---|---|
| **−5%** | **38.7%** |
| −8% | 25.7% |
| −12% | 12.2% |
...
А те seat'ы, что всё-таки наполнялись, приходили на медианных **−20.83%** от
сигнала — глубже собственной цели. То есть нас наполняли исключительно обвалы,
а всё, что восстанавливалось, мы пропускали. Это ровно отбор наоборот, и он
объясняет, почему `wait_dip` даёт худший P&L среди всех источников:
**−$0.311 на позицию** против −$0.168 у прямого входа `dex`.

**Овершут снижен с 5 до 3.** При цели −5% и овершуте 5 сумма равна нулю, и
защита «никогда не покупать выше сигнала» в `waitDipMaxPriceUsd` схлопывает
потолок обратно в саму цель, не оставляя места ни на какой отскок. При 3 окно
идёт от готовности на −5% до потолка на −2%.

**Как нашли.** Пользователь прислал `268Lr2sC…`, где два бота-лидера сидят в
монете, а мы нет. Оказалось, у нас на ней стоял припаркованный seat за 204
секунды до входа лидера: он купил по текущей цене, мы ждали ещё −15%.

Заодно проверено, что вселенная не при чём: за 6 часов мы касались 2573 монет,
лидеры купили 175, пересечение **156 — 89.1%**. Мы видим почти всё, что они
берут; вопрос был только в том, покупаем ли.

**Rollback.** `MILD_DIP_WAIT_DIP_PCT: '-15'`,
...
### Зелёная ветка включена, клип $1

Порог возраста 6 часов (1.11.864) отрезал 59% наших сделок, и освободившееся
место занимает зелёная ветка.

**Врезка входа.** Зелёная проверка стоит **перед** `structuralOk`, потому что
это другая сделка со своими порогами: моментум-имя моложе и горячее всего, что
нужно дип-ветке, и общий возрастной пол в 6 часов оставил бы от неё 18%.
Источник сигнала — новый `dipSource: 'green_momentum'`, и он освобождён от
требования стрим-цены: решение целиком принимается по снапшоту DexScreener.

**Свой порог возраста — 1 час.** Зелёные сигналы идут с медианным возрастом
пары 0.67 ч (p25 0.33, p75 1.88). Шестичасовой пол оставил бы 26 сигналов из
147, часовой оставляет 57 — около 39%. Смягчение оправдано конструкцией
выхода: ветка выходит через десять минут максимум и режет на −6%, тогда как
дип-ветка держит просадку до −25%.

**Свой клип — $1.** Ветка непроверенная, поэтому размер задаётся отдельно и
берётся минимум из зелёного клипа, ограничения по риску рага и пробного
размера.

**Свой выход** (подключён в 1.11.860): +30% / −6% / 10 минут, без лестницы,
трейла и порога безубытка.

**Расход на Helius не растёт** (1.11.862): зелёные сумки исключены из платного
сэмплирования, а вход целиком считается по бесплатному снапшоту.

Итого при 57 сигналах в сутки под риском около $57 в день с ограничением
убытка на позицию в −6%.

**Rollback.** `MILD_DIP_GREEN_ENABLED: '0'`.
...
### Минимальный возраст пары 0.2 ч → 6 ч

Разбор `YC2ncFUT…`: купили монету в возрасте 22 минут, $2.00 → $0.14, минус 93%.
На входе метрики были в норме — ликвидность $23 662, капитализация $111 916,
объём 5м $71 028, pc5m −12.42. Монета в возрасте десяти минут выросла на +92.77%,
мы поставили `wait_dip` на −15% от сигнала, она провалилась на −37.3%, и мы
купили в этот обвал.

**Корреляция по 513 закрытым позициям**, сведённым со снимком метрик на момент
покупки, счёт по кассе:

| возраст пары | n | медиана | среднее | винрейт | итог | на позицию |
|---|---|---|---|---|---|---|
| **0–0.5 ч** | **270** | −4.76% | **−10.71%** | 0.359 | **−$115.00** | −0.426 |
| 0.5–1 ч | 7 | −14.35% | −23.17% | 0.143 | −$4.17 | −0.595 |
| 2–6 ч | 20 | −9.86% | −2.99% | 0.400 | −$9.05 | −0.453 |
| 6–24 ч | 30 | −5.61% | −4.34% | 0.400 | −$4.60 | −0.153 |
| 24–168 ч | 78 | −1.55% | −1.42% | 0.423 | −$0.89 | −0.011 |
| >168 ч | 101 | −4.68% | −3.61% | 0.366 | −$2.51 | −0.025 |

Что сколько стоит, если отрезать:

| отсечка | сделок | убыток | доля всего убытка |
|---|---|---|---|
| **возраст < 2 ч** | 282 | **−$115.47** | **84.9%** |
| возраст < 6 ч | 302 | −$124.52 | 91.6% |
| pc5m < −30% | 140 | −$55.97 | 41.2% |
| ликвидность < $10k | 137 | −$38.37 | 28.2% |
| объём 5м > $60k | 62 | −$36.89 | 27.1% |
| капитализация < $30k | 166 | −$32.73 | 24.1% |

Возраст перекрывает всё остальное, и прочие признаки с ним сильно
перекрываются: у десятиминутной монеты по определению мала ликвидность и
капитализация и велик объём.

| минимальный возраст | остаётся | итог |
|---|---|---|
| 0.2 ч (было) | 513 | −$138.54 |
| 2 ч | 231 | −$20.48 |
| **6 ч** | **211** | **−$11.43** |
| 24 ч | 181 | −$6.82 |

Поставлено **6 часов**: снимает 91.6% убытка, оставляя 41% оборота.

**Это разворот 1.11.858**, где я опустил порог до 0.2 ч ради совпадения с
покрытием лидеров. Они запуски действительно торгуют (их p25 по возрасту
0.39 ч), но на нашей кассе мы на них теряем.

Следующий кандидат по величине — потолок объёма 5м: внутри `возраст ≥ 6 ч`
добавление `vol5m ≤ 60k` улучшает −$11.43 до −$4.96. Отдельного знобa для этого
пока нет, и складывать вторую отсечку, подобранную на той же выборке, я не стал.

**Rollback.** `MILD_DIP_MIN_PAIR_AGE_HOURS: '0.2'`.
...
### Покрытие сканирования и гейт `h1_pump_chase`

Разбор двух пропущенных проливов показал две независимые причины, и ни одна не
про пороги пролива.

#### Покрытие: нас нет в тот момент, когда они покупают

Из 4486 покупок лидеров по монетам, которые вообще есть в нашем журнале:

| у нас есть запись в пределах | доля |
|---|---|
| **±5 секунд** | **13.4%** |
| ±30 секунд | 33.6% |
| ±60 секунд | 43.7% |
...
Интервал между сканами одной монеты за 6 часов: медиана **128 с**, p75 506 с,
p90 **28 минут**. Медианная монета осматривается **один раз**.

Причина рукотворная, и бюджет тут ни при чём. При батче в 30 монет на запрос и
потолке 120 RPM путь тянет **3600 монет в минуту**, а мы сканировали **35** —
один процент. Ограничивали две вещи:

- `Math.min(cfg.enrichMax, 12)` в `loop.ts` — жёсткий потолок в 12 монат за
  проход независимо от конфигурации;
- `Math.max(cfg.scanIntervalMs, 15_000)` — при **любой** открытой позиции
  интервал скана прыгал с 3 до 15 секунд. Позиция открыта почти всегда, так что
  медленная ветка была основной.

Обе сняты: потолок теперь берётся из конфигурации (**60**), floor при открытых
позициях стал настраиваемым и равен обычному интервалу (**3 с**). Это ~40
запросов в минуту, треть бюджета.

#### Гейт: `h1_pump_chase`

Монета `AvecKFxn` осматривалась пять раз за 100 секунд до входа лидера, и каждый
раз отказ был `h1_pump_chase`. Правило: если рост за час ≥15%, требуем пролив
не мельче −15%.

Из 1666 покупок лидеров, уже попадающих в нашу полосу −25…0, **503 (30.2%)**
сделаны на монетах, выросших за час на 15%+. Их пролив там: p25 −14.77,
**медиана −9.85**, p75 −5.23.

| требование | отвергает от всех подходящих покупок |
|---|---|
| **−15% (было)** | **22.9%** |
| −10% | 15.3% |
| **−8%** | **12.1%** |
...
Порог опущен до **−8%**: возвращает 10.8% всех подходящих входов и остаётся
выше их медианы, то есть от мелкого отката в пампе всё ещё бережёт.

**Rollback.** `MILD_DIP_ENRICH_MAX: '12'`,
`MILD_DIP_SCAN_INTERVAL_WITH_OPENS_MS: '15000'`,
...
### Зелёная ветка не тратит кредиты Helius

Вопрос был: можно ли кормить зелёную логику из потока, за который мы уже
платим, не увеличивая расход. Ответ — да, и разрешение получится даже выше.

**Где расход.** Главный потребитель платного RPC в торговом контуре —
`stream-price-sampler.ts:108`: на каждый своп по интересующей монете уходит
`getTransaction`. Замер за сутки: **41 986** вызовов, 131 465 свопов пропущено
без вызова, устойчивый темп 0.5 вызова в секунду. Открытые позиции
сэмплируются принудительно.

**Во что обошлась бы зелёная ветка.** Кандидат по определению горячий — порог
`buys5m ≥ 43` означает под сотню свопов за десять минут жизни позиции, то есть
~150 вызовов на сумку. Сто зелёных сумок в сутки добавили бы около трети к
этой статье.

**И это ничего не покупает:**

| источник | медианный интервал |
|---|---|
| лента сэмплера, на которой выводился зелёный выход | **26.83 с** |
| наши марки DexScreener на открытых позициях | **6.08 с** |
| марки из стрима | 6.16 с, p90 **159.59 с** |

Правило +30% / −6% / 10 минут подбиралось на ленте с шагом 27 секунд. Бесплатные
марки DexScreener идут раз в 6 секунд — вчетверо чаще, чем разрешение, под
которое правило создавалось. Стрим здесь не нужен.

**Правка.** `forceFetch` в сэмплере цен пропускает зелёные сумки. Их расход на
Helius сводится к вызовам на исполнение сделок — около 2 000 в сутки при ста
позициях, против ~24 000 текущих.

Вход тоже бесплатен: `pc5m`, `pc1h`, `vol5m`, `vol1h`, ликвидность, число
покупок и продаж приходят в структурном снапшоте DexScreener, а сам сэмплер
зелёных (`mild-dip-leader-green`) не обращается к Helius ни разу — только
DexScreener и Jupiter.

**Rollback.** Вернуть `forceFetch: (mint) => Boolean(state.open[mint])`.
...
### Лестница тейк-профита больше не бесконечная

Когда очередная ступень оставила бы меньше **20%** от исходной сумки, она
вместо этого закрывает позицию целиком.

При шаге в половину остатка получается ровно три ступени:

| ступень | MFE | было в сумке | продаём | останется |
|---|---|---|---|---|
| 1 | +8% | 100% | половину | 50% |
| 2 | +16% | 50% | половину | 25% |
| **3** | **+24%** | 25% | **всё** | — |

Третья ступень раньше срезала бы четверть до одной восьмой; теперь она забирает
остаток целиком.

Это согласуется с тем, где банкуют лидеры: за чистый день 08-12 **42.9%** их
закрытий пришлось на диапазон 0…+25% и лишь 8.4% выше него. Заодно снимается
возня с крошками — остаток меньше 20% всё равно достался бы трейлу или
`dust_close`.

Порог настраиваемый и работает при любом шаге: при доле 0.25 за ступень
лестница дотягивает до шестой, где остаток впервые падает ниже 20%.
`MILD_DIP_EXIT_TP_GRID_MIN_REMAINDER: '0'` возвращает лестницу без потолка.

**Rollback.** `MILD_DIP_EXIT_TP_GRID_MIN_REMAINDER: '0'`.
...
### Зелёная ветка: выход подключён к движку

Позиция получила поле `lane`. Когда оно равно `green`, `decideMarkExit` уходит
в `decideGreenExit` и не трогает ни лестницу, ни трейл, ни порог безубытка:
фиксированная цель +30%, стоп −6%, потолок 10 минут.

Разница между ветками зафиксирована тестами на одной и той же позиции, где
меняется только флаг:

| цена | зелёная ветка | дип-ветка |
|---|---|---|
| −7% | `green_stop`, выход | держит (стоп −25%) |
| +31% | `green_tp`, вся сумка | `tp_grid`, половина остатка |
| +3% через 10 мин | `green_max_hold` | держит |

Вход пока не подключён — зелёные кандидаты в ветку не попадают, поле `lane`
никем не выставляется. Это следующий шаг.

**Rollback.** Убрать блок `pos.lane === 'green'` из
`src/milddip/exit-engine.ts`.
...
### Зелёная ветка: модуль входа и выхода (пока не подключён)

Отдельная ветка для моментум-входов — класс, который лидеры помечают `green`.
Это не дип-стратегия с другим порогом, это противоположная сделка, и выход ей
нужен противоположный, поэтому она живёт отдельно.

**Вход выведен из сэмплера.** 566 828 замеров по 678 монетам размечены по тому,
покупал ли лидер эту монету в ближайшие 10 минут. База 5.52%. Признаки
ранжированы по сдвигу шансов (нижняя пятая часть против верхней):

| признак | нижняя пятая | верхняя пятая |
|---|---|---|
| `vol5m / liq` | ×0.22 | **×3.18** |
| `vol1h` | ×0.15 | ×2.46 |
| `buys5m` | ×0.17 | ×2.45 |
...
Два признака неочевидны: они **избегают односторонней ленты** (доля покупок выше
0.85 роняет шансы до ×0.23) и входят **на микро-откате** — при pc5m +10.7%
последняя минута у них медианно −0.51%.

Комбинация даёт точность **0.275** против базы 0.055, то есть **×4.97**.

**Выход выведен из форвардной ленты.** Зелёные входы вспыхивают и умирают:
медиана −14.4% через 15 минут и −43.6% через час, при этом 63.2% успевают
коснуться +10% за первые 15 минут. Трейл здесь — неподходящий инструмент.
Перебор по 156 дедуплицированным сигналам:

| TP | стоп | максимум | среднее | винрейт |
|---|---|---|---|---|
| **+30%** | **−6%** | **10 мин** | **+4.91** | 0.316 |
| +30% | −8% | 15 мин | +3.89 | 0.323 |
| +20% | −6% | 5 мин | +3.04 | 0.374 |
| +6% | −15% | 5 мин | −2.20 | 0.581 |

Последняя строка — это то, что делает с зелёной свечой дип-подобный выход, и он
теряет деньги.

**Что в этом релизе.** Чистый модуль `src/milddip/green-lane.ts`
(`evaluateGreenLane`, `decideGreenExit`) и секция конфигурации `green` с
клипом $1. Восемнадцать тестов, включая проверки на то, что обычный пролив в
зелёную ветку не попадает, что односторонняя лента отсекается и что отсутствие
поля означает отказ, а не проход.

**Ветка выключена** (`MILD_DIP_GREEN_ENABLED=false`) и ещё не подключена к
торговому пути — подключение отдельным шагом.

**Rollback.** Удалить секцию `green` из `src/milddip/config.ts` и файл
`src/milddip/green-lane.ts`.
...
### Пороги входа: ликвидность $15k → $6k, полоса до нуля, объём $300 → $150

**Как измеряли.** Для каждой покупки лидера нашли нашу же запись метрик по той
же монете в пределах десяти минут — 1288 совпадений. Затем спросили, какой
именно порог нас останавливал.

| порог | блокирует | доля |
|---|---|---|
| **ликвидность < $15 000** | 848 | **65.9%** |
| pc5m мельче −3% | 534 | 41.5% |
| возраст < 0.5 ч | 409 | 31.8% |
| объём 5м < $300 | 249 | 19.4% |
| pc5m глубже −25% | 231 | 18.0% |
| капитализация < $5 000 | 100 | 7.8% |
| возраст > 720 ч | 33 | 2.6% |

**Все наши пороги проходили лишь 176 покупок из 1288 — 13.7%.**

Их метрики на входе, измеренные нами: ликвидность p25 $6 726, **медиана
$11 344**; pc5m медиана −3.39, p75 +4.94; объём 5м p10 $153.

Наш порог ликвидности стоял выше середины диапазона, в котором они работают.

**Три сделки, с которых начался разбор:**

| монета | pc5m | объём 5м | ликвидность | что нарушено |
|---|---|---|---|---|
| `488SaFq6` | +1.26 | $130 | $26 479 | объём, полоса |
| `B4G24zZR` | −0.14 | $11 | $21 845 | объём, полоса |
| `2uEveTPS` | +32.75 | $5 667 | $9 285 | ликвидность, полоса |

Две из трёх — не зелёные, а плоские: −0.14% и +1.26%.

**Новые значения:** ликвидность **$6 000**, потолок полосы **0%** (нижняя
граница −25% не меняется), объём 5м **$150**, минимальный возраст **0.2 ч**.

Через эту сетку проходит **444 из 1288** покупок лидеров вместо 176 — воронка
шире в **2.5 раза**.

Нижнюю границу полосы намеренно не опускали: −40% добавил бы ещё 61 покупку, но
это класс `rug_knife`, который убыточен и у самих лидеров (−$345 за 08-12 при
винрейте 0.350). Глубокие проливы и так имеют отдельную ветку через
`TURN_DUMP_KNIFE_BRANCH`. Зелёные входы остаются вне рассмотрения.

**Rollback.** `MILD_DIP_MIN_LIQUIDITY_USD: '15000'`, `MILD_DIP_MAX_DIP_PCT: '-3'`,
`MILD_DIP_MIN_VOLUME_5M_USD: '300'`, `MILD_DIP_MIN_PAIR_AGE_HOURS: '0.5'`.
...
Все три тира (`POSITION_USD`, `THICK_POSITION_USD`, `MICRO_POSITION_USD`)
подняты с $1 до $3.

**Порог dust заодно снижен $2 → $0.60.** Лестница режет остаток пополам: при
клипе $3 после первой ступени остаётся $1.50, после второй $0.75. Порог $2
подмёл бы каждого бегунка как пыль через полчаса удержания — это ровно та
ошибка, что была в 1.11.841, когда клип упал до $2 при пороге $2. $0.60
срабатывает только на том, что осталось после трёх ступеней.

`MILD_DIP_RUG_KNIFE_CLIP_USD` остаётся $2 и теперь действительно работает как
пониженный клип для рискованных входов — раньше он был вдвое больше базового.

**Rollback.** Вернуть три тира в `'1'` и `MILD_DIP_EXIT_DUST_CLOSE_USD: '2'`.
...
### Возврат лестницы фиксации и трейл 30% → 12%

**Что сломалось.** Позиция `236wfN8D…` провисела 28 минут: пик MFE **+25.02%**,
затем распад до +10.97% и ни одного сработавшего правила.

| правило | порог | факт |
|---|---|---|
| трейл | откат −30% | −11.24% |
| порог безубытка | pnl ≤ 0 | +10.97% |
| стоп | pnl ≤ −25% | +10.97% |

Между −11% и −30% отката образовалась мёртвая зона. Сняв в 1.11.850 обе
лестницы, я не оставил ни одного правила, забирающего умеренную прибыль.

**Почему 1.11.850 был неверен.** Он опирался на выборку, смешавшую дни до и
после починок обзервера: до них нативные переводы SOL не читались, обороты
занижены в 25 раз, сотни ног шли вообще без котировки. Плюс фильтр «цена сделки
в пределах 3x от Dex» оставлял 8% данных. Отсюда взялись и «99.8% прибыли из 13
сделок», и вывод «лестницы у них нет». И то и другое — артефакт.

**Чистый день 08-12** (единственный, где ни одна нога не осталась без
котировки), 219 закрытых позиций лидеров, счёт по SOL без фильтров:

| кошелёк | n | медиана | винрейт | P&L |
|---|---|---|---|---|
| `7BNaxx6K` | 128 | +0.19% | 0.500 | **+$4 961** |
| `8zkgFGVZ` | 91 | +0.50% | 0.527 | **+$3 686** |

Распределение закрытий: ниже −20% — 17.8%, от −20% до 0 — 31.1%,
**от 0 до +25% — 42.9%**, от +25% до +100% — 7.8%, выше +100% — 0.5%. Одна
сделка выше +100% за день, и та на $457. Никакой лотереи: деньги делаются на
94 позициях в диапазоне 0…+25% (+$11 965).

С трейлом 30% в эту полосу попадало 16% наших выходов. Возвращена лестница
+8% × половина остатка (без потолка) и трейл поджат до 12%: пик +25% теперь
даёт две ступени и выход остатка около +10%, то есть ровно там, где банкуют они.

Пороги убытка не тронуты: безубыток после +8%, стоп −25%, cliff −50%.

**Rollback.** `MILD_DIP_EXIT_TP_GRID_STEP_PCT: '0'` и
...
### Порог безубытка: сумка, побывавшая в плюсе, не возвращается убытком

**Сделка `2iKmjMW3…`:** вход `1.1357e-04`, пик `1.2890e-04` (MFE **+13.5%**),
выход через 48 минут по `peak_giveback` на `8.4572e-05` — откат 33.17% от пика,
результат **−25.53%**.

Это не сбой, а арифметика трейла: откат 30% на пике +13.5% даёт
1.135 × 0.70 = **−20.5%** ещё до всякого проскальзывания. У лидеров пики на
победителях медианно +28.75%, там те же 30% оставляют плюс; на наших пиках нет.

**Проверка на их данных, 355 путей:**

| трейл | порог | среднее | медиана |
|---|---|---|---|
| 30% | нет (было) | +14.92 | **−5.44** |
| **30%** | **+8% → 0** | **+13.53** | **0.00** |
| 40% | нет | +8.10 | −8.31 |
| 70% | +8% → 0 | +13.87 | 0.00 |

Порог стоит **1.4 пункта среднего** и переводит медиану из минуса в ноль. Хвост
почти не страдает: из их позиций, которые взвели +8% и потом ушли под ноль,
выше +100% доработали лишь **5.4%**, выше +500% — 2.7%.

**Правка.** После того как MFE достигал `breakevenArmPct` (**8%**), полный выход
при падении P&L до `breakevenFloorPct` (**0%**). Ниже по-прежнему распоряжаются
стоп −25% и `cliff_dump` −50%.

**Отдельно стоит записать.** Механический трейл вообще не воспроизводит их
результат: их фактическое среднее по этим же 355 позициям **+103.06%**, а лучшая
трейл-конфигурация даёт **+14.92%**. Ширина трейла тут ни при чём — перебор от
30% до 80% укладывается в диапазон +1…+15%. Их монстры переживают глубокие
просадки посреди роста, и фиксированный откат выбивает из них раньше времени.
Чем именно они закрывают такие позиции, по данным не видно.

**Rollback.** `MILD_DIP_EXIT_BREAKEVEN_ARM_PCT: '0'`.
...
### Полоса входа: потолок −8% → −3%

**Почему откатываем 1.11.825.** Полоса была сужена до −8% на разборе MFE,
посчитанном по нашей ленте марок. Лента оказалась отравлена (1.11.847 —
подстановка цены чужого токена, 1.11.848 — устаревшая марка как база), поэтому
обоснование недействительно.

**Что говорят лидеры.** 3288 дедуплицированных покупок с записанным изменением
за 5 минут: p10 −33.72, p25 −16.99, медиана **−5.67**, p75 +6.81.

| полоса pc5m | доля их покупок | наш гейт |
|---|---|---|
| глубже −25% | 16.1% | режем |
| −25…−15% | 12.2% | пропускаем |
| −15…−8% | 15.6% | пропускаем |
| **−8…−3%** | **13.7%** | **режем** |
| −3…0% | 6.8% | режем |
| зелёные | 35.6% | режем |

В нашу полосу попадает лишь **27.8%** их покупок. Медиана их входа −5.67% лежит
ровно в той зоне, которую мы закрыли.

Разбивка отказов за 20 минут подтверждает: после подъёма потолка возраста
главным блокиратором стала именно полоса — **60.2%** всех `structural_fail`
приходится на `pc5m > −8`, тогда как на возраст остаётся 24.2%.

Потолок возвращён на **−3**. Зелёные входы (35.6% у них) остаются вне
рассмотрения, как и договаривались.

**Rollback.** `MILD_DIP_MAX_DIP_PCT: '-8'`.
...
### Потолок возраста пары 72 часа → 30 суток

**Как нашли.** Пользователь прислал `2F373Bxe…` с вопросом, почему мы не видим
монету, которую лидеры торгуют вовсю. Мы её видели: 345 записей за пять часов,
из них 145 отказов, и 135 из них — `structural_fail`. Метрики в момент отказа:

| метрика | значение | порог | вердикт |
|---|---|---|---|
| ликвидность | $135 330 | ≥ $15 000 | проходит |
| объём 5м | $71 830 | ≥ $300 | проходит |
| капитализация | $1 892 891 | ≥ $5 000 | проходит |
| pc5m | −9.03 … −12.37% | −25 … −8% | проходит |
| **возраст пары** | **1026 ч (42.8 суток)** | **≤ 72 ч** | **режет** |

Два лидера торговали её клипами по $870–1125.

**Масштаб.** Из 845 монет, купленных лидерами, возраст удалось измерить у 725:
медиана 35 ч, p75 96 ч, p90 346 ч.

| возраст | доля монет лидеров |
|---|---|
| старше 24 ч | 59.4% |
| старше 72 ч | **32.7%** |
| старше 7 суток | 18.3% |
...
Из 237 отсечённых по возрасту **113 проходят все прочие пороги**, то есть
**15.6% их вселенной мы отказывали только из-за возраста**. Все 237 отклонялись
именно как `structural_fail`.

Крупнейшие отказы: `Ge87EtsjwR` (650 ч, ликвидность $957 828, объём 5м
$145 784, 636 отказов), `Ai66LHZG9M` (404 ч, $928 901, $96 979, 798 отказов),
`A13oRB9FFa` (95 ч, $563 548, $273 001, 848 отказов).

Обратная сторона: медианный возраст монет, которые смотрим мы, а лидеры не
трогают, — **7.2 часа**. Мы ловили свежие запуски, они торгуют состоявшиеся
имена с ликвидностью на порядок глубже.

**Правка.** 720 часов — это покрывает 96% их вселенной. За «живость» отвечают
пороги ликвидности $15k и объёма $300 за 5 минут, они не меняются; сорокадневная
монета с объёмом $270k за пять минут мёртвой не является.

**Rollback.** `MILD_DIP_MAX_PAIR_AGE_HOURS: '72'`.
...
### Fix: один фантомный принт из стрима выбивал позицию по стопу

**Сделка `2JPU5K89…`, последние секунды:**

| t | цена | pnl | источник |
|---|---|---|---|
| −17.7с | 5.6950e-04 | +22.89% | dex |
| −11.6с | 5.6420e-04 | +21.75% | dex |
| −5.6с | 5.6420e-04 | +21.75% | dex |
| **−1.5с** | **3.2402e-04** | **−30.08%** | **stream, −42.57% за тик** |
| 0.0с | — | — | `hard_stop`, вся сумка |

Позиция шла на +21.75% при MFE +18.82%. Один принт из стрима обвалил марку на
42.57% относительно соседней, стоп −25% принял это за правду и закрыл всё.
Исполнение при этом прошло по реальной цене `5.6545e-04` (+22.02%), то есть
деньги на этой сделке мы получили — но из растущей монеты нас выбросил фантом,
а соседние марки от Dex стояли ровно.

**Правка.** Марка, ушедшая от предыдущей принятой более чем на
`markJumpConfirmPct` (живое значение **25%**), помещается в карантин: она ничего
не решает, не двигает пик, арм и `lastMarkPriceUsd`, а только запоминается.
Решение принимается, когда следующая марка подтверждает уровень. Настоящий обвал
стоит одного лишнего тика; фантом стоил позиции.

Марки в карантине пишутся в журнал с флагом `quarantined`, потому что pnl и mfe
для них не считаются — офлайн-анализ обязан их отбрасывать, а не читать нули.

Пять тестов на числах этой сделки: фантом не закрывает позицию, пик и арм не
трогаются, возврат к реальному уровню продолжает работу как обычно, повторный
принт на том же уровне стоп всё-таки исполняет, обычное движение на 9% под
карантин не попадает.

**Rollback.** `MILD_DIP_EXIT_MARK_JUMP_CONFIRM_PCT: '0'`.
...
### Fix: правило отскока обязано было продавать, пока цена ещё в минусе

`never_arm_bounce` создавалось, чтобы выходить в отскок. Но у него стоял гейт
`neverArmBounceRequireRedPct: 3`, который **требует, чтобы позиция была минимум
на 3% в минусе**. То есть правило структурно не могло дождаться, пока отскок
вернёт нас в ноль: как только цена отрывалась от дна на 8%, оно продавало —
обязательно в красной зоне.

**Сделка `gjj1wK9v…` (6SyrTP), по секундам:**

| t+с | цена | pnl | событие |
|---|---|---|---|
| 90.7 | 3.8207e-05 | −21.82% | дно |
| 117.1 | 4.4354e-05 | −9.24% | отскок +16% от дна |
| 118.6 | 4.5244e-05 | **−7.42%** | `never_arm_bounce` продал половину |
| 139.4 | 4.9161e-05 | **+0.59%** | цена выше входа |

Итог $1.00 → $0.9764. Через двадцать секунд после продажи бумага стоила дороже
входа.

**Правка.** Добавлен `neverArmBounceMinPnlPct` — пол по P&L, ниже которого
отскок продавать не имеет права. Живое значение **0**, а `REQUIRE_RED_PCT`
снят в 0. Теперь в отскок выходим только когда он окупил провал; ниже нуля
бумагой распоряжаются стоп −25% и `never_arm_time_red`. Отсутствие поля
означает «выключено», поэтому вызовы, написанные до этой правки, ведут себя
как раньше.

Четыре теста на цифрах этой сделки: старый гейт продавал на −7.42%, новый
держит через тот же отскок, продаёт после возврата в плюс, а ниже — работает
стоп.

**Заодно проверено:** вторая присланная сделка, `4LQhpQfi…` (XFdE5w), оказалась
прибыльной — куплена по 2.2894e-04, продана двумя ногами по 2.6721e-04 (+16.72%)
и 2.6165e-04 (+14.28%), $1.00 → $1.177. Продажи прошли выше последней марки,
красной свечи там не было.

**Rollback.** `MILD_DIP_EXIT_NEVER_ARM_BOUNCE_MIN_PNL_PCT: '-1000'` и
...
### Лестницы тейк-профита нет. Один полный выход по трейлу 30% от пика

Разгадка стратегии выхода лидеров, ради которой всё и затевалось. Ответ:
**лестницы у них нет, и именно её отсутствие является стратегией.**

**Как считалось.** 8963 дедуплицированных ноги обзервера. В зачёт шли только
те, что доказывают себя сами: реальное движение SOL или USDC на обеих сторонах,
цена сделки в пределах 3x от цены Dex, и сумка сходится по токенам (0.9–1.1x).
Выжило 3526 ног, 342 позиции. Флаг `sizeUsdEstimated` игнорировался — большая
часть файла старше самого флага.

**Проверка вменяемости прошла:** медиана −3.36%, победителей 44.5%, выше +500%
заявляют 3.8% позиций. До фильтров те же данные давали медиану +121% и среднее
+1201%, то есть были непригодны.

**Откуда у них деньги:**

| результат позиции | штук | доля прибыли |
|---|---|---|
| выше +500% | 13 (3.8%) | **99.8%** |
| +100…500% | 12 (3.5%) | 5.7% |
| +25…100% | 32 (9.4%) | 3.9% |
| 0…25% | 96 (28.1%) | 2.5% |
| −20…0% | 105 (30.7%) | −2.4% |
...
Тринадцать сделок из 342 дают всё. Среднее +108.65% при медиане −3.36%.

**Что делает с этими же сделками лестница:**

| выход | среднее |
|---|---|
| как торгуют они — один полный выход | **+108.65%** |
| +8% × 50% (1.11.849) | **−8.17%** |
| +16% × 50% | −5.56% |
| +25% × 50% | −3.47% |
| +50% × 50% | +0.66% |
...
Любая ступень режет тот самый хвост, который оплачивает все убытки. Вчерашняя
лестница `bank1 +6% / bank2 +8%` и выкаченная час назад лестница Oscar
+8%×50% — обе делают ровно это.

**Структура их выходов подтверждает то же самое:** 90.7% ног закрывают весь
остаток, частичных всего 9.3%, и 49.3% позиций закрываются одной сделкой.
Несколько ног по одной монете — это не ступени, а отдельные заходы через часы
(2-я нога медианно через 76.8 мин, 3-я через 124.8 мин).

**Новая конфигурация.** Обе лестницы выключены, частичного скейл-аута нет,
полный выход при откате **30% от пика** — ширина взята из их собственной ленты
марок, где отдача на выходе держится константой ~30% пика для любого пика выше
15% (−29.4% / −29.9% / −33.6% по корзинам). Трейл взводится с +5% MFE.

| | было (1.11.849) | стало |
|---|---|---|
| ступени | +8%, +16%, … по половине остатка | нет |
| частичный скейл-аут | 3% отката, половина | нет |
| выход | ступени + трейл 12% | **один полный выход, откат 30% от пика** |
| потолок прибыли | ~+16% на бегунке | нет |

**Выходы из убыточных монет не изменены:** `hard_stop` −25%, `cliff_dump` −50%,
`never_arm_time_red`, `never_arm_bounce`, `dust_close`. Лидеры, к слову, держат
убыток шире нас: 24.6% их позиций закрываются ниже −20%.

**Rollback.** Вернуть `MILD_DIP_EXIT_MFE_BANK1_PCT: '6'`,
`MILD_DIP_EXIT_PARTIAL_GIVEBACK_PCT: '3'`, `MILD_DIP_EXIT_GIVEBACK_PCT: '8'`.
...
### Бесконечная лестница take-profit из Live Oscar. Выходы из убытка не тронуты

**Что было.** `bank1` продавал 40% на +6% MFE, `bank2` — оставшиеся 60% на +8%.
Сумка кончалась на +8%, то есть потолок прибыли с любой монеты был около +7%,
тогда как проигравшие доходили до стопа −25%. Замер по кассе за сутки:
победители 40.4% по +8.78%, проигравшие 59.6% по −16.10%. Мы рисковали 25, чтобы
заработать 7; чтобы выйти в ноль при такой доле побед, средний проигравший
должен быть не глубже −5.96%.

**Что стало.** Порт лестницы Live Oscar (`WAVE_B_FLAT_TP_HALF8_RUNNER`,
`src/papertrader/executor/exit-policy-wave-b.ts:74`): **половина остатка на
каждые +8%**, верхней ступени нет. Считается как у Oscar — `maxK` без потолка
(`tp-grid-effective.ts:42`). Сумка при этом никогда не заканчивается: монета,
которая продолжает расти, продолжает платить.

| | было | стало |
|---|---|---|
| ступени | +6% (40%) и +8% (60%) | +8%, +16%, +24%, … без конца |
| доля | от исходной сумки | **половина остатка** |
| потолок прибыли | ~+7% | нет |
| трейл остатка | 12% от пика | 12% от пика (без изменений) |

**Ступени считаются от текущей цены, а не от пика.** Это ровно как у Oscar
(`pnlFrac = xAvg − 1`, `tracker.ts:5541`) и это принципиально: при отсчёте от
пика после схода с +50% лестница осталась бы должна пять ступеней и вываливала
бы сумку по дороге вниз по ценам, которых уже нет. Ошибка была допущена в первой
версии правки и поймана тестом.

**Выходы из убыточных монет не изменены ни в одном значении:** `hard_stop` −25%,
`cliff_dump` −50%, `never_arm_time_red` (≥5 мин и ≤−15% и pc5m ≤−5%),
`never_arm_bounce` (провал ≥8%, дно старше 60 с, отскок +8%, всё ещё ≥3% в
минусе), `mfe_bank_sleeve` с половиной под водой, `dust_close`. Отдельный тест
это фиксирует.

**Чем рискуем, честно.** Монета, которая дошла ровно до +8% и развернулась,
теперь даёт примерно +1.5% вместо прежних +7.2%: половина уходит на ступени,
половина ловит трейл на −12% от пика. Выигрыш появляется только на тех, кто
идёт дальше. Прежний замер «72% монет после +10% возвращаются к нулю» считался
по отравленным маркам (см. 1.11.847 и 1.11.848) и доверия не заслуживает —
пересчитать надо на чистых данных. Откат в одну строку.

**Rollback.** `MILD_DIP_EXIT_TP_GRID_STEP_PCT: '0'` — лестница выключается,
двухступенчатый bank возвращается без других изменений.
...
### Fix: устаревшая марка Dex читалась как мгновенная прибыль

Марка Dex и наша цена исполнения живут на разных шкалах. Когда снапшот
залежался или относится к другому пулу, он может стоять заметно выше цены,
которую реально дал Jupiter. Позиция в этот момент показывает плюс, хотя цена
не двигалась.

**Сделка `493B9wrr…` (EUB1eZ), по секундам:**

| t+с | событие | цена |
|---|---|---|
| 9017.0 | марка **до** покупки | `8.492e-05` |
| 9018.1 | покупка, факт | `7.683e-05` |
| 9021.1 | марка через 3 с | `8.492e-05` → pnl **+10.52%**, `trail_armed` |
| 9022–9028 | четыре попытки продать | `no_token_balance` |
| 9031.8 | продажа прошла | `7.279e-05` → **−5.26%** |
| 9036.9 | остаток, `mfe_bank_2` | `7.542e-05` → −1.83% |

Марка `8.492e-05` не изменилась ни разу: она стояла до покупки и держалась все
двенадцать секунд после. Мы купили на 9.5% ниже неё, движок счёл разницу
прибылью, взвёл трейл и дёрнул `mfe_bank_1`. Итог по кассе: $1.00 → $0.97475.

**Масштаб.** За сутки 27 из 187 позиций (**14.4%**) набирали ≥ +6% MFE в первые
десять секунд после сделки — то есть проходили порог `mfe_bank_1` до того, как
цена вообще шевельнулась. У тех, кто показывал ≥ +5% за 10 с, касса вышла в
среднем −5.32%.

**Правка.** База для MFE, взведения трейла и giveback — цена Dex, на которой
принималось решение о входе (`entryMarkPriceUsd`, пишется при создании позиции
из `c.priceUsd`). Применяется только когда она выше цены сделки: покупка выше
марки коррекции не требует, она MFE занижает, и за неё уже отвечает стоп.
`pnlPct` сохраняет базу по цене сделки — стоп обязан отвечать за реальные
деньги.

Четыре теста на сценарий: неподвижная цена не взводит трейл и не банкует;
последующий рост считается от ленты; марка ниже сделки базу не меняет;
настоящее движение при совпадении марки и сделки считается полностью.

**Первая попытка была нерабочей** и это стоит записать. Она защёлкивала базу на
«первой марке после сделки» через условие `pos.peakPriceUsd == null`, но
`entry-attempt` создаёт позицию с уже проставленным `peakPriceUsd`
(строки 673 и 928), так что условие не выполнялось никогда. Тест этого не
поймал, потому что его фикстура оставляла поле пустым — в отличие от боевого
кода. Обнаружено на сделке `4LDzrue1…`, где `mfe` в журнале по-прежнему считался
от цены сделки. Фикстура теста приведена к тому, как позиция рождается в бою.

Заодно выяснилось, почему «первая марка» — неверный признак: если цена честно
выросла сразу после покупки, такое правило гасило настоящий рост. Признаком
служит не порядок марок, а то, что марка не менялась с момента до покупки, — а
это и есть цена входного решения.

**Rollback.** Убрать `entryMarkPriceUsd` из `src/milddip/state.ts`,
`src/milddip/entry-attempt.ts`, `src/milddip/exit-engine.ts`,
`src/milddip/loop.ts` и `mfeBasisPriceUsd` из `src/milddip/gates.ts` — MFE
вернётся к базе по цене сделки.
...
### Fix: батч DexScreener подставлял монете цену чужого токена

`pickBestSolanaPairForMint` при отсутствии пары для запрошенного минта откатывался
на весь список пар из ответа:

```ts
let pool = asBase.length > 0 ? asBase : asQuoteOnly;
if (pool.length === 0) pool = solana;   // ← чужие токены
```

Для одиночного запроса это было почти безобидно — в ответе лежали пары только
этого токена. Но с 1.11.820 мы ходим батчем по 30 адресов, и ответ битком набит
чужими парами. Любой минт, для которого DexScreener пары не вернул, получал
**самый ликвидный пул постороннего токена** и жил дальше с его ценой,
ликвидностью, mcap и объёмом.

**Как нашли.** Контрольная проверка бэктеста: симуляция действующей лестницы на
нашей же ленте марок дала +$40 448 при среднем +1855%, тогда как касса за те же
сутки показала −$65.97. Дальше в журнале нашлись три разные монеты с разными
ценами входа и одним и тем же пиком:

| минт | entryPx | peakPx | mfePct |
|---|---|---|---|
| 6WUbAGBQd | 1.719e-05 | **1.545e-03** | +8886% |
| 4MpMPRkkS | 7.817e-06 | **1.545e-03** | +19665% |
| 2pkwbAevw | 3.976e-05 | **1.545e-03** | +3786% |

**Масштаб.** 0.22% марок за сутки заявляли ≥ +100% (максимум +192491%), задеты
24 из 209 минтов (11.5%). 126 sell-плечей ушли с `mfePct ≥ +50%`, из них 30 через
`mfe_bank_sleeve` — то есть движок считал giveback от несуществующего пика. 64
плеча (2.8%) заявили пик ≥2x и закрылись ниже входа.

Заражение шло не только в марки: те же поля кормят входные гейты, поэтому монета
могла пройти пороги ликвидности, mcap и объёма по данным чужого токена.

**Правка.** Нет пары для минта — это промах, а не повод взять чужую цену.
`pool.length === 0` теперь возвращает `null`. Три теста закрывают сценарий:
отсутствующий минт даёт `null`, присутствующие в том же ответе резолвятся
нормально, и двум разным отсутствующим минтам не выдаётся одна и та же цена.

**Rollback.** Вернуть `if (pool.length === 0) pool = solana;` в
`src/papertrader/pricing/dexscreener-pair-pick.ts`.
...
### Fix: лента лидеров отставала на пять часов из-за отсутствующего `commitment`

`getSignaturesForAddress` по умолчанию отвечает на уровне **finalized**, а
finalized на этом провайдере отстаёт. Замер 12 августа, один и тот же кошелёк в
одну и ту же секунду:

| commitment | самая свежая подпись | отставание |
|---|---|---|
| по умолчанию | 03:46:42 | **313 минут** |
| `finalized` | 03:46:42 | 313 минут |
| **`confirmed`** | **08:59:47** | **0** |

Транзакции при этом были доступны через `getTransaction` всё это время —
отставал именно адресный индекс.

**Как нашли.** Пользователь прислал две транзакции лидеров, датированные 08:16 и
08:55 UTC. Прямой запрос по подписи их видел, а `getSignaturesForAddress` для тех
же адресов отдавал максимум 03:47. Публичный `api.mainnet-beta` в тот же момент
показывал 08:56 — то есть дело было не в цепочке.

**Что это обесценивает.** Всё, что построено на ленте лидеров: `leader-seed`,
гейт `REQUIRE_LEADER_SEEN`, датасет обзервера, вчерашние и сегодняшние сравнения
«мы против лидеров» по времени. В частности вывод «в момент их покупки мы не
смотрим на монету в 85% случаев» измерялся против отметок времени, которые мы
узнавали на пять часов позже, — это не опровергает вывод, но делает его
непроверенным.

**Что НЕ затронуто.** Части A ночного разбора (наши убытки против признаков
входа) и порог ликвидности $15k — они считаются целиком по нашему журналу.

**Починка.** `{"commitment": "confirmed"}` в обоих местах опроса кошельков:
`scripts/milddip/leader-observer.py` и `src/copytrader/rpc.ts`.
...
### Fix: обзервер переиздавал одну транзакцию сотни раз — все объёмы лидеров были завышены в ~48 раз

`_save_state` обрезал память подписей так: `list(self.seen)[-5000:]`. У множества
нет порядка, поэтому срез оставлял **произвольные** 5000 и выбрасывал свежие
подписи. Выброшенная подпись переобрабатывалась на каждом опросе, пока не
выпадала из окна `lookbackSec = 1800`.

Замер за 12 августа:

| | |
|---|---|
| эмитировано ног | **45 006** |
| различных транзакций | **924** |
| худшая подпись | переиздана **348 раз** за 29.7 минут |
| каденс переизданий | **5.1с** — ровно интервал опроса |
| доля USD-объёма внутри повторов | **98%** |

**Что это обесценивает.** Все цифры оборота и денежного потока лидеров, включая
приведённые мной вчера и сегодня утром. Сырой подсчёт давал «$5.1M покупок и
+$2.2M чистыми за сутки» — после дедупликации по подписи это:

| лидер | покупок | продаж | чистыми | минтов |
|---|---|---|---|---|
| 8zkgFGVZ | 217 ($58 087) | 212 ($56 060) | **−$2 027** | 133 |
| 7BNaxx6K | 244 ($54 250) | 251 ($57 663) | **+$3 414** | 141 |
| **вместе** | | | **+$1 387** | |

За восемь часов, при обороте ~$112k на двоих. Около **1.2%** на оборот, и один
кошелёк в минусе.

Также обесценивается вывод «лидеры входят сотнями ног с разрывом менее 5 секунд»:
это и был эхо-артефакт с каденсом опроса.

**Что НЕ затронуто.** Часть C ночного разбора: метки «лидер купил этот минт в
ближайшие 10 минут» строятся по `blockTime`, одинаковому у всех дублей, поэтому
позитивы и точность отсечек (turn ≥ 0.3 → 8.0x и т.д.) остаются в силе. Части A и
B считаются по нашему собственному журналу.

**Починка.** `self.seen` — это `dict signature -> blockTime`; обрезка по возрасту
(`lookback × 3`, минимум 2 часа), потому что подпись старше окна вернуться не
может. Счётный потолок остался запасным и применяется от новых к старым. Старый
формат списка читается и стареет при первой обрезке.
...
### Большой ночной разбор: причина убытков, discovery, отбор лидеров

13 часов чистых данных, 390 закрытых сумок, 388 032 снимка вселенной с
негативами, 30 596 покупок лидеров.

**Состояние.** Кошелёк за 12ч **+$0.07** — впервые не в минусе. Средняя сделка
−1.55%, медиана **+1.93%**, положительных 52%. Худшие 5% (19 сумок) дают −908
пунктов при общей сумме −604, то есть **без них книга в плюсе**.

#### A. Что предсказывает наши убытки

Доля сделок хуже −25% по признакам входа (база 12%):

| признак | плохой срез | доля глубоких | хороший срез | доля |
|---|---|---|---|---|
| возраст пары | 0.5–2ч | **21%** | 8–24ч | 2% |
| ветка turn/dump | knife | **26%** | main | 8% |
| оборот vol5m/liq | ≥0.3 | 18–20% | <0.3 | 4% |
| ликвидность | <$20k | 13–14% | ≥$50k | **0%** |
| доля покупок 5м | >0.60 | mean −6.68 | 0.50–0.60 | mean +1.80 |

Комбинации на сумме: `age≥2ч и turn<0.3` → +1.9 (было −647);
`age≥2ч и turn<0.3 и liq≥20k` → **+130 при нулевой доле глубоких убытков**.

#### B. Discovery: мы видим больше монет, но не в тот момент

- Мы оцениваем **3742** различных минта за 12ч; лидеры купили **229**.
- Но в момент их покупки (±15с) мы **не смотрим на монету в 85%** случаев.
  Медиана разрыва с нашего прошлого взгляда на этот минт — **214 секунд**.
- Наша собственная задержка «первый взгляд → филл»: медиана **7106 секунд**.
  Доля филлов в пределах 60с от первого взгляда — **0%**.
- Потолок: стрим отдаёт **122 события/мин**, мы оцениваем **40 минтов/мин**.

Причины отказа ровно в момент покупки лидера: `structural_fail` 3087,
`cooldown_bounce` 1100, `rebuy_below_exit` 584, `turn_dump` 503.

#### C. Отбор: почему из 14 они берут одну

На 388 032 снимках с негативами базовая вероятность «лидер купит в ближайшие
10 минут» = **2.57%**. Одиночные отсечки:

| отсечка | точность | подъём |
|---|---|---|
| `turn ≥ 0.3` | 20.5% | **8.0x** |
| `vol5m ≥ 20k` | 21.1% | **8.2x** |
| `vol5m ≥ 5k` | 16.1% | 6.3x |
| `mcap ≥ 80k` | 7.1% | 2.7x |
| `liq ≥ 20k` | 6.8% | 2.6x |
| `age ≥ 2ч` | 1.9% | **0.74x** — хуже базы |

Лучшая комбинация: `vol5m≥5k И turn≥0.3 И buyShare≥0.5` → точность **25.2%**,
подъём **9.8x**, recall 30%. То есть «одна из 14» превращается в **одну из
четырёх**.

Медианы позитивов против негативов: `vol5m` 3127 против **0.53**, `turn` 0.174
против 0.003, `vol1h` 64516 против 676. Первый порядок ответа простой: **лидеры
покупают монеты, в которых вообще идёт торговля**, а наша вселенная в основном
мёртвая.

Плюс лидеры не входят один раз: 30 596 ног на 230 минтов, медиана размера ноги
**$128**, и **98%** соседних ног на одной монете идут с разрывом менее 5 секунд.

#### Главное противоречие

**Признаки, по которым опознаётся покупка лидера, у нас предсказывают убыток.**

| срез | у лидеров | у нас (mean / доля глубоких) |
|---|---|---|
| `turn ≥ 0.3` | подъём 8.0x | **−2.07% / 18%** |
| `vol5m ≥ 5k` | подъём 6.3x | +0.21% / 16% |
| `age < 2ч` | предпочитают | −1.98% / 19% |
| `liq ≥ 20k` | подъём всего 2.6x | **+1.49% / 9%** |

Наш лучший фильтр не является их селектором, а их лучший селектор нам вреден.
Копировать их отбор без их размера и их сотен ног — значит покупать ровно те
сделки, на которых мы теряем.

### Change: порог ликвидности $5k → $15k

Единственная правка, которую разбор поддерживает сам по себе. Средняя по порогу
монотонна на хорошо покрытом диапазоне: $5k −1.44%, $10k −1.17%, $12k −0.57%,
**$15k +0.39%**, $20k +1.39%; разворот на $30k при n=76 — шум.

$15k выбран вместо более красивого $20k: это самый высокий порог, который
улучшает среднюю **во всех трёх** подокнах (−1.36→+1.57, −0.61→+1.35,
−2.67→−1.35), и он оставляет половину оборота, а не треть.

Механизм, а не только подгонка: тонкая ликвидность — источник наших худших
сделок. Глубокие убытки это разрывы цены (марка около нуля, следующий филл
−30…−70%), и стоп не может исполниться внутри такой книги.

Цена: пропускаем половину сделок, и 48% пропущенных были прибыльными (лучшие
пропущенные +81%, +65%, +51%).

**Не менял:** пороги по обороту и возрасту — они конфликтуют с отбором лидеров, и
на одном рыночном режиме этого решать нельзя. Логику выхода — сетка из 54
вариантов вчера показала, что текущая лучшая.

**Rollback:** `MILD_DIP_MIN_LIQUIDITY_USD=5000`.
...
Разбор монеты `EvCDdrbv…`-класса, о которой пришёл вопрос «зачем нам пороги, если
мы их не соблюдаем». Пороги как раз соблюдались — бот отказал по ним **десять
раз**, пока монета умирала:

| время | vol5m | liq | mcap | вердикт |
|---|---|---|---|---|
| 18:24 | $4 415 | $19 107 | $77 387 | `structural_fail` |
| 18:51 | $1 275 | $14 905 | $48 696 | `structural_fail` |
| 19:03 | $1 260 | $14 282 | $45 018 | `structural_fail` |
| 19:09 | $923 | $13 057 | $37 427 | `structural_fail` |
| 19:10 | **$181** | $13 145 | $37 932 | `structural_fail` |
| 19:15 | — | — | — | **`wait_dip_start`** — место припарковано |
| 19:22 | $1 850 | **$2 484** | **$2 620** | **КУПЛЕНО** |

Место паркуется с метриками момента парковки (`watch.metrics`) и стреляет через
минуты, а `attemptMildDipEntry` структурные флоры заново не проверяет. Ликвидность
$2 484 и mcap $2 620 против флоров $5 000 — то есть вдвое ниже.

Замер: за 4 часа **6 из 156** исполненных покупок нарушили флор, и **все 6** прошли
через этот путь (6 из 89 wait-dip = 7%). Нарушалась всегда ликвидность, в четырёх
случаях ещё и mcap.

**Починка.** Перед выстрелом припаркованного места структура перечитывается, и при
непрохождении флоров место снимается с записью `mild_dip_wait_dip_refloor_skip`
(включая то, какими метрики были при парковке — чтобы цена этого гейта осталась
измеримой). Кандидат теперь собирается на свежем снимке, а не на снимке парковки.
...
По факту срабатывало на 3% выходов — потому что большинство сумок не живут 30
минут, — но на той же разобранной монете именно `dust_close` и закрыл позицию на
−13.88%.

**Починка.** Пылью считается только **остаток**: требуется `scaleOutDone` или
`mfeBankStage ≥ 1`. Правило писалось под огрызки от банков и отскоков, и теперь
...
`LEADER_GREEN_MIN_PC5M` был `-2`, то есть всё ниже −2% отбрасывалось и ветка
проливов оставалась без негативов. Лидеры торгуют обе ветки, и у ветки проливов
та же нехватка.

**Почему это важно, в числах.** Проверил формулу turn/dump, которую вывел
предыдущий анализ, на 18 515 покупках лидеров с рыночным снимком:
...
Совпадение (recall) — **80.7%**, а не 97%. Но решает не оно. Применил ту же
формулу к нашему собственному потоку кандидатов за 6 часов:

| | |
|---|---|
| оценённых моментов с известными turn и dump | 992 |
| формула сказала «покупай» | **335** (34%) |
| из них лидер купил монету в пределах 60с | **23** |
| **точность** | **6.9%** |

То есть формула — **фильтр формы пролива, а не селектор монеты**. 93% её сигналов
лидеры не берут. Это независимо совпадает с тем, что уже записано в
`leader-green-entry-formula.md` про зелёную ветку: recall ≥80% да, precision нет,
лучшее ~28%.

Найти селектор можно только по негативам: проливы, прошедшие фильтр, которые лидер
**не** взял. Их и начинаем писать. Порог убран (`-100`), объём строк +45%.

### Проверено и НЕ изменено: лестница выхода

Запрос «сделать выход нормальнее» проверен сеткой из 54 вариантов на реальных
лентах марок, 195 сумок чистого окна. Сравнение честное — текущий конфиг
симулирован на тех же лентах, чтобы потери на исполнении не искажали выбор.
Ленты с невозможными принтами (>+300% или <−99%) отброшены.

**Все 54 варианта хуже текущего по сумме.** Лучший по медиане —
`bank1 +8%×0.4, bank2 +10%×0.35, sleeve −12%` — даёт медиану 8.10 против 7.88, но
сумму −326 против **−219**, то есть на 107 пунктов хуже. И это не шум:

| | лучший вариант | текущий | разница |
|---|---|---|---|
| первая половина окна | +141.0 | +208.2 | **−67.2** |
| вторая половина окна | −467.2 | −427.6 | **−39.6** |

Хуже в обеих половинах.

Причина арифметическая: раннер за вторым уровнем едет на sleeve −12% от пика, а
у большинства сумок пик всего +8…+12%. Сумка с пиком +10% на sleeve выходит около
−3%, тогда как банк на +8% забирает +8%. Редкие движения на +30% не окупают этой
платы на частом случае.

Вывод: **дефицит не в лестнице.** Он в шести сумках из 139 с разрывом цены (95%
...
### Fix: нельзя продать то, что только что купил

Третий и последний член того же семейства. 1.11.829 починил чтение баланса
**после** продажи, 1.11.831 — **перед** продажей. Это чтение сразу **после
покупки**: нода ещё отвечает состоянием до покупки, а исполнитель принимал этот
ноль за пустой кошелёк и отказывался выходить.

Живой разбор `k6BE8rsFShzuQ4t2Q5cfpjCdQFFerfqex8Me7Wupump`:

```
17:56:46 SELL frac=0.4 mfe_bank_1 pnl=+14.36 mfe=14.36 hold=25s err=no_token_balance
17:56:48 SELL frac=0.4 mfe_bank_1 pnl= +0.88 mfe=14.36 hold=27s err=no_token_balance
17:56:50 SELL frac=0.4 mfe_bank_1 pnl= +0.88 mfe=14.36 hold=29s err=no_token_balance
17:56:52 SELL frac=0.4 mfe_bank_1 pnl= +0.88 mfe=14.36 hold=31s err=no_token_balance
17:56:57 SELL frac=0.4 mfe_bank_1 pnl= -2.83 mfe=14.36 hold=33s ok
```

Движок решил банковать на **+14.36%**, получил `no_token_balance` четыре раза за
11 секунд и исполнился на **−2.83%**. Следующий круг на той же монете: решение на
**+20.35%**, реализовано **−11.37%**. И раньше в тот же день: решение на
**+67.72%**, реализовано **+3.54%**.

**Починка.** Нулевое чтение баланса — не доказательство пустоты. До 3
перечитываний по 350мс, и только на отказном пути, то есть максимум ~1с вместо
11-секундного шторма. Ограничение продажи размером on-chain баланса сохранено.

### Разбор: лидеры не «сидят», и на этой же монете они теряли

Утверждение «лидеры купили и сидят» на их же примере не подтверждается. Их ноги
по `k6BE8rs` (размеры теперь читаются после 1.11.834):

| лидер | купил | продал | держал |
|---|---|---|---|
| 8zkgFG | $10.40 | $28.45 | 758с |
| 7BNaxx | $12.28 | $7.25 | 2543с |
| 7BNaxx | $10.46 | — | 912с |
| 8zkgFG | **$430.41** | **$336.13** | **301с** |
| 8zkgFG | **$318.88** | **$316.06** | **457с** |

Они круглят так же, как мы, держат 300–900 секунд и на этой самой монете взяли
**−$94** на одном круге и ноль на другом. Разница не в терпении, а в **размере**:
их клип $300–430 против нашего $5, то есть их удачный круг это +$18, а наш +$0.70.

Мы на этой монете сделали 8 кругов с результатами +9.12%, −19.1%, +14.33%,
+4.43%, +2.35%, −2.83%, −7.93% — по форме то же самое, что у них.

**Что это значит.** Проблема не в том, что мы «выходим слишком рано, а они сидят».
Проблема в том, что мы **конвертируем крупную бумажную прибыль в мелкую или
отрицательную реализованную**: MFE +14% → −2.83%, MFE +20% → −11.37%, MFE +68% →
+3.54%. Часть этого — данный баг, часть — задержка между маркой и филлом на
быстрых движениях, и вторая часть остаётся открытой.
...
### Разбор: мы не слепые — мы видим и отказываемся

Вопрос «почему лидеры покупают массу монет, которых мы не покупаем, и не пора ли
поднять несколько ботов на разные участки вселенной» проверен по данным. Из **235
монет**, которые лидеры купили за окно, покрытое нашим журналом (12ч):

| | монет | доля |
|---|---|---|
| мы **видели** | **221** | **0.94** |
| из них попытались купить | 155 | 0.70 |
| видели, но не пытались | 66 | 0.30 |
| **не видели никогда** | **14** | **0.06** |

И у 11 из 14 непойманных общее свойство: это **не-pump** минты (79% против 6% в
общей популяции покупок лидеров). То есть единственное настоящее «слепое пятно»
— перекос дискавери в сторону pump, и он стоит ~6% покупок лидеров.

**Шардирование вселенной эту задачу не решает.** Мы уже видим 94% того, что
покупают лидеры; узкое место не в том, сколько минтов мы способны просмотреть, а
в том, отвечает ли DexScreener. Это лимит **на IP**, поэтому несколько ботов на
том же хосте сделают хуже — они будут конкурировать за ту же квоту, которая уже
отдаёт 429. Шардирование помогает только при разных IP или платном тарифе.

**Что реально останавливает входы** (по монетам, которые видели и не купили):

| причина | монет |
|---|---|
| `structural_fail` — не прошли **наши пороги** | 63 |
| `structural_fetch_null` — DexScreener не ответил | 53 |
| `no_stream_price` | 17 |
| `no_dip_source` | 16 |

Это две разные вещи, и их надо не путать. `structural_fetch_null` — **27% всех**
fast-path отказов (25 222 из 93 529): данных нет. А `structural_fail` — данные
пришли, но не прошли наши флоры, и вот какие именно режут монеты лидеров:

| связывающий флор | событий |
|---|---|
| возраст пары < 0.5ч | 3 895 |
| vol5m < $300 | 3 416 (+1 226 вместе с возрастом) |
| возраст пары > 72ч | 2 271 (+1 226) |
| liq / mcap | ~1 500 |

Значения на отвергнутых монетах лидеров: `vol5m` p10 **$30**, p25 **$120**;
`ageH` p10 **0.11ч**, p25 **0.36ч**, p75 **73.5ч**. То есть лидеры торгуют по обе
стороны нашей полосы: и моложе 30 минут, и старше 72 часов, и тише $300 за 5м.

Смягчение флоров — это **изменение стратегии**, а не устранение слепоты, и по
нормативу оно требует проверки на нашем собственном P&L (сегодняшний пример:
срезы pc5m ниже −45% и turn выше 3.0 у нас убыточны, хотя лидеры их берут). Так
что здесь только измерение, без изменения порогов.

### Fix: устаревший структурный снимок живёт 30с → 120с

`structural_fetch_null` — потеря чисто инженерная, стратегического суждения не
требует. Снимок считался свежим 8 секунд, а откат на устаревший — 30 секунд, при
том что поля, которые он питает (ликвидность, mcap, возраст пары, объём за 5м), на
такой шкале почти не меняются, а возраст пары не меняется вовсе.

`MILD_DIP_FAST_PATH_STRUCTURAL_STALE_MS=120000`. По умолчанию в коде остаётся
30 000, то есть правка включается только конфигурацией; схема ограничена 600 000.

**Ограничение, за которым надо следить:** снимок несёт и цену, поэтому переиспользование
устаревшего означает, что проверка полосы может пройти по цене возрастом до 120с.
Предпокупочный путь делает свежий запрос к Dex, price-ring питается стримом, и
теперь на каждом структурном отказе журналируется `structAgeMs` — если входы по
более старым снимкам окажутся хуже, это тот knob, который надо вернуть.

**Rollback:** `MILD_DIP_FAST_PATH_STRUCTURAL_STALE_MS=30000`.

## [1.11.836] — 2026-08-11

**Тег:** `sa-1.11.836`

### Feat: второй обзервер под зелёную ветку — сэмплер вселенной с негативами

Зелёные входы это ~32% покупок лидеров, и существующий обзервер объяснить их не
может **по устройству**: он видит монету только **после** покупки, поэтому в
зелёном корпусе каждая строка — положительный пример. Прежний разбор
(`leader-green-entry-formula.md`) честно на это и уперся: recall ≥80% на обоих
кошельках, а precision **~28%**. Без сопоставимых негативов ничего не отличит
«зелёную монету, которую лидер взял» от «зелёной монеты, которую он пропустил».

Новый процесс `mild-dip-leader-green` сэмплирует **вселенную** по такту и пишет
каждого кандидата независимо от того, купил его лидер или нет. Метки процесс
**не** пишет: `leader_buy_observed` с `blockTime` уже есть в основном обзервере,
поэтому положительный пример — это офлайн-join (монета снята в T, лидер купил в
пределах +N минут), а все прочие монеты того же цикла — негативы **на той же
метке времени**. Без RPC сэмплер не может отстать от такта.

**Откуда данные, и почему не всё из DexScreener.** Квота DexScreener исчерпана:
одиночный проверочный запрос с этого хоста получает **429 три раза подряд**, при
том что бот через свой шлюз держит здоровые 10 марок/мин на позицию. Каденс марок
— это то, на чём работает трейл выхода (1.11.827), так что третьего тяжёлого
потребителя там нет.

Поэтому разделено по скорости изменения полей:

| поле | источник | почему |
|---|---|---|
| цена | **Jupiter price v3** | отдельная квота, 40 id за запрос, замерено 40 цен за 0.09с; цена и есть суть зелёного сигнала |
| liq / mcap / age / vol5m | **DexScreener, по норме** | меняются медленно: обновление раз в `STRUCT_TTL_SEC=600`, потолок **3 запроса/мин** против ~30 у бота |

Jupiter без ключа — free tier **1 RPS**, поэтому батчи разгоняются на ~1.1с:
320 монет это 8 батчей ≈ 9с внутри 20-секундного такта.

**Что пишется в строку.** Помимо структурного снимка — моментум по нашему
собственному такту: `ret30s`, `ret1m`, `ret3m`, `ret5m`, `buyShare5m`,
`turnover5mLiq`, возраст структурного снимка. Это и есть та мелкая лента, которой
не хватало прежнему корпусу с одним снимком на покупку.

Вселенная: `leader-seed` (монеты, которые лидеры трогали — **контроль**: они их
знают и в этот момент не купили) плюс boosts/profiles на своём медленном таймере.
Дискавери при отказе повторяется через 45с, а не сжигает пятиминутный таймер —
иначе вселенная остаётся только из seed (замерено).

Замер на живом за ~3 минуты: 514 строк, 82 монеты, все четыре окна моментума
заполнены, происхождение 409 seed / 105 discovery, структура известна для 131 из
131, DexScreener 3 запроса/мин.

Выход: `data/milddip/leader-green-<YYYYMMDD>.jsonl`.

**Rollback:** `pm2 delete mild-dip-leader-green`; на бота не влияет.

## [1.11.835] — 2026-08-11

**Тег:** `sa-1.11.835`

### Fix: обзервер выдумывал цену филла на ногах без котировочного плеча

Второй дефект той же цепочки. 1.11.834 научил читать нативный SOL, после чего
нечитаемых ног стало 0%. Но осталась категория, где котировочного плеча **нет
вовсе**: обмен токен-на-токен. SOL и USDC при этом не двигаются, и парсер ловил
только шум ренты, а затем делил его на реальное количество токенов.

Замер на 4968 ногах, у которых были обе цены:

| dexPrice / quoteFillPrice | доля |
|---|---|
| в пределах 20% от 1.0 | **1%** |
| расхождение более 3x | **97%** |
| расхождение более 10x | 87% |

медиана расхождения — **43x**. При этом только **3%** отношений попадают на
степень десяти, то есть это **не** ошибка в decimals. Худшие случаи: $0.20
шума SOL против ~9.2 млн токенов — выдуманная цена филла в **2822 раза** ниже
рыночной.

**Починка.** Если котировочная дельта меньше `QUOTE_PLAUSIBLE_MIN_RATIO = 0.2`
от подразумеваемого DEX-номинала, котировочная база отбрасывается: нога
считается по DEX-оценке и помечается `sizeUsdEstimated = True`. Такая нога
непригодна, а не «неточна», и агрегаты должны её отбрасывать, а не усреднять.

Проверка на числах инцидента: $0.20 против 9.2 млн токенов при DEX 6.271e-05 →
`source=dex`, `estimated=True`, $576.87 (верный порядок) вместо цены в 2822 раза
ниже. Настоящая нога на $577 остаётся `source=quote`. Нога в нативном SOL без
DEX-цены ($230.37) остаётся `source=quote`.

### Состояние вопроса о выходах лидеров

После 1.11.834 и этой правки **проценты по цене** всё ещё нельзя считать по
ленте марок: `markPriceUsd` берётся из `dexPriceUsd`, а он расходится с
котировочной ценой филла в медиане на 43x. Пока цена марки и цена филла не
приведены к одной шкале, просадку и ширину трейла лидеров не измерить.

Что **уже** можно считать и что подтвердилось: размер клипа. На исправленном
парсере (по 90 входов каждого класса, восстановлено через RPC) медиана клипа
**$267** на knife-именах против **$450** на обычных, отношение **0.61**. То есть
лидеры действительно сокращают ставку на ножевых именах — вывод 1.11.830 в силе,
хотя прежние абсолютные цифры ($0.92–4.32 против $10–27) были артефактом
USDC-подвыборки.

**Rollback:** revert коммита; `QUOTE_PLAUSIBLE_MIN_RATIO = 0` отключает проверку.

## [1.11.834] — 2026-08-11

**Тег:** `sa-1.11.834`

### Fix: обзервер лидеров не видел выручку в нативном SOL

`quote_leg_deltas` читал только **SPL**-балансы лидера (WSOL / USDC / USDT).
Но свопы с `wrapAndUnwrapSol` получают WSOL на временный аккаунт и закрывают его
в той же транзакции — выручка попадает в **нативный** баланс, а закрытый аккаунт
в `postTokenBalances` не появляется вовсе. Итог: `quoteSolDelta = 0`,
`quoteUsdcDelta = 0`, `sizeUsd = None`.

Проверено на цепочке. Четыре продажи, записанные как «нулевые»:

| подпись | нативная дельта | что записали | что было |
|---|---|---|---|
| `p9xPapBw6x…` | +1.151855 SOL | `sizeUsd=None` | **$230.37** |
| `5yjuQGLdBz…` | +1.297813 SOL | `sizeUsd=None` | **$259.56** |
| `NnvLYFm2Ym…` | +4.631150 SOL | `sizeUsd=None` | **$926.23** |
| `3wnCKifHXM…` | +0.967667 SOL | `sizeUsd=None` | **$193.53** |

Масштаб искажения: **43% ног продажи** (16 457) читались как нулевые, и любая
сессия с такой ногой считалась как −100% — отсюда «74% лузеров ровно по −100%».
Плюс 14 730 ног, где котировочная цена филла расходилась с ценой DEX более чем
втрое (на `Gepjas79V` — в 46–57 раз).

**Починка.** SOL считается как `нативная дельта + SPL WSOL`. Это верно для любой
формы маршрута: wrap переводит native → WSOL (в сумме ~0), unwrap переводит
WSOL → native, выплата напрямую в native видна только в нативной ноге. Пошлина
прибавляется назад, когда лидер платил её сам (он fee payer), — она не часть
свопа. Остаточный шум — рента ATA (~0.00204 SOL на создание/закрытие), против
филлов в $200–900 незначим; обе составляющие пишутся в журнал
(`quoteSolNativeDelta`, `quoteSolSplDelta`, `quoteSolNativeReadable`).

Проверка: 6 из 6 «нулевых» ног восстановились как `source=quote`,
`estimated=False`.

### Следствие: под сомнением и вывод о размере клипа лидеров

Утверждение из 1.11.830 — «на knife-именах лидеры ставят $0.92–4.32, на
конвикшн-именах $10–27» — измерялось на **читаемой** подвыборке, то есть на
USDC-ногах. Ноги в SOL, которые и составляют основную часть, были невидимы, а
они на порядок-два крупнее. Обоснование ножевого клипа надо перепроверить на
исправленных данных; сам порог при этом остаётся оправдан отдельным замером —
срезы pc5m ниже −45% и turn выше 3.0 убыточны **на наших собственных сделках**,
а это измерение от обзервера не зависит.

**Rollback:** revert коммита; поведение вернётся к SPL-только чтению.

## [1.11.833] — 2026-08-11

**Тег:** `sa-1.11.833`

### Ops: приоритетная пошлина 50 000 → 20 000 ламportов

Нога стоила **0.000055 SOL** = $0.011 при $200/SOL, и это раскладывается так:

| часть | ламportов | $ |
|---|---|---|
| базовая пошлина Solana | 5 000 | 0.001 |
| приоритетная (кап `PRIORITY_MAX_SOL=0.00005`) | 50 000 | 0.010 |

То есть мы **упирались в кап на каждой ноге**. Теперь кап 20 000 → 25 000
итого, около **$0.005** за ногу.

Целевые $0.001 — не настройка, а ровно базовая пошлина: достичь их значит не
платить за приоритет вообще. На проливающейся монете это обменивает газ на
попадание в блок, то есть работает против 1.11.829/831, которые как раз
добивались того, чтобы выходы исполнялись.

Запас измерен, а не предположен: на 4357 живых ногах отказов класса «включение
в блок» (`confirm_timeout` / `blockhash` / send 429) было **30 = 0.69%**, до
обрыва далеко. За этой долей надо следить: если поползёт — это тот самый knob,
который надо вернуть.

### Замечание: данные обзервера лидеров непригодны для выводов о выходах

Вопрос «сколько просадки терпят лидеры и какой у них трейл» ответить **нельзя**,
и вот почему:

- **43% ног продажи** лидеров: токены уходят (`tokenDelta < 0`), а поступления
  не читаются — `quoteSolDelta` и `quoteUsdcDelta` оба нули (16 457 ног).
  Сессия с такой ногой считается как −100%.
- **14 730 ног**, где котировочная цена филла и цена DEX для того же токена в
  тот же момент расходятся более чем в 3 раза; на `Gepjas79V` — в **46–57 раз**.

Отсюда абсурд в агрегатах: медианный MFE 4398%, p75 ROI +2059%, 74% «лузеров»
ровно по −100%, capture ratio 414%.

**Отзываю** прежнее утверждение (1.11.830) о том, что лидеры «получают скам в
10.7% сессий против наших 5.3%»: оно построено на `pnlPctApprox`, то есть на том
же сломанном поле.

**Остаётся в силе** то, что не зависит от цены филла:

- **размеры клипа** — это `quoteUsdDelta`, прямая сумма в USD: на knife-именах
  лидеры ставят $0.92–4.32, на конвикшн-именах $10–27. Обоснование ножевого
  клипа 1.11.830 стоит на этой части.
- **входные снимки** (`pc5m`, `turn`, класс входа) — берутся из DEX на входе:
  глубокие проливы у лидеров это 5.4% входов.

**Rollback:** `LIVE_JUPITER_PRIORITY_MAX_SOL=0.00005`.

## [1.11.832] — 2026-08-11

**Тег:** `sa-1.11.832`

### Выход по затуханию объёма остаётся выключенным — и вот почему

`never_arm_vol_fade` выключен с 1.11.755 (вместе с `stale`, `dead`, `freefall`
и max-hold). Вопрос «а не пора ли включить» проверен на 3034 эпизодах, и ответ —
**нет**. Медиана изменения P&L от выхода по затуханию **отрицательна во всех
испробованных вариантах** (от −0.02 до −2.56 pct-of-clip), доля ухудшений
47–76%. Знак суммы — шум: то же правило даёт +32.9 при min-hold 15 минут и
−101.9 при 30 минутах.

Живой контрпример на той самой монете, из-за которой возник вопрос:
`HBeaQ6Pn5` уже один раз затухала на **−11.28%** и реализовала **+20.23%**
через 95 минут.

Почему мёртвая сумка вообще не выходит: `never_arm_time_red` требует
одновременно pnl ≤ −15% **и** pc5m ≤ −5%. У затухшей монеты pc5m ≈ 0, поэтому
второе условие невыполнимо. `HBeaQ6Pn5` стоит на −14.15%, то есть не проходит
и первое. Расширять это правило данные не разрешают — см. выше.

### Feat: закрытие крошек (`dust_close`)

Что действительно расточительно — не время удержания, а остатки. Ладдеры
`mfe_bank` и `never_arm_bounce` оставляют огрызки на $1–2, и они не бесплатны:

| | |
|---|---|
| открытых сумок ≤$2 | 8 из 15 (книга $11.80 из $45.30) |
| держались | 9–23 часа |
| марок за 6ч на них | **22 407 из 51 655 = 43%** |
| шесть крупнейших потребителей марок | по ~3 540 марок на сумку в $1–2 |

То есть половина книги по количеству съедала почти половину пропускной
способности DexScreener — того самого ресурса, под который делался батчинг
(1.11.820) и ускорение марок под трейл (1.11.827).

При этом на такой сумке исход не имеет значения: ±1.3% от $1.20 — это ±$0.02.
Газ на закрытие — **$0.011**, около 1% крошки.

`MILD_DIP_EXIT_DUST_CLOSE_USD=2`, `MIN_HOLD_MS=1800000`. Порог ниже остатка от
40%-го банка при клипе $5 ($3), поэтому живые раннеры не затрагиваются; 30 минут
заметно больше медианы удержания (12.5 мин). Правило применяется **после**
гейтов, поэтому учёт пика / arm / trough / vol-fade сохраняется, и оно никогда
не перебивает выход, который гейты уже выбрали.

**Rollback:** `MILD_DIP_EXIT_DUST_CLOSE_USD=0`.

## [1.11.831] — 2026-08-11

**Тег:** `sa-1.11.831`

### Fix: тот же устаревший баланс, но на предпродажном чтении

1.11.829 починил чтение **после** продажи. Живая проверка сразу показала, что
у чтения **перед** продажей та же болезнь. `J7o48eA9q` после выката:

```
+19.7s SELL ok=true  frac=0.4 mfe_bank_1
+21.2s SETTLE keep_runner src=stale_read before=18258326467 sold=7303330586
                                              rem=10954995881   ← верно
+24.9s SELL ok=false frac=1 mfe_bank_2 err=Custom:6024
+29.3s SELL ok=false frac=1 mfe_bank_2 err=Custom:6024
+33.1s SELL ok=false frac=1 mfe_bank_2 err=Custom:6024
+35.8s SELL ok=true  frac=1 mfe_bank_2 pnl=25.57
```

Расчёт остатка теперь верный (`10 954 995 881`), но `mfe_bank_2` спрашивает
баланс у ноды, получает ещё **до** bank_1 (`~18.2e9`), просит продать столько —
и получает InsufficientFunds. Три сожжённые ноги за 11 секунд, пока нода
догоняет. Выход исполнился на +25.57%, но так везёт не всегда: в том же окне
второй эпизод прошёл путь +10.14% → −6.03%.

**Починка.** Исполнитель продаёт `min(tokenRawBase, on-chain ATA)`, поэтому
осевший `tokenRaw` (`before − sold`) — безопасный потолок, и он передаётся как
`tokenRawBase`. Квота с покупки таким потолком быть не может: Jupiter `outAmount`
на покупке на несколько процентов выше фактического филла — именно с этого и
начались 6024. Поэтому флаг `tokenRawSettled` на позиции: потолок применяется
только после осевшей ноги, сбрасывается при scale-in (сумка выросла) и при
записи из «сырого» чтения баланса.

Замер после 1.11.829 (12 минут): доля исполненных ног продажи **0.26 → 0.44**,
попыток на монету было до 74, стало 1–2 у шести монет из восьми. Эта правка
снимает остаток.
...
### Feat: риск скама оценивается размером, а не запретом

Разбор `8UT4zBa3TsCbZafz6gNVacxtTENyYqeTkgJ5D6K8pump`: вход 3.03e-05, пик
+25.5%, выход −94.9% через 36 секунд, клип **$10**, минус $5.69 кэшем.

**Наши гейты скам не видят вообще.** 774 закрытых позиции, 41 обвал ≤ −70%.
Ликвидность, mcap и отношение liq/mcap у скамов и у остальных статистически
одинаковы:

| | скамы | остальные |
|---|---|---|
| медиана liq | $19.4k | $19.5k |
| медиана mcap | $67.8k | $66.3k |
| liq/mcap | 0.27 | 0.29 |

**Лидеры такие монеты покупают — чаще нас.** 10.7% их закрытых сессий
заканчиваются ≤ −70% против наших 5.3%; у них есть отдельный класс
`rug_knife` (57 сессий) и `deep_knife` (203). Разница — **размер**: на входах
с pc5m < −35% и turn > 0.8 их клип $0.92–$4.32, на конвикшн-именах $10–$27.
Мы ставили одинаково: средний клип $6.10 на скаме и $6.32 на всём остальном.

**Пороги измерены, а не назначены.** Реализованный результат по срезам:

| срез | n | mean | winrate |
|---|---|---|---|
| pc5m −60…−45 | 19 | **−34.7** | **0.11** |
| pc5m ниже −60 | 14 | −13.4 | 0.36 |
| pc5m −45…−35 | 12 | +3.7 | 0.33 |
| pc5m −35…−25 | 15 | +1.9 | 0.53 |
| turn ≥ 6 | 9 | **−28.4** | 0.33 |
| turn 3…6 | 32 | **−14.6** | 0.34 |
| turn 1.5…3 | 63 | −1.3 | 0.44 |

Первая версия этого правила ставила порог на −35% и turn 1.5 — и была
**неверной**: под нож попадали 126 позиций, из которых 47% доходили до +15%,
то есть лучше обычного тира (35.7%). Ущерб живёт ниже −45% и выше 3.0.

`MILD_DIP_RUG_KNIFE_CLIP_USD=2`, `DUMP_PCT=-45`, `TURN=3`. По выборке это
+470 единиц pct-of-clip, и вся окрестность порога положительна (turn 3 → +472,
turn 4 → +505, turn 6 → +405) — результат не держится на одной настройке.

Важно: базовый клип с 1.11.828 равен **$5**, а перепись считалась на эпохе
$10. Значит срез даёт 2.5x, а не 5x — направление то же, величина меньше.

**Запрет входа оставлен выключенным** (`RUG_BLOCK_DUMP_PCT=0`): входы ниже
−85% — это 6 позиций, **ноль** скамов и 4 дошли до +15%. Самые глубокие проливы
не скамы, а самые сильные отскоки.

Журнал: `rugTier` / `rugReasons` / `rugTurn` в `entrySnapshot`,
`mild_dip_rug_block_skip` при включённом запрете.

### Внимание: наш собственный P&L был смещён

Кэш-P&L считается только по ногам, которые исполнились, а они систематически
прибыльные:

| ноги продажи | n | mean | медиана |
|---|---|---|---|
| исполнились | 5823 | **+2.54%** | +0.68% |
| не исполнились | 10389 | **−9.56%** | −18.22% |

Поэтому `trades.jsonl` за 40ч показывал **+$1031**, а кошелёк — **−$92.91**.
Убыточные выходы не исполнялись (причина — 1.11.828) и в учёт не попадали.
После 1.11.828 отчётный P&L **упадёт** — это не регресс, а появление в отчёте
тех сделок, которые всегда были убыточными.

**Rollback:** `MILD_DIP_RUG_KNIFE_CLIP_USD=0` возвращает плоский клип.

## [1.11.829] — 2026-08-11

**Тег:** `sa-1.11.829`

### Fix: продажа исполнялась, но позиция оставалась «открытой» на фантомной сумке

Корень: после успешной продажи остаток читался с RPC **сразу** после отправки,
и это чтение возвращало баланс **до** продажи. Дальше `settleAfterSuccessfulSell`
видел `remainder_above_dust`, держал раннер, и движок выхода продолжал стрелять
по сумке, которой в кошельке уже нет.

`6tfuqq`, куплено `217 727 192 747`:

| нога | продано | чтение после | должно быть |
|---|---|---|---|
| `mfe_bank_1` frac 0.4 | 40% | `215 941 362 360` | `130 636 315 649` |
| `never_arm_bounce` frac 1 | остаток | `129 564 817 416` | `0` |

Оба раза чтение вернуло ровно то, что мы только что продали.

Следствия за 8ч, 53 монеты, 543 попытки продажи — успешны **143 (26%)**:

- `Custom:6024` (InsufficientFunds) — **230**: продаём то, чего нет
- `no_token_balance` — **230**: та же фантомная сумка
- до **74 попыток** на одной монете, `hard_stop` 95 отказов против 9 исполнений
- фантомная позиция держала слот в открытой книге до первого «пустого» ответа —
  отсюда и «почему в книге так мало монет»

**Починка.** Исполнитель отдаёт `tokenRawBefore` / `tokenRawSold` — то, от чего
он считал, и то, что реально отправил. `resolveSellRemainder` берёт
арифметический остаток `before − sold` за истину и **не принимает** чтение выше
него: такое чтение означает только то, что нода нашу продажу ещё не видела.
При расхождении — до 3 перечитываний по 450мс, затем арифметика.

Плюс относительный порог пыли: остаток ≤ 2% от предпродажного баланса — это
`flat`, а не раннер. Абсолютный `HOLDING_DUST_RAW = 1000` для 6-decimal сапплая
в ~2e11 raw — это 5e-9 сумки, любая крошка читалась как раннер.

Гарантия против оригинального инцидента (`CkTFDN`, орфан −80%) сохранена: ниже
арифметического остатка чтению доверяем, то есть сумку никогда не бросаем
раньше, чем она действительно продана.

Журнал: `remainderSource` (`observed` / `stale_read` / `expected_only`) в
`mild_dip_sell_settle` и `mild_dip_cooldown_set`.
...
### Ops: mild-dip clip flat $5

Base / thick / micro → `$5` (was `$10`).
...
### Feat: частые марки по открытым позициям + пробные покупки на блоках

**Марки.** `MARK_DEX_REFRESH_MS 8000 → 2000`, `MARK_CACHE_TTL_MS 20000 → 3000`.

Трейл не мог работать на прежней частоте. `6tfuqq`: пик +22.95%, sleeve стоит
на −12% от пика, то есть выход должен был быть около +8% — вышли на **−23.6%
от пика**. Весь путь туда и обратно уложился между двумя годными марками.

CF на 91 сделке подтвердил следствие: **любое** расширение выхода проигрывает
текущему, потому что отдаёт ещё больше:

| конфигурация | P&L |
|---|---|
| банк +6/+8, sleeve −12 (сейчас) | **−$7.34** |
| банк +6/+15 | −$8.74 |
| раннер 30% + sleeve −18 | −$13.46 |
| только банк +6, трейл −20 | −$19.82 |

При этом медиана MFE **19.5%**, p75 — **46.75%**. Движения есть, забрать их
нечем. Сначала частота марок, потом ладдер.

Стало возможно только после 1.11.820: весь открытый портфель — один батч на
30 монет, то есть ~19 позиций при 2с это ~30 запросов/мин из 120.

**Пробные покупки.** `rebuy_liq_drop` (1385 за 3ч) и `rebuy_below_exit`
(762 за 3ч) — два главных блокиратора перезахода, и оценить их нечем: после
отказа мы перестаём вести ленту по монете, форвардных данных нет.

Вместо синтетической инструментации — **живая покупка на $2**: настоящий филл,
настоящее проскальзывание.

- `MILD_DIP_PROBE_BLOCKED=1`, `USD=2`, `MAX_PER_HOUR=6` — риск $12/час
- Размер жёстко ограничен probe-клипом, обычный клип не применяется
- В журнале `mild_dip_probe_override` и поле `probe` на филле — в статистику
  книги они не попадают
- Когда часовой бюджет исчерпан, правило блокирует как раньше

**Откат:** `MILD_DIP_PROBE_BLOCKED=0`; марки — `REFRESH_MS=8000`, `TTL=20000`.
...
### Change: сид лидеров задаёт порядок обхода (бюджет не тронут)

Замер за 12ч: лидеры открыли **220** монет, мы оценили **203 (92%)**, купили
**71**. Но по **53** из них в окне ±15 минут вокруг их покупки у нас **не было
ни одного события** — монета в universe есть, просто в ту минуту мы смотрели
на другие имена. При universe ~3 800 и обходе по 12 за проход монета попадает
под проверку раз в несколько минут, а пролив живёт секунды.

Это проблема **порядка**, а не покрытия и не пропускной способности.

`MILD_DIP_DISCOVER_SOURCES: 'stream,boosts,profiles' → '…,leaders'`.

Механизм уже был в `collectCandidateMints`: источник `leaders` кладёт монеты
из `leader-seed.json` (250 имён, окно 2ч) в начало списка кандидатов сразу
после приоритетов кулдауна. **`MILD_DIP_ENRICH_MAX` остаётся 12** — те же
двенадцать слотов за проход, но первыми в них попадают имена с чужим сигналом,
а не случайные из трёх тысяч.

Ни одного дополнительного запроса и ни одного лишнего слота: меняется только
порядок. Именно поэтому это не повторение прошлых поломок на enrich — там
ломалось от увеличения бюджета за Dex-гейтом.

Проверка гипотезы: доля «тихих» монет лидеров должна упасть, а число покупок
на их именах вырасти при том же числе запросов в минуту.

**Откат:** убрать `,leaders` из `MILD_DIP_DISCOVER_SOURCES`.
...
### Ops: observer обратно, но батчевый; профиль выходов лидеров

Observer выключался в 1.11.820, потому что жёг квоту DexScreener по запросу на
монету. После батчинга (1.11.819) проход стоит **~2 запроса вместо 60**, так
что его можно вернуть — а он единственный источник, способный ответить, **как
лидеры выходят**.

- `autostart: true`
- `LEADER_OBSERVER_DENSE_ONLY_TD: 0 → 1` — плотная 1Гц лента только по
  turn→dump мешкам: это та линия, которую копируем, и так проход остаётся
  маленьким и не может снова задушить вход

**Новый инструмент** `scripts/milddip/leader-exit-profile.py [часов]` считает
ровно три вещи по их закрытым TD-сделкам:

1. **какую просадку терпят** — `maePct` по перцентилям, отдельно у выигравших
   и проигравших сделок
2. **где фиксируют** — `pnlPctApprox` по перцентилям, медиана MFE и какую долю
   MFE они реально забирают (`capture_of_mfe_pct`)
3. **какой у них трейл** — `givebackPctAtExit`, то есть сколько отдают от пика,
   плюс `bouncePctAtExit` для выходов в отскок

Плюс по плотной ленте: как глубоко TD-мешок уходил в минус и какая доля тех,
кто проваливался ниже −10%, потом делала +5%.

Считаются только сессии с `cashPnlReliable` и `pathReliable` (1.11.811). До
этих флагов observer оценивал непрочитанные продажи по протухшей Dex-цене и
выдавал медианы вида +900%.

**Откат:** `autostart: false` у observer.
...
### Fix: марк, снятый ДО покупки, рисовал фантомный MFE и выносил позицию

Точная хронология `6tfuqq` (продажа `21iiB4Y5…`):

| +сек | событие |
|---|---|
| −31.3 | марк `px=0.0001596` — **до** нашей покупки |
| −28.1 | **покупка по 0.00012981** |
| −27.3 | марк снова `px=0.0001596` → `pnl=+22.95%`, `newPeak=true`, arm, `mfe_bank_1` |
| −21.2 | первый настоящий марк `px=0.0001219` → реально **−6.09%** |
| −13.6 | bank1 всё-таки прошёл |
| +0.5 | bank2 добил остаток, итог **−0.26%** |

Никакого роста на +23% не было. Кольцо цен держало доиповую цену, снятую за
3 секунды **до** нашего филла, и первый же марк принял её за движение в плюс:
армирование, лестница банков, полный выход по цене входа.

Правка: сэмпл из кольца старше `openedAtMs` больше не является марком для этой
позиции. Пока не пришёл марк, снятый после покупки, позиция просто не
оценивается — вместо того чтобы оцениваться по чужой цене.

**Откат:** `git checkout sa-1.11.821 -- src/milddip/loop.ts`.
...
### Fix: банк срабатывал до расчёта покупки; ослаблен liq-порог перезахода

Разбор `21iiB4Y5…` (`6tfuqq`) объясняет «купили и сразу продали в ноль»:

| +сек от входа | что |
|---|---|
| 0.0 | покупка |
| 2.0 | армирование, MFE +8.17% → `mfe_bank_1` |
| 2.2 … 10.2 | **пять отказов `no_token_balance`** — баланс SPL ещё не виден |
| 16.5 | bank1 наконец прошёл, +9.82% |
| 20.8 … 28.8 | **три отказа `Custom 6024`** |
| 31.7 | bank2 прошёл, +5.39% — мы полностью вышли |
| 48 … 135 | цена идёт **+6.9% → +19% → +26% → +31.8%** |

То есть вышли не по стратегии, а по расчётной гонке: банк выстрелил через
2 секунды после покупки, когда продать физически нечего.

Насколько это системно (12ч): **1089 неудачных ног против 400 успешных**,
из них `Custom 6024` — 584, `no_token_balance` — 429. Bank1 срабатывает
раньше 10 секунд после входа у **19%** позиций.

- `MILD_DIP_EXIT_MFE_BANK_MIN_HOLD_MS=20000` — банки не трогают позицию первые
  20 секунд. Стопы (`hard_stop`, `cliff_dump`) не затронуты: они должны
  срабатывать всегда.

### Порог перезахода по ликвидности

`MILD_DIP_REBUY_LIQ_DROP_MIN_DROP_PCT: 0 → 25`. Любое падение ликвидности
против нашего выхода закрывало монету на 6 часов: **17 004** блокировки на
56 монетах за 12ч, медиана падения **22.9%** — это обычный шум pump-имён.
Самый частый блокиратор перезаходов и главная причина, почему мы не
пересекаемся с лидерами на повторных входах.

**Откат:** `MFE_BANK_MIN_HOLD_MS=0`, `REBUY_LIQ_DROP_MIN_DROP_PCT=0`.
...
### Perf: observer шлёт в DexScreener пачками (1 запрос вместо 30)

Уточнение по бюджету: **$500 — это Helius** (RPC/WS), и он тут ни при чём.
429 приходят от **DexScreener**, за который мы не платим вообще: публичный
лимит ~300 запросов в минуту.

Куда он уходил:

- observer делал **один HTTP-запрос на монету** — при 60 открытых мешках это
  60 запросов на проход
- бот отдельно дросселирует себя гейтом на **120 RPM** (файл
  `data/dexscreener-api-gate.json`)
- 1.11.811 добавил observer'у отбой на 429 — 429 ушли, но покрытие Dex упало
  до **9.4%** (64 снапшота из 683 покупок лидеров)

DexScreener принимает **до 30 адресов через запятую** в одном запросе —
проверено на живом API: 2 монеты → 200 OK, 30 пар, 2 разных base-mint.

- `fetch_dex_batch()` — чанки по 30, один запрос на чанк
- Кэш на монету (`LEADER_OBSERVER_DEX_CACHE_MS`, 20с): лидеры многократно
  доливаются в одни и те же имена
- Проходы по открытым мешкам и dense-тики прогревают кэш одним батчем
- Интервал между запросами 1200мс → **400мс**, отбой 60с → 30с: при 30-кратном
  сокращении числа запросов такая осторожность больше не нужна

Ожидаемо: ~60 запросов на проход → **2**, покрытие Dex обратно к 80–90%.

**Откат:** `git checkout sa-1.11.818 -- scripts/milddip/leader-observer.py`.
...
### Revert: leader-seen гейт душит вход (покупок 0)

Замер сразу после старта 1.11.816/817: за **3.7 минуты** гейт отклонил
**394** раза (**212** уникальных монет), прошло **22**, покупок — **0**.
Размер сида — **44** монеты.

Ошибка рассуждения на моей стороне: разбивка «+$9.82 против −$7.46» считалась
на сделках, **которые мы уже совершили**. Там leader-seen был почти бесплатным
признаком: наши покупки и так тяготели к именам лидеров. Поставить тот же
признак **в начало воронки** — совсем другое: он режет 90% кандидатов ещё до
Dex, и до формулы пролива доживают единицы.

Главный факт, который гейт вскрыл: **наш universe и сид лидеров пересекаются
примерно на 10%** (22 из 212 за окно). Это диагноз discovery, а не готовый
фильтр.

- `MILD_DIP_REQUIRE_LEADER_SEEN: 1 → 0`
- Код гейта и увеличенный сид (250) остаются — сид полезен сам по себе

**Откат:** `MILD_DIP_REQUIRE_LEADER_SEEN=1`.
...
### Hotfix: бот в краш-лупе после 1.11.816 (`leaderSeedMax` > 80)

1.11.816 поднял `MILD_DIP_LEADER_SEED_MAX` до `250`, но zod-схема ограничивала
поле восемьюдесятью — осталось от времён, когда сид был просто подсказкой для
пробуждения, а не входным гейтом. Итог: `mild-dip-bot` падал на старте с
`config invalid: leaderSeedMax: Number must be less than or equal to 80`,
107 рестартов, журнал не писался.

- Потолок схемы `80 → 500`
- Тест фиксирует границу, чтобы env и схема больше не разъезжались

**Откат:** `MILD_DIP_LEADER_SEED_MAX=80`.
...
### Feat: торгуем только то, что трогал лидер

Разбивка тех же 183 сделок по признаку «касался ли монеты лидер»:

| группа | n | P&L | winrate |
|---|---|---|---|
| принт лидера **был известен нам до нашей покупки** | 115 | **+$9.82** | 0.63 |
| лидер купил **после** нас | 11 | −$0.63 | 0.55 |
| лидер **никогда** не трогал монету | 56 | **−$7.46** | 0.52 |

Это причинный фильтр, а не подгонка задним числом: в 115 случаях из 183 запись
лидера уже лежала в нашем логе к моменту нашей покупки, медианный возраст
сигнала **29 минут** (p25 9.7м, p75 72м). Окно 2ч покрывает распределение.

- `MILD_DIP_REQUIRE_LEADER_SEEN=1`, окно `7200000` мс
- Гейт стоит **до** Dex-запроса, поэтому заодно экономит rate-бюджет
- `LEADER_SEED_MAX` 40 → **250** у бота и observer'а: лидеры открывают ~36
  мешков в час, 40 слотов покрывали меньше часа из двухчасового окна и
  вытесняли монеты, которые мы как раз дозревали купить
- Пропуски видны как `mild_dip_not_leader_seen_skip`

Ожидаемо срезает ~30% оборота (183 → 115 сделок).

**Откат:** `MILD_DIP_REQUIRE_LEADER_SEEN=0`.
...
9.5ч на новом входе (184 покупки, $432, филл медиана −18.7%) — выборка выросла
с 49 до 183 сделок, и она **переворачивает** вывод 1.11.810.

Ключевой факт: медианный MAE у сделок, которые в итоге **выигрывают**, равен
**−19.3%**, и **56%** из них сначала проваливаются ниже −15%. То есть стоп −15
срабатывал не на лузерах, а на будущих победителях. В live-логе это видно
прямо: `hard_stop` — 70 ног и **−$28.01**, а всё остальное вместе **+$21.8**.

Сетка на полной выборке:

| конфиг | P&L | ex_top3 | winrate |
|---|---|---|---|
| sleeve8 hard15 (1.11.810) | **−$9.52** | −14.81 | 0.52 |
| sleeve12 hard20 | −$0.17 | −11.36 | 0.58 |
| **sleeve12 hard25** | **+$4.15** | −7.50 | 0.59 |
| sleeve12 hard30 | +$6.19 | −5.46 | 0.60 |

Возвращаем `HARD_STOP=25` и `SLEEVE=12`. Лестницу банков (+6 / +8) оставляем —
на полной выборке она по-прежнему в лучшей группе.

### Отсечка по возрасту пары

Пары старше **72ч** — мёртвые деньги: **20** сделок, **−$7.60**, winrate
**0.25**, медиана MFE **1.1%**. Они просто не отскакивают.

`MILD_DIP_MAX_PAIR_AGE_HOURS: 0 → 72`. Это единственный фильтр, который
делает **`ex_top3` положительным** (−$7.50 → **+$0.09**): результат переживает
удаление трёх лучших сделок, а не держится на них.
...
### Fix: observer выжигал Dex-бюджет бота и портил ленту выходов

Запрос «подсмотреть выходную логику лидеров» упёрся в то, что данных для этого
нет. Замер за 4 часа:

| | |
|---|---|
| их новых покупок | 344 |
| из них **без Dex-снапшота (HTTP 429)** | **274 (80%)** |
| sell-ног без читаемого quote | **2634** из 4681 |
| сессий с абсурдным MFE (>300%) | **291** из 368 |
| **пригодных для анализа сессий** | **5** |

Три причины, все наши:

1. **Observer ходил в DexScreener в обход гейта бота.** Свой rate-limit
   (`DEX_MIN_GAP_MS=1200`) + отбой на 60с после 429. Это же должно снять часть
   `structural_fetch_null` у бота — он делил тот же лимит.
2. **Один битый Jupiter-принт travil ленту.** `plausible_mark()` отбрасывает
   марк, отличающийся от входа/пика более чем в 20×. Именно так появлялся
   `mfePct=4184` и пик в 42× от входа.
3. **Непрочитанная sell-нога писалась как proceeds 0**, то есть фейковый −100%.
   Теперь `proceedsMissingLegs` + `cashPnlReliable` + `pathReliable`.

Пример из присланной транзакции `F51tc9Zk…` (8zkg, `7rMnp9EJ`, $27.75, холд
768с): `dex: HTTP 429` → class `unknown`, gates пустые; выход без цены →
`cashPnlUsd=-27.75`; `mfePct=4184`. Все три дефекта в одной сделке.

**Откат:** `git checkout sa-1.11.810 -- scripts/milddip/leader-observer.py`.
...
### Change: выход под новый профиль входа — банк раньше, стоп ближе

Первое окно на глубоком входе (3.8ч, 49 сделок, филлы медиана **−20.9%** от
сигнала, холд медиана **567с** вместо 3670с) даёт однозначный ответ, и он
**противоположен** гипотезе «отпустить правый хвост как у лидеров».

Свободная сетка (162 правила): наверху `tp8 / sl15`, внизу `trail35`, `time20m`.
После −20% входа отскок — возврат к среднему, а не тренд. Медиана MFE 14.9%,
медиана realized 3.4% — хвоста, за который мы держали, у нас нет.

Ближайшая выразимая движком конфигурация (769 правил, все топ-10 с hard 15):

| | было | стало |
|---|---|---|
| bank1 | +8% × 0.4 | **+6% × 0.4** |
| bank2 | +15% × 0.4 | **+8% × 0.6** (выход полностью) |
| sleeve | −12% | **−8%** |
| hard stop | −25% | **−15%** |

На том окне: **−$3.97 → −$0.48**, winrate **0.37 → 0.55**.

Оговорка: 49 сделок, и `ex_top3` у всех правил остаётся отрицательным. Это
направление по лучшим доступным данным, а не доказанный эдж — откат в одну
строку.

**Откат:** вернуть `BANK1_PCT=8`, `BANK2_PCT=15`, `BANK2_FRACTION=0.4`,
`SLEEVE=12`, `HARD_STOP=25`.
...
### Ops: логировать дно запаркованного seat'а при истечении

`mild_dip_wait_dip_expire` писал только последнюю цену, поэтому две
противоположные проблемы выглядели одинаково:

- монета **не дошла** до цели → цель слишком глубокая, надо поднимать `PCT`
- монета **дошла**, но мы не успели купить → проблема в скорости филла

Теперь в событии есть `troughPriceUsd` и `troughDumpFromSignalPct`.

**Откат:** `git checkout sa-1.11.808 -- src/milddip/loop.ts`.
...
### Fix: окно филла wait-dip было 2%, поэтому seat'ы только истекали

Живые 30 минут: **37** запарковано, **39** истекло, филлов ≈ **0**.
Отказы по потолку кучкуются на **−8.63%** при требуемых −10%.

Арифметика: при `PCT=-12` / `OVERSHOOT=2` ready срабатывает на `0.88×signal`,
а потолок стоит на `0.90×signal`. Между ними **2.3%** — любой обычный откуп
после дна съедает окно, и seat умирает по таймауту.

Меняем не строгость, а асимметрию: **просим глубже, принимаем выше**.

- `WAIT_DIP_PCT` −12 → **−15** (ready на `0.85×signal`)
- `MAX_OVERSHOOT_PCT` 2 → **5** (потолок остаётся `0.90×signal`)
- окно филла становится **~5.9%** вместо 2.3%
- выше −10% от сигнала по-прежнему не платим никогда
- `MAX_CHASE_PCT` 5 → **8** — потолок и есть связывающее ограничение, chase
  теперь ловит только откровенные рипы

Плюс троттлинг `mild_dip_wait_dip_ready` (15s на монету): один seat писал
**363** одинаковые строки перед истечением и засорял журнал, по которому мы
же потом считаем качество входов.

**Откат:** `PCT=-12` + `OVERSHOOT=2` + `CHASE=5`, либо `MILD_DIP_WAIT_DIP=0`.
...
### Change: wait-dip chase cap 3% → 5%

После 1.11.805 покупки пошли (`7SNuFk` — фил **−22%** от сигнала), но узким
местом стал догон: за 20 минут **35** отказов `wait_dip_chase_ready>3%` при
**4** покупках.

Распределение отказов: медиана **3.73%**, p25 **3.16%** — это обычный откуп
после −12% ожидания, а не погоня. Хвост совсем другой: p75 **36.97%**,
max **59.64%**.

- `MAX_CHASE_PCT` 3 → **5**: проходит 22/35, при 15 было бы 23/35 — то есть
  всё, что выше 5%, это 37–60% рипы, которые догонять нельзя
- Абсолютная цена по-прежнему ограничена потолком wait-dip (signal × 0.90)

**Откат:** `MILD_DIP_WAIT_DIP_MAX_CHASE_PCT=3`.
...
### Fix: wait-dip fill fallback — ring тоже пустой

1.11.804 добавил фолбэк на `price-ring`, но покупок так и не появилось:
за 4 минуты снова **16** `wait_dip_prebuy_missing_price` и **0** buy.
Причина: у запаркованных монет кольца часто нет вовсе — ready-чек считает
дамп от сигнала по watch-записи, а не по ring.

Цепочка теперь: **fresh Dex → ring → candidate mark** (та цена, которую
ready-чек только что валидировал). Защита от дрейфа не теряется — потолок
wait-dip (`signal × (1 + (pct + overshoot)/100)`) по-прежнему режет фил выше
цели, тест на это добавлен.

**Откат:** `git checkout sa-1.11.803 -- src/milddip/entry-attempt.ts` + reload,
или `MILD_DIP_WAIT_DIP=0`.
...
All mild-dip buy clips → **$5**:
- `MILD_DIP_POSITION_USD=5`
- `MILD_DIP_THICK_POSITION_USD=5`
- `MILD_DIP_MICRO_POSITION_USD=5`
- `MILD_DIP_LEADER_ALIGN_SCALE_IN_USD=5` (was $15)

**Откат:** restore prior 30/30/30 + scale-in 15 + reload.
...
Leader evidence backing `patience=0` (`diag-leader-exit-policy.json`, 1506 8zkg
sessions / 80h): of his never-worked sessions only **5%** close within 5m and
19% within 10m — our 5m/−6% knife was far tighter than the behaviour it copied.

`GIVEBACK_PCT=8` is **not** backed by a counterfactual yet (open A/B).
...
### Change: copy-trader-8zkg — hold cap 30 минут

На `FxQf…` / `copy-trader-8zkg` раньше не было тайм-аута по удержанию
(`EXIT_MODE=mirror`). Добавлен `COPY_TRADER_MIRROR_HOLD_CAP_MS=1800000`: полный
market-sell после **30 мин**, даже если лидер ещё держит. Уже открытые ноги
старше 30м закроются на ближайшем тике после деплоя.

Mirror (vol) без этого капа.

**Откат:** `MIRROR_HOLD_CAP_MS=0` + reload, или `git revert`.
...
### Change: 8zkg — жёсткий premium 5%, ретрай до выхода лидера

Откат логики first-shot 10%/8s и terminal-skip на premium из 1.11.634.

Политика входа теперь:
- **не покупаем дороже лидера больше чем на 5%** (snapshot + Jupiter quote);
- если котировка выше — **не отменяем**, а ждём и пробуем снова каждые 6 с;
- стоп ретраев: лидер начал выходить (`leaderHoldingsShrunkSinceSignal`) или
  окно 2h (`BUY_RETRY_WINDOW_MS=7200000`) как потолок;
- first-shot / grace выключены (`0`).

Оба лейна: `BUY_PRICE_MAX_PREMIUM_PCT=5`, `QUOTE_PREMIUM_GUARD_PCT=5`.

**Откат:** вернуть 6/10/8s + terminal, или `git revert`.
...
### Fix: 8zkg — троттлинг провайдера больше не отбрасывает продажу

Наблюдение после выката `1.11.632`: за 28 минут на trail-лейне **6** продаж
завершились `qn_rate:Too Many Requests` и были классифицированы как
**терминальные** — `isSellRetryableError` их не покрывал, `pendingSell`
удалялся, выход оставался неисполненным до следующего перезаряда политики.

429 — самая транзиентная из возможных ошибок: провайдер говорит «позже», а не
«никогда». Добавлено в ретраибельные: `qn_rate:*`, любые `too many requests` и
`rate limit`. Такая ошибка **не** считается unroutable, поэтому получает
щедрый бюджет `COPY_TRADER_SELL_MAX_ATTEMPTS` (`40`) с backoff, а не жёсткий
лимит для «нет маршрута».

**Тесты:** 3 кейса в `tests/copytrader/sell-storm-guards.test.ts` (228 зелёных
по `tests/copytrader`).

**Откат:** `git revert` коммита + reload.
...
### Fix: 8zkg — шторм продаж на фантомной позиции (`tokenRaw=1`)

Живое окно 22.9h на trail-лейне: **1436** неудачных `copy_sell` против **108**
успешных, из них **1283** — `jupiter_sell_quote_failed`. Два минта дали 1299 из
1436: `FVZhiS1u` (972 неудачи) и `6N46H85p` (327). Медиана — попытка через
**24 минуты** после последней успешной продажи этого минта, максимум — 68 минут.
На mirror-лейне того же шторма нет (медиана хвоста 15 секунд), потому что там
продажи запускает только лидер, а не самоперезаряжающийся тик трейла.

**Корневая причина — «живой» считалась позиция с одной сырой единицей токена.**
В `state.json` лежал `6N46H85p` с `tokenRaw: "1"` (1e-6 токена) и `sizeUsd:
$118.09`. Все проверки на пустоту в коде были вида `bal === 0n` / `!== 0n`,
поэтому остаток в одну единицу держал позицию открытой вечно. При этом
`syncPositionFromWallet` переписывал `sizeUsd` только при `notional > 0`, а
`walletNotionalUsdFromRaw` округляет до центов — значит на сметённой позиции
notional давал `0`, присваивание не выполнялось и **`sizeUsd` навсегда застревал
на последнем реальном значении**. Политика трейла видела «позицию на $118»,
ставила настоящий выход, Jupiter отказывался котировать одну сырую единицу, и
`processPendingSells` ретраил это каждые ~3.5 с в течение часа
(`SELL_RETRY_WINDOW_MS=3600000`).

Каждая такая попытка — это `fetchDexInfo` + resolve цены + RPC-баланс + котировка
Jupiter + `sleep(150)` внутри общего цикла продаж. То есть застрявший минт жёг
~1500 котировок в сутки из общей квоты (в журнале есть `qn_rate: Too Many
Requests`) и задерживал продажи, которые реально исполнились бы.

Сопутствующий дефект, найденный там же: расчёт объёма шёл **до** сверки с
кошельком. `sellableRaw` брался из состояния, затем вызывался
`syncPositionFromWallet`, и продавалась уже устаревшая величина. Из-за этого
`FVZhiS1u` в 00:01:47 и 00:01:54 дважды продал «по $58.205» — вторая нога
сайзилась от той же несинхронизированной позиции и вычерпала остаток.

**Что сделано:**

1. **`copyPositionIsDust`** (`position-reconcile.ts`) — общий порог пыли вместо
 сравнения с нулём: `COPY_TRADER_DUST_MIN_TOKEN_RAW` (по умолчанию `1000`) как
 абсолютный пол в сырых единицах и `COPY_TRADER_DUST_MIN_USD` (по умолчанию `1`)
 как порог по notional. Применён в `refreshPositionFromWallet`,
 `reconcileGhostPositions` и `processPendingSells`. При недоступной цене
 USD-порог не применяется — обрыв прайс-фида не закрывает живую позицию.
2. **`syncPositionFromWallet`** переписывает `sizeUsd` при любой известной цене,
 включая нулевой notional. Устаревший размер больше не залипает.
3. **Порядок в `processPendingSells`**: сверка с кошельком выполняется **до**
 расчёта `sellableRaw` и объёма продажи.
4. **Предохранитель по попыткам**: `COPY_TRADER_SELL_MAX_ATTEMPTS` (`40`) и
 отдельный, более жёсткий `COPY_TRADER_SELL_MAX_UNROUTABLE_ATTEMPTS` (`8`) для
 ошибок «нет маршрута» (`isUnroutableSellError`). Слиппедж (`Custom:6024`,
 `0x1771`) ретраится щедро — там движение цены реально помогает; отсутствие
 маршрута не самоизлечивается.
5. **Экспоненциальный backoff** `nextSellRetryDelayMs` с потолком
 `COPY_TRADER_SELL_RETRY_BACKOFF_MAX_MS` (`60000`) вместо плоских 3.5 с.
6. **Кулдаун после отказа**: при исчерпании попыток пишется `sell_abandoned`,
 уходит алерт в Telegram и на позицию ставится `sellBlockedUntilTs` на
 `COPY_TRADER_SELL_ABANDON_COOLDOWN_MS` (`600000`), чтобы тик трейла не
 перезаряжал тот же обречённый выход.
7. **Дедуп одновременных выходов** в `scheduleTrailExitSell`: на минт держится
 одна продажа в полёте; полный выход (`fraction=1`) вытесняет частичную ступень,
 чтобы риск-менеджмент не застревал за ней.

Застрявшая позиция `6N46H85p` расчищается сама при рестарте: стартовый
`reconcileGhostPositions` теперь признаёт `tokenRaw=1` пылью и закрывает её с
`reason: wallet_balance_dust`.

Пороги — дефолты конфига, то есть действуют на все copy-trader лейны; в
`ecosystem.config.cjs` ничего не переопределяется.

**Тесты:** `tests/copytrader/sell-storm-guards.test.ts` (21 кейс) — регрессия на
`tokenRaw=1`, залипание `sizeUsd`, классификация unroutable/slippage, бюджеты
попыток, backoff.

**Откат:** `git revert` коммита релиза + reload обоих лейнов. Точечно: выключить
предохранители через `COPY_TRADER_SELL_MAX_ATTEMPTS=0`,
`COPY_TRADER_SELL_MAX_UNROUTABLE_ATTEMPTS=0`, `COPY_TRADER_DUST_MIN_USD=0`,
`COPY_TRADER_DUST_MIN_TOKEN_RAW=0`, `COPY_TRADER_SELL_RETRY_BACKOFF_MAX_MS=0` —
поведение вернётся к прежнему без отката кода.
...
### Fix + Change: 8zkg — post-quote premium guard, stop cutting the leader's tail

Живое окно 2026-08-02 14:18 → 08-03 12:36 UTC, 362 закрытые сессии лидера.
Лидер за окно: среднее **+5.41%**, медиана **+2.13%** на сессию; топ-5 сессий из
362 дают **40%** всей его суммарной доходности. Мы за то же окно: trail
**−$1076** кэша на $5900 оборота, mirror **−$632** на $6000. Разбор — три
утечки, все три закрываются здесь.

**1. Fix (баг): кап премии входа не применялся к реальному свапу.**
`COPY_TRADER_BUY_PRICE_MAX_PREMIUM_PCT` проверялся в `evaluate.ts` против
snapshot-цены DEX, а `live-exec.ts → executeLiveCopyBuy` брал котировку Jupiter
и сразу отправлял транзакцию без повторной сверки. Фактические заливы: медиана
+2.14%, среднее +3.93%, **p90 +10.26%** при кэпе 3%. Два самых дорогих случая
окна — целиком отсюда: `GfyVVfTS` вход **+22.8%** над лидером (лидер закрыл
сессию +20.7%, у нас kill −60.5%), `4G2sQnfg` вход **+23.8%** (лидер +24.6%,
наша позиция не продалась и умерла как `wallet_balance_zero`, −$100).

Новая чистая функция `checkQuotePremium()` в `src/copytrader/evaluate.ts` +
проверка исполняемой котировки перед `liveBuildUnsignedSwapTx`. Блокировка
пишет событие `buy_quote_premium_blocked` и возвращает `quote_premium_too_high`;
pending-buy остаётся в окне ретраев, а не отбрасывается. Новый ключ
`COPY_TRADER_QUOTE_PREMIUM_GUARD_PCT` (**0 = выключено** по умолчанию), на обоих
8zkg-лейнах — `6`. Свободнее snapshot-кэпа намеренно: котировка уже несёт
route impact, задача гарда — срезать разгон, а не переигрывать правило входа.

**2. Change: гейты перестают резать хвост.** Из 6 сессий ≥+100% мы взяли **0**,
из топ-5 — **0**, из 48 сессий ≥+30% — 5. По взятым лидер сделал +3.03%, по
пропущенным +5.85%.

| Ключ | Было | Стало | Основание (живое окно) |
|---|---|---|---|
| `ENTRY_MIN_VOL_TO_MCAP_1H` | `0.20` | `0` (выкл) | участвовал в отказе по 88 сессиям, среднее **+7.8%**, из них 11 ≥+30% и 2 ≥+100%; в одиночку отрезал 5 сессий со средним −5.4% |
| `ENTRY_MIN_PAIR_AGE_HOURS` | `0.3` | `0.1` | в одиночку отрезал 46 сессий, среднее **+3.11%**, медиана +2.75% — это не защита |
| `ENTRY_MIN_TURNOVER_5M` | `0.09` | `0.06` | единственный гейт с отрицательной медианой отрезанного (**−3.03%**) — оставлен, но на 0.09 он участвовал в отказе по 13 сессиям ≥+30% |

`MIN_LEADER_PRIOR_SESSIONS` / `MIN_LEADER_PRIOR_AVG_PCT` остаются выключенными:
за окно они успели съесть 93 сессии со средним **+10%**.

**3. Change: выход перестаёт срезать остаток хвоста (только trail-лейн).**
На сессиях, где лидер сделал ≥+15%, он в среднем **+40.4%**, мы **+1.2%**.
Лестница +8%×0.5 сливала половину позиции до того, как движение развернётся:
`2B5hFQNw` — лидер +72.5%, мы вышли целиком на +18.2%.

| Ключ | Было | Стало |
|---|---|---|
| `TRAIL_TP_STEP_PCT` | `8` | `25` |
| `TRAIL_TP_SELL_FRACTION` | `0.5` | `0.25` |
| `TRAIL_ARM_PCT` | `8` | `15` |
| `TRAIL_GIVEBACK_PCT` | `8` | `18` |

`TRAIL_KILL_PCT=50`, `TRAIL_TIME_CAP_MS=1800000` и `TRAIL_TRAIL_SELL_FRACTION=0.2`
без изменений.

**4. Change: mirror перестаёт ждать на выходе.** `SELL_DELAY_MAX_MS: 2000 → 0`.
Медианный лаг от продажи лидера до нашей — 7с, цена выхода на **−4.25%** хуже
его. Детекция (poll 3с) и round trip свопа структурны, случайный джиттер 0–2с —
нет. `POLL_INTERVAL_MS` не трогаем: квота Alchemy.

На сделках, где лидер сделал меньше +15%, мы шли с ним вровень (−5.9% против
−5.6%) — весь ущерб был в хвосте, и все четыре пункта бьют именно туда.

**Файлы:** `src/copytrader/{evaluate,config,executor,live-exec,main}.ts`,
`tests/copytrader/quote-premium-guard.test.ts`, `ecosystem.config.cjs`.

**Откат:** `COPY_TRADER_QUOTE_PREMIUM_GUARD_PCT: '0'` отключает гард без ревёрта
кода. Гейты: вернуть `VOL_TO_MCAP_1H='0.20'`, `PAIR_AGE_HOURS='0.3'`,
`TURNOVER_5M='0.09'`. Выход: вернуть `TP_STEP=8`, `TP_SELL_FRACTION=0.5`,
`ARM=8`, `GIVEBACK=8`. Mirror: `SELL_DELAY_MAX_MS='2000'`. Затем
`pm2 reload ecosystem.config.cjs --update-env`.
...
Жёсткий полный выход на +25% обрезал раннеры: свечка +200% превращалась в +25%.
У живого Оскара другая схема — лестница частичных TP и трейл по остатку.

**Как у `half8_runner` на `copy-trader-8zkg`**

- каждые **+8%** от входа → продажа **50% остатка** (+8 / +16 / +24 …);
- трейл вооружается на **+8%**; каждый откат **−8% от пика** → продажа **20% остатка**;
- новый хай сбрасывает ступени трейла;
- kill **−50%**; тайм-кап **выключен** (раннер едет под трейлом);
- устаревший `TRAIL_TAKE_PROFIT_PCT=25` → **0**.

Зеркало продаж лидера по-прежнему только у `copy-trader-8zkg-mirror`.

**Откат:** вернуть `TAKE_PROFIT=25`, `TP_STEP=0`, `TRAIL_SELL=1`, `KILL=0`,
`TIME_CAP=3600000` и reload.

---

## [1.11.623] — 2026-08-02

### Fix: у `trail_runner` появился жёсткий TP, arm снова +8%

При arm +20% рост на +18% трейл не включал — позиция сидела до часового капа и
могла уехать в глубокий минус. Без зеркала лидера такой порог бессмысленен.

**Изменено** (`copy-trader-8zkg`)

- `COPY_TRADER_TRAIL_ARM_PCT` `20` → `8` — пик +18% уже под защитой giveback.
- `COPY_TRADER_TRAIL_TAKE_PROFIT_PCT=25` — жёсткий полный выход при +25% от входа.
- giveback 8%, cap 60m без изменений.

**Добавлено**

- `trailTakeProfitPct` / `COPY_TRADER_TRAIL_TAKE_PROFIT_PCT` (по умолчанию `0` —
  остальные линии не затронуты).

**Откат:** вернуть `ARM=20`, `TAKE_PROFIT=0` и reload `copy-trader-8zkg`.
...
Гейт «лидер раньше терял на этой монете» — это память о конкретном тикере, а не
причина. Перемер это подтвердил: ранговая корреляция `priorAvgPct` с нашим результатом
**+0.048 / +0.005** по половинам окна, то есть ноль. Хуже: у `priorSessions` знак
**противоположен** гейту — корзина «0–3 прошлых круга» на отложенной половине лучшая
(+1.53%), а «29+ кругов» худшая (−4.83%). Гейт `sessions >= 3` отсекал именно лучшее.

**Что было не так с прежними замерами.** Цена входа бралась из снапшота по одному
правилу, а траектория выхода — по другому. Любой признак, посчитанный от того же
снапшота (в первую очередь `chase_5m`), частично измерял собственную цену входа.
Пересобрано: вход и выход читаются из одной поминутной траектории, вход — первая точка
после покупки лидера (это же учитывает наш лаг ~30 с), доходности винзоризованы
−95…+200%, три временных фолда на 3 624 сессиях.

**Главный факт.** Слепое копирование этого лидера даёт **−0.89% на сделку** против его
собственных **+2.08%**. Лаг входа (~1.2%) плюс круг издержек 2.5% съедают всё его
преимущество. Отбор должен окупить сначала их.

**Что реально работает** — одно и то же в разных видах: насколько пул торгуется
относительно своего размера. `vol5m/ликвидность` (+0.084/+0.067), `vol1h/mcap`
(+0.066/+0.069), возраст пары (−0.080/−0.066). Абсолютные капитализация и ликвидность
меняют знак между половинами — «слишком мелкая» или «слишком неликвидная» полосы не
существует, существует «её никто не торгует».

| Правило | n | Фолд 1 | Фолд 2 | Фолд 3 |
| --- | --- | --- | --- | --- |
| без гейтов | 3 624 | −1.49% | −0.44% | −0.74% |
| прежние гейты по истории | 1 308 | −2.31% | +1.00% | +1.18% |
| возраст ≤30ч и оборот ≥0.09 | 1 440 | +1.31% | +1.03% | +0.83% |
| …и vol1h/mcap ≥0.33 | 1 083 | +1.95% | +1.19% | +1.26% |

**Добавлено**

- `COPY_TRADER_ENTRY_MIN_TURNOVER_5M` — объём за 5 минут к ликвидности пула.
- `COPY_TRADER_ENTRY_MIN_VOL_TO_MCAP_1H` — объём за час к капитализации.
  Оба «мягко закрыты»: нечитаемый фид — это `unknown`, а не «оборот ноль».

**Изменено** (`copy-trader-8zkg`)

- `COPY_TRADER_MIN_LEADER_PRIOR_SESSIONS` `3` → `0`, `COPY_TRADER_MIN_LEADER_PRIOR_AVG_PCT`
  `0` → `-100` — память по монете выключена.
- `COPY_TRADER_ENTRY_MAX_PAIR_AGE_HOURS` `24` → `30`.
- `COPY_TRADER_ENTRY_MAX_CHASE_5M_PCT` `10` → `0` — сигнал исчез, как только цена входа
  перестала зависеть от того же снапшота.
- Новые пороги: оборот `0.09`, `vol1h/mcap` `0.33`.

**Оговорки, зафиксированные в аудите**

- Заработок — это хвост: медиана +0.42%, верхние 5% сделок дают 293% результата.
- Весь запас в издержках: при круге 2.5% правило даёт +1.37%, при 3.5% — +0.37%,
  при 4% уходит в минус. Сокращение лага входа стоит дороже любого следующего гейта.

Замер: [`docs/strategy/copytrader/LEADER_8ZKG_AUDIT.md`](../copytrader/LEADER_8ZKG_AUDIT.md),
раздел «Market structure, measured properly».

Изменения применены к **обеим** линиям — `copy-trader-8zkg` и близнецу
`copy-trader-8zkg-mirror` (1.11.620): у них общий контроль на входе, и расхождение
превратило бы сравнение выходов в сравнение отбора монет.

**Откат:** вернуть в `ecosystem.config.cjs` прежние значения в обоих блоках
(`PRIOR_SESSIONS=3`, `PRIOR_AVG_PCT=0`, `MAX_PAIR_AGE_HOURS=24`, `MAX_CHASE_5M_PCT=10`,
оба новых порога `0`) и `pm2 reload ecosystem.config.cjs --update-env`. Новые ключи
по умолчанию `0`, поэтому остальные линии не затронуты.
...
### Вторая линия копирования: тот же отбор монет, выход лидера (`copy-trader-8zkg-mirror`)

Спор о выходе решается только измерением на одинаковых входах. Аудит лидера дал **$1411 при
зеркальном выходе против $549 при трейлинге с ограничением по времени** на той же истории входов;
работающая линия при этом идёт с трейлингом. Новая линия — близнец первой: **тот же лидер, тот же
отбор монет, те же $100 на сделку**, а выход исключительно по продаже лидера, в его пропорции, без
трейлинга, без ограничения по времени и без стопа.

Все параметры входа сверены с близнецом до совпадения: ворота лидера включены, минимум 3 прошлые
сессии, средняя прибыль по монете выше 0, возраст пары 1–24 часа, отсечка погони +10% за 5 минут,
задержка входа 5 секунд, проскальзывание 150 б.п., финансирование в USDC. Отличается **только**
`COPY_TRADER_EXIT_MODE`. Любое расхождение на входе превратило бы сравнение выходов в сравнение
чего-то другого.

Изоляция полная: отдельный кошелёк `BLthwsgz…`, отдельный журнал `data/copytrader-8zkg-mirror/`,
отдельное состояние. Общий кошелёк заставил бы две линии драться за одни и те же остатки токенов, и
ни один из результатов не значил бы ничего. Оскар эту линию не усыновляет.

Пока кошелёк не пополнен, гейт финансирования пропускает сигналы **до** запроса котировки — холостых
попыток свопа и мусора в логах не будет.

**Изменения:** `ecosystem.config.cjs` — блок `copy-trader-8zkg-mirror`.

**Откат:** `pm2 delete copy-trader-8zkg-mirror` и удалить блок; первая линия не затрагивается.

---

## [1.11.619] — 2026-08-02

### Tuning: порог опыта лидера +5% → 0 (`copy-trader-8zkg`)

После перехода на зеркальный выход воронка сузилась до **1 одобренной покупки из 45**
за два часа. Из 15 отказов по `leader_prior_avg_pct` почти все — значения **+2.9…+4.8%**,
то есть монеты, на которых лидер исторически **в плюсе**, но не дотягивал до порога +5%.

Порог был откалиброван под выходы лидера. С нашим текущим выходом (arm +20% / giveback 8% /
cap 60 мин / зеркало) корзины перемерены на 3 402 сессиях: убыточна только корзина
`prior avg < −5%` (−1.88% на сделку). Полоса 0…5% зарабатывает столько же, сколько 5%+.
Проверка на отложенной половине окна: порог +5% даёт 520 сделок и +2.97% на сделку,
порог 0 — 667 сделок и **+2.99%** на сделку. Порог не покупал качество, только терял поток.

**Изменено**

- `ecosystem.config.cjs`: `COPY_TRADER_MIN_LEADER_PRIOR_AVG_PCT` `5` → `0` в `copy-trader-8zkg`.
  Блокируем только монеты, на которых лидер реально терял деньги.

Остальные гейты без изменений: `≥3` прошлых круга (корзина «1 круг» — единственная
отрицательная по опыту, −2.12% на сделку), возраст пары 1–24 ч (24–48 ч — +0.16%,
48–72 ч в минусе), анти-догон +10%.

Замер: [`docs/strategy/copytrader/LEADER_8ZKG_AUDIT.md`](../copytrader/LEADER_8ZKG_AUDIT.md),
раздел «Gates under the shipped exit».

**Откат:** вернуть `COPY_TRADER_MIN_LEADER_PRIOR_AVG_PCT: '5'` и
`pm2 reload ecosystem.config.cjs --only copy-trader-8zkg --update-env`. Только конфиг,
код не затронут.
...
Аудит лидера `8zkgFGVZ` за 30 дней (35 163 свопа, 17 581 сессия, 4 776 монет) —
[`docs/strategy/copytrader/LEADER_8ZKG_AUDIT.md`](../copytrader/LEADER_8ZKG_AUDIT.md).
Лидер делает ~586 покупок в день и весь плюс собирает на холдах до 30 минут;
на 1–3 часа у него −$76 244 при win rate 32%. Наш лаг входа 31 с, поэтому
зеркалить его один-в-один — значит получить его хвост убытков без быстрых
выигрышей. Ставка на отказ от худших входов и на собственный выход.

### Added
- `src/copytrader/leader-history.ts` — учёт закрытых кругов лидера по каждой монете
  (сессии, win rate, средний результат, винзоризация +300%/−100%). Сильнейший
  одиночный сигнал: у монет с ≥3 прошлыми сессиями и средним > +5% сумма PnL
  +$28 020, у всех остальных категорий — минус.
- `src/copytrader/entry-gates.ts` — гейты `COPY_TRADER_LEADER_GATES=1`: опыт лидера
  по монете, возраст пары 1 ч…3 сут, buys/sells 5м ≥ 1.05, запрет входа после
  спайка > +15% за 5 минут. Отсутствие данных = отказ (fail-closed). Воронка
  3 582 → 696 входов за 30 дней (~23 в сутки вместо 586).
- `src/copytrader/entry-context.ts` — свой парсинг живого пейлоада DexScreener для
  этих метрик: общий quote-кэш их не переносит (его пишут и легаси-продюсеры).
- `src/copytrader/trail-exit.ts` + режим `COPY_TRADER_EXIT_MODE=trail_runner` —
  трейлинг от пика (взвод +8%, отдача 6%) и жёсткий таймаут 45 мин; продажа
  лидера остаётся страховкой. Медианная монета к +60 мин стоит −1.7% от входа,
  но по пути показывает +14.6% — выход по лидеру отдаёт всю экскурсию. Выходим
  раньше него в 65% сделок. Стоп-лосса нет: на выборке любой SL режет хвост
  сильнее, чем спасает.
- `npm run copy-trader:bootstrap-history` — разовый бэкфилл истории лидера из
  цепочки, иначе гейт по опыту открыл бы первую позицию только через несколько суток.
- PM2 `copy-trader-8zkg`: отдельный кошелёк `FxQfFTmj6xfjbzE2LcXteJMjd1KpBjMhH9nzEiijUGHX`,
  своё состояние в `data/copytrader-8zkg/`, $100 на сделку, без handoff в Оскара.
  Добавлен в `strategy-process-watch` и `sa-collector-health-telegram`.

### Fixed
- `dex-info.ts` подставлял время запроса в `pairCreatedAtMs`, из-за чего гейт
  `COPY_TRADER_MIN_PAIR_AGE_HOURS` видел любую пару как только что созданную.
  Теперь поле `null`, а реальный возраст приходит из `entry-context.ts`.

### Changed
- `DexScreenerPairDetails` дополнен `pairCreatedAtMs` и `priceChangeM5Pct`
  (только опциональные поля, обратная совместимость сохранена).
- `COPY_TRADER_APP_NAME` — имя heartbeat/fatal-файлов, чтобы несколько
  copy-trader-процессов не перетирали ops-файлы друг друга.

### Backtest
$100 на сделку, комиссии 2.5% круг, доходности винзоризованы, пути
провалидированы против фактических цен лидера. Копировать всё и выходить с ним:
+$1.08 на сделку. С гейтами: +$3.90 (win 58.8%, макс. просадка −$202).
С гейтами и трейлингом: +$2.11 (пессимистичный фил) … +$3.35 (оптимистичный).
Out-of-sample на последних 15 днях при подборе на первых 15: +$3.30 … +$4.48,
win 60.5–67.5%.

### Rollback
`pm2 delete copy-trader-8zkg` — остальные процессы не затрагиваются: линия
изолирована кошельком, состоянием и журналом. Полный откат — revert коммита;
существующий `copy-trader` не меняется, все новые гейты выключены по умолчанию
(`COPY_TRADER_LEADER_GATES` default `0`, `exitMode` default `oscar_half8`).
...
### Fix: фантомный up-tick не должен вызывать ложный TP / breakeven-exit (ZEREBRO RCA)

- **RCA `8x5Vqb…So2Wn` (ZEREBRO, live-oscar prod):** позиция закрыта `BREAKEVEN_EXIT` в минус при `peakPnlPct=-10%`, а в журнал попал `pnlPct=+49%` и `effective_exit_price` ×10 к рынку. Причина — устойчивый «фантомный» котиров-всплеск (0.046→~0.12, +155%): пер-тиковый клэмп `clampLiveTrackerMtmForExit` (±12%/тик) **ратчетил** решающую цену вверх ~12%/тик до уровня призрака (т.к. `lastObservedPriceUsd` каждый тик переустанавливается в клэмпнутое значение), что зажигало +8% ступень TP-лестницы и взводило Wave B. `peakMtmUsd` (sell-gated) при этом корректно стоял на месте — рассинхрон источников цены для пика и для триггера TP.
- **Fix (scoped: live-oscar):** апсайд-движение, которое зажигает TP / взводит трейл, теперь должно быть подтверждено исполнимым sell-пробом (`peakMtmUsd > 0`). Без подтверждения `clampLiveTrackerMtmForExit(..., allowUpRatchet=false)` не даёт цене ратчетить **выше** прошлого наблюдённого значения (даунсайд/kill/DCA не тронуты — консервативно для выходов). Прочие стратегии — прежний клэмп.
- Тесты: `papertrader-exit-policy-wave-b` — hold без sell-подтверждения, ратчет с подтверждением, даунсайд не блокируется.
- **Усреднение −20%:** осознанно **не меняем** — для этого mcap-бэнда живого −20% тира нет (потолок съедается сплит-ногами + одним −10% добором); −20% живёт только в `dynamicKillstopShadow` (наблюдаемость).

**Откат:** redeploy `1.11.612` (вернёт безусловный up-ratchet клэмпа).
