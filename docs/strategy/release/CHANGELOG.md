...
### Хронологический разбор входов: хард-стоп −25% → −15%

Разбор построен на 721 нашей позиции за сутки. Для каждой лента цены собрана из
наших марок и продлена данными сэмплера — то есть смотрим хронологически, куда
реально пошла цена после нашей покупки, а не что нам померещилось на выходе.

**Ловим ли мы дно — нет.** На уровне ±5% цена сначала идёт **вниз в 60.3%**
случаев. Медианная просадка до первого +5% равна **−7.55%**.

**Но падение — не приговор.** Из 434 монет, ушедших сначала на −5%:

| дальше по ленте | доля |
|---|---|
| дошли до +5% | 62.0% |
| **дошли до +10%** | **51.6%** |
| дошли до +20% | 38.7% |
| так и не вернулись к нулю | 9.9% |

Чтобы забрать те 51.6%, надо пересидеть медианные **−12.53%**; треть из них
сначала уходит ниже −20%, а ниже −30% — только 17%.

**Лидеры делают ровно это.** 1723 их эпизода, путь считается по их же лентам
марок: сидят через −10% в 49.3% позиций, через −20% в 32.3%, через −30% в 21.1%.
Медиана самой глубокой пересиженной просадки −9.68%, p25 −25.14%. Их вход лучше
нашего только на коротком горизонте (±5%: 54.3% вверх первым против наших
39.4%), а на ±20% и дальше — та же монетка, что и у нас. Дна они не ловят.

**Их тейк-профит — пропорциональный трейл.** Отдача от пика на выходе держится
константой ~30% от самого пика:

| пик до выхода | медиана отдачи | отдача как доля пика |
|---|---|---|
| 15–40% | −7.13 п.п. | −29.4% |
| 40–100% | −16.01 п.п. | −29.9% |
| >100% | −61.50 п.п. | −33.6% |

Поэтому время удержания у них и разное: это не параметр, а следствие трейла.
Медиана 21 мин, p75 80 мин.

**Их формулу к нашим входам применять нельзя.** Чистый трейл 30% от пика на
наших 721 путях даёт среднее +2.30 при медиане −4.38 — хуже действующей
лестницы (+5.28 / +7.78). Причина в том, что наши монеты после +10%
разворачиваются: 72.3% возвращаются к нулю. Проверка бегунка за bank2 это
подтверждает — доля bank2 0.6 (сумка целиком выходит на +8%) даёт +3574, при 0.5
уже +3211, при 0.4 +2849. Гипотеза «сливаем половину и остаёмся с пустой сумкой
на пампе» на наших данных не подтверждается.

**Что действительно стоит денег — уровень хард-стопа.** Живой замер за 6 часов,
235 выходов: `hard_stop` 22 штуки с медианой −27.82%, и один этот бакет
перевешивает всё, что приносят `mfe_bank_1` (+10.38) и `mfe_bank_2` (+8.11).
Перебор уровня на тех же путях с учётом проскальзывания 2.8 п.п.:

| стоп | сумма | среднее | медиана |
|---|---|---|---|
| −12 | +3135 | +4.35 | +0.12 |
| **−15** | **+3842** | **+5.33** | **+7.63** |
| −18 | +3753 | +5.21 | +7.69 |
| −20 | +3579 | +4.96 | +7.69 |
| −25 (было) | +3574 | +4.96 | +7.78 |
| −30 | +3454 | +4.79 | +7.84 |

Это настоящий оптимум, а не край диапазона: −12 обваливается до +3135.
Механика видна в разбивке — победители не меняются (380 × +22.22 против
386 × +22.10), улучшается средний проигравший: −13.50 вместо −14.80.

Выигрыш **+0.37 п.п. на позицию** (+268 на 721 сделке, около +7.5% к сумме).
Скромно, но это единственная правка, которую подтверждают данные; остальные
проверенные варианты лестницы оказались хуже действующей.

**Rollback.** Вернуть `MILD_DIP_EXIT_HARD_STOP_PNL_PCT: '25'` в
`ecosystem.config.cjs` и дефолт `25` в `src/milddip/config.ts`.
