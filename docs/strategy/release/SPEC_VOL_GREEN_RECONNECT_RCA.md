...
## Resolution (green agent, 2026-08-07) — STATUS DONE

Full write-up: **`docs/strategy/volgreen/RCA_RECONNECT_2026-08-07.md`** (product `1.11.722`).

| Deliverable | Result |
|-------------|--------|
| Metrics definition | Heartbeat fields: `ws_close_1006_count`, `ws_reconnect_backoff_count`, `process_start_count`, `enrich_over_budget_count`, `tick_error_count` (+ by_code) |
| RCA verdict | **~79%** opens = process restart; **~21%** = true `1006` reconnect (~4/day) |
| undici | Fallback to `globalThis.fetch`; entry script fails boot if `undici/index.js` missing; prefer full `npm ci` |
| enrich >15s | probeMax ≤24, buyForce take 8, budget 22s |
| Helius plan | Shared key **accepted short-term** (~½ stream credits); dedicated LERA key recommended when billing hurts — not required for WS stability |
