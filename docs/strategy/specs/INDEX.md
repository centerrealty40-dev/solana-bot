...
| W9.1 | [`W9.1_peak_giveback_exit_flow_spec.md`](./W9.1_peak_giveback_exit_flow_spec.md) | **normative** — exit «flow» = **peak-giveback**: arm MFE → full exit на откате от локального пика; **без** time-stop и **без** SL% от entry; убыток-by-flow допустим; для нового / mild-dip бота | mark path, position state, journal |
