...
- **Статус:** shadow ENABLED in 1.11.470, collecting lag (консьюмер подключается с 1.11.471). Флаг `PAPER_LIVE_OSCAR_SHYFT_SHADOW_ENABLED='1'` на проде, креды (`SHYFT_GRPC_TOKEN`/`SHYFT_GRPC_ENDPOINT=https://grpc.fra.shyft.to`) в `.env` на `salpha-v2`. В 1.11.471 исправлен gRPC-консьюмер (CJS/ESM interop резолв класса `Client` + явный `connect()` перед `subscribe()`) — токен принят, стрим подключается. В 1.11.472 зарегистрированы shadow-kind'ы в live-схеме журнала (`live_shyft_shadow_status`/`live_shyft_shadow_price` ранее отбрасывались валидатором). Консьюмер журналирует `live_shyft_shadow_price`; идёт сбор распределения лага PG 24–48 ч перед активацией Stage 1.2. Прочие Shyft-флаги остаются OFF.
...
### 1.2 Stream-цена primary для discovery-eval и MTM + freshness-gate — АКТИВИРОВАН на проде в 1.11.549
- **Статус:** **ACTIVE on prod** (1.11.549). `SHYFT_PRICE_PRIMARY_ENABLED='1'`, `SHYFT_PRICE_PRIMARY_MTM_ENABLED='1'`, `SHYFT_PRICE_PRIMARY_DISCOVERY_ENABLED='1'`, `SHYFT_MAX_STALE_MS=5000`. Shadow Stage 1.1 остаёт `'1'` для журнала сравнения. Код: `resolvePrimaryPriceUsd` + инъекции в MTM (`tracker.ts`) и discovery (`dip-clones.ts`). Наблюдение: `live_shyft_price_primary`.
...
### 1.3 DeFi API Shyft для mcap/liq на кандидатах — АКТИВИРОВАН на проде в 1.11.549
- **Статус:** **ACTIVE on prod** (1.11.549). `SHYFT_DEFI_MCAP_ENABLED='1'`, `SHYFT_DEFI_MCAP_TTL_MS=12000`. Модуль `src/papertrader/stream/shyft-defi-mcap.ts`; override `refMcap` (tier) + `evalRow.market_cap_usd/liquidity_usd`. Ключ `SHYFT_DEFI_API_KEY`/`SHYFT_API_KEY` в `.env`. Наблюдение: `live_shyft_defi_mcap`.
