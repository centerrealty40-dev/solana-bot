...
- **Статус:** shadow ENABLED in 1.11.470, collecting lag — флаг `PAPER_LIVE_OSCAR_SHYFT_SHADOW_ENABLED='1'` на проде, креды (`SHYFT_GRPC_TOKEN`/`SHYFT_GRPC_ENDPOINT`) в `.env` на `salpha-v2`. Консьюмер журналирует `live_shyft_shadow_price`; идёт сбор распределения лага PG 24–48 ч перед активацией Stage 1.2. Прочие Shyft-флаги остаются OFF.
