...
/** Buy-TWAP того же кита по той же монете — закрыть бумажные short (перекрёст, long доминирует). */
export function closePaperForWhaleBuyReversal(
  sig: Pick<NormalizedTwapSignal, 'user' | 'coin' | 'displaySymbol'>,
  cache: HyperliquidMarketCache,
): number {
  const filePath = paperJournalPath();
  ...
  for (const pos of opens.values()) {
    ...
    if (pos.whaleUser.toLowerCase() !== user || pos.coin !== sig.coin) continue;
    const px = exitPxForOpen(pos, cache);
    if (closePaperTrade({ hash: pos.hash, displaySymbol: pos.displaySymbol }, px, 'whale_buy_reversal')) {
      closed += 1;
    }
  }
  return closed;
...
export function handlePaperOnTwapEnd(
  ...
  if (opens.has(sig.hash)) {
    if (!shouldCloseOnWhaleTwapCancel(endedStatus)) {
      console.log(
        `[hl-twap] paper ignore whale TWAP end ${sig.displaySymbol} (${endedStatus}) — timer exit −${twapExitEarlyMinutesForDuration(sig.minutes)}m`,
      );
      return;
    }
