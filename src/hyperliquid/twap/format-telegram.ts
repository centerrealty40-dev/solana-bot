...
import { telegramMessageLink, twapHourlyImpactPct } from './coin-twap-analysis.js';
...
export function buildCrossingNote(opts: TwapCrossingNoteOpts): string | null {
  ...
  const lines = opts.opposingTwaps.map((t) => {
    ...
    const hourly = twapHourlyImpactPct(t);
    const impactStr =
      hourly != null
        ? `${formatPctShare(hourly)}%/h`
        : formatPctShare(t.volumeSharePct);
    return `• ${side} ${impactStr} · до ${end} · <a href="${href}">${label}</a>`;
  });
  const agg = `long ${formatPctShare(opts.plan.buyPctPerHour)}%/h · short ${formatPctShare(opts.plan.sellPctPerHour)}%/h`;
  const delta =
...
export function buildTwapStartMessage(
  ...
  const volLine =
    sig.dayNtlVlmUsd != null
      ? `Объем: ${formatUsdCompact(sig.dayNtlVlmUsd)} (${formatPctShare(sig.volumeSharePct)} дня${hourly != null ? ` · ${formatPctShare(hourly)}%/h` : ''})`
      : `Объем: n/a`;
