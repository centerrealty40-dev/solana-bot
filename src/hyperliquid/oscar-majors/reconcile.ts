...
import type { MajorsTraderState } from './trader.js';
...
export async function reconcileMajorsOpensForLiveMode(args: {
  cfg: HlOscarMajorsConfig;
  state: MajorsTraderState;
  ...
  for (const [id, pos] of [...state.opens.entries()]) {
    ...
    paperClosed += 1;
    console.warn(
      `[hl-oscar-majors:reconcile] closed paper journal open ${pos.coin} (${id.slice(0, 8)}) — not on exchange`,
    );
  }
  ...
  if (state.opens.size > 0) {
    ...
    for (const [id, pos] of [...state.opens.entries()]) {
      ...
      exchangeOrphans += 1;
      console.warn(
        `[hl-oscar-majors:reconcile] closed journal orphan ${pos.coin} (${id.slice(0, 8)}) — missing on HL`,
      );
    }
  }
  ...
  if (paperClosed > 0 || exchangeOrphans > 0) {
    console.log(
      `[hl-oscar-majors:reconcile] paperClosed=${paperClosed} exchangeOrphans=${exchangeOrphans} remaining=${state.opens.size}`,
    );
  }
...
export function countMajorsOpensByMode(state: MajorsTraderState): {
  live: number;
  paper: number;
} {
  let live = 0;
