 ...
 * within the lookback (typical one-shot wash / sybil burst тАФ e.g. GOAT: 3 active
...
export interface VolumeEphemeralFeatures {
  ...
  /** Live vol5m dead but PG neighbors healthy тАФ do not block. */
  singleTickStaleIgnored?: boolean;
  /** Audit flag when {@link singleTickStaleIgnored}. */
  staleIgnoreFlag?: string;
  /** @deprecated Familiar bypass removed тАФ audit field retained for journal compat. */
  familiarMintBypass?: boolean;
  /** PG hourly blocks skipped тАФ fresh Birdeye/DexScreener shows healthy vol spread. */
...
export interface VolumeEphemeralEvalOpts {
  /** Mint with prior bot trade in lookback тАФ relaxed tail / wash rules. */
  knownMint?: boolean;
  /** Fresh Birdeye/DexScreener quote on evalRow тАФ skip PG-blind ephemeral blocks when spread healthy. */
...
/**
 * True when PG neighbor hourly windows show sustained healthy vol5m тАФ one dead live
...
export function neighborVolumeHealthy(
  ...
  /** 24h active-hour count тАФ many active hours => not a one-off stale tick. */
  ...
  /** 2+ adjacent hours (prev3тЖТcurrent) with hourly max vol5m >= threshold. */
  const series = [
...
export function evaluateVolumeEphemeralGuard(
  ...
  /** Fresh Birdeye/DexScreener on evalRow: healthy vol spread proves live market тАФ skip PG-blind blocks. */
  if (
  ...
  if (!features.coverageOk) {
    /** PG hourly context missing тАФ still block obvious live-snapshot wash (MUSHU RCA 2026-06-30). */
  ...
  /** Legacy alias kept for tests/docs тАФ merged into notYetSustained tail above. */
  if (
    cfg.volumeEphemeralTailBlockEnabled &&
    peakSignificant &&
    peakToCurrent != null &&
