...
const ConfigSchema = z.object({
  ...
  rpcMinGapMs: z.coerce.number().int().min(20).max(500).default(55),
  meteredRpcEnabled: z.coerce.boolean().default(true),
  balanceCacheTtlMs: z.coerce.number().int().min(2000).max(120_000).default(10_000),
  exitMarkTtlMs: z.coerce.number().int().min(5000).max(120_000).default(20_000),
  exitMarkMaxStaleMs: z.coerce.number().int().min(10_000).max(180_000).default(45_000),
  exitQuotesPerTick: z.coerce.number().int().min(1).max(15).default(2),
  watchlistStreamPreferPg: z.coerce.boolean().default(true),
  watchlistStreamFreshMs: z.coerce.number().int().min(30_000).max(600_000).default(120_000),
  /** Min PG snapshot age for watchlist (minutes). */
  watchlistPgLookbackMin: z.coerce.number().int().min(15).max(10_080).default(360),
  /** Refresh on-chain spot for top dump candidates each tick. Off when stream feeds PG. */
  watchlistRpcRefreshEnabled: z.coerce.boolean().default(false),
  watchlistRpcRefreshPerTick: z.coerce.number().int().min(1).max(40).default(4),
...
export function loadPumpswapComboConfig(): PumpswapComboConfig {
  ...
  return ConfigSchema.parse({
    ...
    rpcMinGapMs: process.env.PUMPSWAP_COMBO_RPC_MIN_GAP_MS,
    meteredRpcEnabled: process.env.PUMPSWAP_COMBO_METERED_RPC,
    balanceCacheTtlMs: process.env.PUMPSWAP_COMBO_BALANCE_CACHE_MS,
    exitMarkTtlMs: process.env.PUMPSWAP_COMBO_EXIT_MARK_TTL_MS,
    exitMarkMaxStaleMs: process.env.PUMPSWAP_COMBO_EXIT_MARK_MAX_STALE_MS,
    exitQuotesPerTick: process.env.PUMPSWAP_COMBO_EXIT_QUOTES_PER_TICK,
    watchlistStreamPreferPg: process.env.PUMPSWAP_COMBO_WATCHLIST_STREAM_PREFER,
    watchlistStreamFreshMs: process.env.PUMPSWAP_COMBO_WATCHLIST_STREAM_FRESH_MS,
