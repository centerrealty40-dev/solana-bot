...
const LiveOscarConfigSchema = z
  .object({
    ...
    /** Block when `ret1h_pct ≤ −this` (percent points). Default 3% — только резкий часовой дамп. `0` = off. */
    liveBtcBlockNewBuys1hDrawdownPct: z.coerce.number().min(0).max(50).default(3),
    /** Block when `ret4h_pct ≤ −this` (percent points). Default 4%. `0` = off. */
    liveBtcBlockNewBuys4hDrawdownPct: z.coerce.number().min(0).max(50).default(4),
    /** Block when `ret24h_pct ≤ −this` (percent points). Default 5%. `0` = off. */
    liveBtcBlockNewBuys24hDrawdownPct: z.coerce.number().min(0).max(50).default(5),
