...
const LOOKBACK_HOURS = Number(process.env.RAYDIUM_BACKTEST_LOOKBACK_HOURS || 72);
const MIN_POINTS_PER_PAIR = Number(process.env.RAYDIUM_BACKTEST_MIN_POINTS || 20);
const MIN_LIQUIDITY_USD = Number(process.env.RAYDIUM_BACKTEST_MIN_LIQ_USD || 5_000);
const RUG_DROP_PCT = Number(process.env.RAYDIUM_BACKTEST_RUG_DROP_PCT || 70); // drop from local peak in 60m
const RUNNER_MULTIPLIER = Number(process.env.RAYDIUM_BACKTEST_RUNNER_MULTIPLIER || 3);
const REPORT_OUT_PATH = process.env.RAYDIUM_BACKTEST_OUT || '/opt/solana-alpha/data/raydium-backtest-report.json';
