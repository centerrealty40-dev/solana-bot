...
const LOOKBACK_HOURS = Number(process.env.MOONSHOT_BACKTEST_LOOKBACK_HOURS || 72);
const MIN_POINTS_PER_PAIR = Number(process.env.MOONSHOT_BACKTEST_MIN_POINTS || 20);
const MIN_LIQUIDITY_USD = Number(process.env.MOONSHOT_BACKTEST_MIN_LIQ_USD || 5_000);
const ENTRY_DELAY_MIN = Number(process.env.MOONSHOT_ENTRY_DELAY_MIN || 10);
const RUG_DROP_PCT = Number(process.env.MOONSHOT_RUG_DROP_PCT || 50);
const REPORT_OUT_PATH = process.env.MOONSHOT_BACKTEST_OUT || '/opt/solana-alpha/data/moonshot-backtest-report.json';
...
async function loadDenylistWallets() {
  ...
  return new Set(
    rows
      .map((r) => String(r.wallet ?? '').trim())
      .filter(Boolean),
  );
}

function printSummary(summary) {
  console.log('\n=== MOONSHOT BACKTEST (SCAFFOLD) ===');
