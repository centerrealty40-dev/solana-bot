 ...
 *        TIMEOUT 12 ч
 *
 * Storage: append-only JSONL `/tmp/paper-trades.jsonl`. Restart-friendly.
 *
 * Запуск:
...
async function discoveryTick(): Promise<void> {
  ...
  for (const row of rows) {
    ...
    append({
      kind: 'eval', mint: row.mint, symbol: row.symbol, ageMin: +ageMin.toFixed(1),
      pass: v.pass, reasons: v.reasons,
      m: { buyers: m.uniqueBuyers, sellers: m.uniqueSellers,
           buy_sol: +m.sumBuySol.toFixed(2), sell_sol: +m.sumSellSol.toFixed(2),
    ...
    if (DRY_RUN) {
      console.log(`[DRY] ${row.mint.slice(0, 8)} $${row.symbol} would PASS (b=${m.uniqueBuyers}, bc=${(m.bcProgress * 100).toFixed(0)}%)`);
      continue;
    }
    ...
    stats.opened++;
    append({ kind: 'open', ...ot });
    console.log(`[OPEN] ${row.mint.slice(0, 8)} $${row.symbol}  mc=$${(cur.mc / 1000).toFixed(1)}k  ` +
                `b=${m.uniqueBuyers}  bc=${(m.bcProgress * 100).toFixed(0)}%  top=${(m.topBuyerShare * 100).toFixed(0)}%`);
  }
}
...
async function main(): Promise<void> {
  ...
  console.log(`[init] SOL=$${SOL_USD.toFixed(2)}`);
