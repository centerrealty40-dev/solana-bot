...
const START_OFFSET_MS = Math.max(0, Number(process.env.PUMPSWAP_COLLECTOR_START_OFFSET_MS || 0));
const MAX_RETRIES = Number(process.env.PUMPSWAP_COLLECTOR_MAX_RETRIES || 4);
const ENRICH_MAX_RETRIES = Number(process.env.PUMPSWAP_COLLECTOR_ENRICH_MAX_RETRIES || 1);
...
async function main() {
  log('info', 'collector start', {
    ...
    enrichMaxRetries: ENRICH_MAX_RETRIES,
