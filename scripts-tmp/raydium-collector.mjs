...
const INTERVAL_MS = Number(process.env.RAYDIUM_COLLECTOR_INTERVAL_MS || 30_000);
...
const DEX_SEARCH_TERMS = (process.env.RAYDIUM_DEX_SEARCH_TERMS || 'raydium,solana,meme')
  .split(',')
  .map((v) => v.trim())
  .filter(Boolean);
...
function normalizeGeckoPool(poolData, bucketTs) {
  ...
  if (!dexName.includes('raydium')) return null;
...
async function fetchFromDexScreener(bucketTs) {
  ...
  for (const term of DEX_SEARCH_TERMS) {
    ...
    for (const p of pairs) {
      ...
      if (String(p?.dexId || '').toLowerCase() !== 'raydium') continue;
      const row = normalizeDexScreenerPair(p, bucketTs);
...
async function upsertSnapshots(rows) {
  ...
  const sql = `
    INSERT INTO raydium_pair_snapshots (
