...
/** Плитки Oscar на `/papertrader2`: live → copy-trader → HL TWAP → DCA live → V2.1 → V2.2. */
...
export const DASHBOARD_PAPER2_BUILD_ID = '2026-06-05-hl-twap-p11';
...
export type DashboardPaper2StrategyRow = {
  ...
  /** HL TWAP paper counters (panel 3). */
...
async function buildPaper2ApiPayload(): Promise<Record<string, unknown>> {
  ...
  const hlTwapRowP = buildHlTwapPaperDashboardRow()
    .then((row) => row as DashboardPaper2StrategyRow)
    .catch((e) => {
  ...
  const [liveRow, copyTraderRow, hlTwapRow, dcaLiveRow, paperV21Row, paperV22Row] =
    await Promise.all([
