...
const INTERVAL_MS = Number(process.env.ORCA_COLLECTOR_INTERVAL_MS || 30_000);
const MAX_RETRIES = Number(process.env.ORCA_COLLECTOR_MAX_RETRIES || 4);
const REQUEST_TIMEOUT_MS = Number(process.env.ORCA_COLLECTOR_TIMEOUT_MS || 15_000);
const DEX_SEARCH_TERMS = (process.env.ORCA_DEX_SEARCH_TERMS || 'orca,whirlpool,orca solana')
  .split(',')
  .map((v) => v.trim())
  .filter(Boolean);
const GECKO_TRENDING_PAGES = Number(process.env.ORCA_GECKO_TRENDING_PAGES || 2);
...
function looksLikeOrcaDexEntry(raw) {
  const text = [
    raw?.dexId,
    raw?.labels?.join?.(' '),
    raw?.url,
    ...
    .join(' ')
    .toLowerCase();
  return text.includes('orca') || text.includes('whirlpool');
}
...
function normalizeDexScreenerPair(pair, bucketTs) {
  ...
  if (!looksLikeOrcaDexEntry(pair)) return null;
...
function normalizeGeckoPool(poolData, bucketTs) {
  ...
  if (!dexName.includes('orca') && !dexName.includes('whirlpool')) return null;
...
async function upsertSnapshots(rows) {
  ...
  rows = await sanitizeSnapshotRows(pool, 'orca_pair_snapshots', rows);
  ...
  const sql = `
    INSERT INTO orca_pair_snapshots (
