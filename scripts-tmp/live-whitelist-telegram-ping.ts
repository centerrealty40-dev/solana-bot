...
function tokenFromEcosystem(): string | undefined {
  try {
    const p = path.join(process.cwd(), 'ecosystem.config.cjs');
    const s = fs.readFileSync(p, 'utf8');
    const m = s.match(/LIVE_MINT_WHITELIST_TELEGRAM_BOT_TOKEN:\s*['"]([^'"]+)['"]/);
    return m?.[1]?.trim();
  } catch {
    return undefined;
  }
...
async function main(): Promise<void> {
  const telegramBotToken =
    process.env.LIVE_MINT_WHITELIST_TELEGRAM_BOT_TOKEN?.trim() || tokenFromEcosystem();
  ...
  const o = { telegramBotToken, telegramChatId };
  await sendTagged(
    'ALERT',
    'live_whitelist_miss',
