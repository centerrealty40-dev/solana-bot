...
describe('mirror PM2 apps', () => {
  it('exports both mirror processes with independent leaders and shared strategy', () => {
    ...
    for (const key of [
      ...
      'MILD_DIP_MIRROR_ORPHAN_DUST_USD',
    ]) {
    ...
    expect(mirror?.env.MILD_DIP_MIRROR_ORPHAN_DUST_USD).toBe('0.1');
    expect(mirror2?.env.MILD_DIP_MIRROR_ORPHAN_DUST_USD).toBe('0.1');
    expect(ecosystemSource).toContain("MILD_DIP_MIRROR_ORPHAN_DUST_USD: '0.1'");
    ...
    expect(ecosystemSource).toContain("MILD_DIP_MIRROR_AVERAGE_MIN_DISCOUNT_PCT: '20'");
    ...
    expect(mirror2?.env.MILD_DIP_MIRROR_AVERAGE_USD).toBe('60');
