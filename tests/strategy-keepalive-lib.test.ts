...
describe('strategy-keepalive-lib', () => {
  it('parses pm2 jlist into status map', () => {
    const execSyncFn = vi.fn(() =>
      JSON.stringify([
        { name: 'mild-dip-bot', pm2_env: { status: 'online' } },
        { name: 'strategy-process-watch', pm2_env: { status: 'stopped' } },
      ]),
    );
    const map = getPm2StatusMap('/opt/solana-alpha', execSyncFn);
    expect(map.get('mild-dip-bot')).toBe('online');
    expect(map.get('strategy-process-watch')).toBe('stopped');
  });
  ...
  it('starts from ecosystem when app is missing', () => {
    ...
    const r = ensurePm2App({
      root: process.cwd(),
      pm2Name: 'mild-dip-bot',
      status: 'missing',
      execSyncFn,
    });
    expect(r.ok).toBe(true);
    expect(r.action).toBe('start');
    expect(calls[0]).toContain('pm2 start ecosystem.config.cjs --only mild-dip-bot');
  });
  ...
  it('restarts when stopped', () => {
    ...
    expect(String(execSyncFn.mock.calls[0][0])).toContain('pm2 restart mild-dip-bot');
  });
});
